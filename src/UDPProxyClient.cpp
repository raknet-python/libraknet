...
bool UDPProxyClient::RequestForwarding(
  ...
  outgoingBs.Write(
      (MessageID)ID_UDP_PROXY_FORWARDING_REQUEST_FROM_CLIENT_TO_COORDINATOR);
  outgoingBs.Write(sourceAddress);
  outgoingBs.Write(false);
  outgoingBs.Write(targetGuid);
  outgoingBs.Write(timeoutOnNoDataMS);
  if (serverSelectionBitstream &&
      serverSelectionBitstream->GetNumberOfBitsUsed() > 0) {
...
void UDPProxyClient::Update() {
  ...
  while (idx1 < pingServerGroups.Size()) {
    ...
    if (psg->serversToPing.Size() > 0 &&
        RakNet::GetTimeMS() >
            psg->startPingTime + DEFAULT_UNRESPONSIVE_PING_TIME_COORDINATOR) {
        ...
        switch (packet->data[1]) {
          ...
          case ID_UDP_PROXY_IN_PROGRESS: {
            ...
            if (packet->data[1] == ID_UDP_PROXY_FORWARDING_SUCCEEDED) {
              ...
              rakPeerInterface->Ping(
                  serverIP.C_String(), forwardingPort, false);
          ...
          case ID_UDP_PROXY_ALL_SERVERS_BUSY:
            if (resultHandler)
              resultHandler->OnAllServersBusy(
                  packet->systemAddress,
                  senderAddress,
                  targetAddress,
                  targetGuid,
          ...
          case ID_UDP_PROXY_NO_SERVERS_ONLINE:
            if (resultHandler)
              resultHandler->OnNoServersOnline(
                  packet->systemAddress,
                  senderAddress,
                  targetAddress,
                  targetGuid,
...
void UDPProxyClient::OnPingServers(Packet* packet) {
  ...
  char ipStr[64];
  for (serverListIndex = 0; serverListIndex < serverListSize;
       serverListIndex++) {
...
void UDPProxyClient::PingServerGroup::SendPingedServersToCoordinator(
