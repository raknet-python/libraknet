...
#define SHABLK0(i)                                           \
  (m_block->l[i] = (ROL32(m_block->l[i], 24) & 0xFF00FF00) | \
       (ROL32(m_block->l[i], 8) & 0x00FF00FF))
#else
...
#define SHABLK(i)                                             \
  (m_block->l[i & 15] = ROL32(                                \
       m_block->l[(i + 13) & 15] ^ m_block->l[(i + 8) & 15] ^ \
...
void CSHA1::Transform(UINT_32* pState, const UINT_8* pBuffer) {
  UINT_32 a = pState[0], b = pState[1], c = pState[2], d = pState[3],
          e = pState[4];
...
bool CSHA1::ReportHash(TCHAR* tszReport, REPORT_TYPE rtReportType) const {
  ...
  if ((rtReportType == REPORT_HEX) || (rtReportType == REPORT_HEX_SHORT)) {
    ...
    const TCHAR* lpFmt =
        ((rtReportType == REPORT_HEX) ? _T(" %02X") : _T("%02X"));
...
bool CSHA1::ReportHashStl(
    std::basic_string<TCHAR>& strOut,
    REPORT_TYPE rtReportType) const {
  TCHAR tszOut[84];
