...
bool CloudServer::GetRequest::AllRemoteServersHaveResponded() const {
  ...
  for (i = 0; i < remoteServerResponses.Size(); i++) {
    if (!remoteServerResponses[i]->gotResult) {
      return false;
    }
  }
  return true;
...
void CloudServer::BufferedGetResponseFromServer::Clear(
...
void CloudServer::OnPostRequest(Packet* packet) {
  ...
  if (maxUploadBytesPerClient > 0 &&
      dataLengthBytes > maxUploadBytesPerClient) {
  ...
  DataStructures::HashIndex remoteSystemsHashIndex =
      remoteSystems.GetIndexOf(packet->guid);
  ...
  CloudDataList* cloudDataList = GetOrAllocateCloudDataList(
      key, &dataRepositoryExists, dataRepositoryIndex);
  ...
  unsigned int keyDataListIndex =
      cloudDataList->keyData.GetIndexFromKey(packet->guid, &keyDataListExists);
  if (!keyDataListExists) {
    ...
    if (forceAddress != UNASSIGNED_SYSTEM_ADDRESS) {
      ...
      cloudData->serverSystemAddress =
          rakPeerInterface->GetExternalID(packet->systemAddress);
      if (cloudData->serverSystemAddress.IsLoopback()) {
        cloudData->serverSystemAddress.FromString(
            rakPeerInterface->GetLocalIP(0));
      }
    }
    if (cloudData->serverSystemAddress.GetPort() == 0) {
      // Fix localhost port
      cloudData->serverSystemAddress.SetPortHostOrder(
          rakPeerInterface->GetSocket(UNASSIGNED_SYSTEM_ADDRESS)
              ->GetBoundAddress()
              .GetPort());
    }
...
void CloudServer::OnReleaseRequest(Packet* packet) {
  ...
  DataStructures::HashIndex remoteSystemIndex =
      remoteSystems.GetIndexOf(packet->guid);
  ...
  for (uint16_t keyCountIndex = 0; keyCountIndex < keyCount; keyCountIndex++) {
    ...
    if (objectExists) {
      bool dataRepositoryExists;
      unsigned int dataRepositoryIndex =
          dataRepository.GetIndexFromKey(key, &dataRepositoryExists);
...
void CloudServer::OnGetRequest(Packet* packet) {
  ...
  GetServersWithUploadedKeys(
      getRequest->cloudQueryWithAddresses.cloudQuery.keys,
      remoteServersWithData);
  ...
  if (remoteServersWithData.Size() == 0) {
    ...
    for (unsigned int remoteServerIndex = 0;
      ...
      getRequest->remoteServerResponses.Insert(
          remoteServersWithData[remoteServerIndex]->serverAddress,
  ...
  if (getRequest->cloudQueryWithAddresses.cloudQuery.subscribeToResults) {
    ...
    for (keyIndex = 0;
      ...
      keySubscriberIndex = remoteCloudClient->subscribedKeys.GetIndexFromKey(
          cloudKey, &hasKeySubscriber);
      ...
      remoteCloudClient->subscribedKeys.InsertAtIndex(
          keySubscriberId, keySubscriberIndex, _FILE_AND_LINE_);

      // Add CloudData in a similar way
      unsigned int dataRepositoryIndex;
      bool dataRepositoryExists;
      CloudDataList* cloudDataList = GetOrAllocateCloudDataList(
          cloudKey, &dataRepositoryExists, dataRepositoryIndex);
      ...
      if (getRequest->cloudQueryWithAddresses.specificSystems.Size()) {
        ...
        subscribedKeysIndex = remoteCloudClient->subscribedKeys.GetIndexFromKey(
            cloudDataList->key, &subscribedKeysIndexExists);
        if (subscribedKeysIndexExists) {
          ...
          for (specificSystemIndex = 0; specificSystemIndex <
            ...
            unsigned int keyDataIndex = cloudDataList->keyData.GetIndexFromKey(
                keySubscriberId
                    ->specificSystemsSubscribedTo[specificSystemIndex],
                &keyDataExists);
            if (keyDataExists) {
...
void CloudServer::OnServerToServerGetResponse(Packet* packet) {
  ...
  bufferedGetResponseFromServer =
      getRequest->remoteServerResponses[remoteServerResponsesIndex];
...
void CloudServer::OnClosedConnection(
  ...
  if (objectExists) {
    ...
    while (getRequestIndex < getRequests.Size()) {
      ...
      unsigned int remoteServerResponsesIndex =
          getRequest->remoteServerResponses.GetIndexFromKey(
              rakNetGUID, &waitingForThisServer);
      if (waitingForThisServer) {
        getRequest->remoteServerResponses[remoteServerResponsesIndex]->Clear(
            this);
        RakNet::OP_DELETE(
            getRequest->remoteServerResponses[remoteServerResponsesIndex],
            _FILE_AND_LINE_);
        getRequest->remoteServerResponses.RemoveAtIndex(
            remoteServerResponsesIndex);
  ...
  DataStructures::HashIndex remoteSystemIndex =
      remoteSystems.GetIndexOf(rakNetGUID);
  if (!remoteSystemIndex.IsInvalid()) {
    RemoteCloudClient* remoteCloudClient =
        remoteSystems.ItemAtIndex(remoteSystemIndex);
    unsigned int uploadedKeysIndex;
    for (uploadedKeysIndex = 0;
      ...
      unsigned int dataRepositoryIndex = dataRepository.GetIndexFromKey(
          remoteCloudClient->uploadedKeys[uploadedKeysIndex],
          &keyDataRepositoryExists);
      if (keyDataRepositoryExists) {
        CloudDataList* cloudDataList = dataRepository[dataRepositoryIndex];
        bool keyDataExists;
        unsigned int keyDataIndex =
            cloudDataList->keyData.GetIndexFromKey(rakNetGUID, &keyDataExists);
        if (keyDataExists) {
    ...
    for (subscribedKeysIndex = 0;
      ...
      unsigned int keyDataRepositoryIndex = dataRepository.GetIndexFromKey(
          remoteCloudClient->subscribedKeys[subscribedKeysIndex]->key,
          &keyDataRepositoryExists);
      if (keyDataRepositoryExists) {
...
void CloudServer::RemoveServer(RakNetGUID systemAddress) {
  bool objectExists;
  unsigned int index =
      remoteServers.GetIndexFromKey(systemAddress, &objectExists);
  if (objectExists) {
...
void CloudServer::GetRemoteServers(
...
void CloudServer::ProcessAndTransmitGetRequest(GetRequest* getRequest) {
  ...
  cloudQueryResult.subscribeToResults =
      getRequest->cloudQueryWithAddresses.cloudQuery.subscribeToResults;
  ...
  if (unlimitedRows ||
    ...
    for (remoteServerResponseIndex = 0;
      ...
      for (cloudQueryRowIndex = 0; cloudQueryRowIndex <
        ...
        bufferedGetResponseFromServer->queryResult
            .rowsReturned[cloudQueryRowIndex]
            ->Serialize(true, &bsOut, this);
...
void CloudServer::SendUploadedAndSubscribedKeysToServer(
...
void CloudServer::OnServerDataChanged(Packet* packet) {
  ...
  unsigned int dataRepositoryIndex;
  dataRepositoryIndex =
      dataRepository.GetIndexFromKey(row.key, &dataRepositoryExists);
  if (!dataRepositoryExists) {
  ...
  NotifyClientSubscribersOfDataChange(
      &row, cloudDataList->nonSpecificSubscribers, wasUpdated);
  DeallocateRowData(row.data);
}
    ...
    if (!remoteServers[i]->workingFlag) {
      if (!remoteServers[i]->gotSubscribedAndUploadedKeys) {
        ...
        for (j = 0; j < keys.Size(); j++) {
          if (!remoteServers[i]->workingFlag &&
              remoteServers[i]->uploadedKeys.HasData(keys[j])) {
            remoteServers[i]->workingFlag = true;
            remoteServersWithData.Push(remoteServers[i], _FILE_AND_LINE_);
            break;
          }
    ...
    if (!cloudDataList->RemoveSubscriber(remoteCloudClientGuid)) {
      for (i = 0; i < keySubscriberId->specificSystemsSubscribedTo.Size();
        ...
        RemoveSpecificSubscriber(
            keySubscriberId->specificSystemsSubscribedTo[i],
            cloudDataList,
...
void CloudServer::RemoveSpecificSubscriber(
    RakNetGUID specificSubscriber,
    CloudDataList* cloudDataList,
    RakNetGUID remoteCloudClientGuid) {
  bool keyDataListExists;
  unsigned int keyDataListIndex = cloudDataList->keyData.GetIndexFromKey(
