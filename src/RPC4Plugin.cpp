...
struct GlobalRegistration {
  ...
  void (*registerBlockingFunctionPointer)(
...
void RPC4::RegisterSlot(
    ...
    int callPriority) {
  LocalSlotObject lso(
      nextSlotRegistrationCount++, callPriority, functionPointer);
  ...
  while (!gotBlockingReturnValue) {
    ...
    if (packet) {
      if ((packet->data[0] == ID_CONNECTION_LOST ||
          ...
          ((systemIdentifier.rakNetGuid != UNASSIGNED_RAKNET_GUID &&
            packet->guid == systemIdentifier.rakNetGuid) ||
        ...
        return false;
          ...
          packet->data[1] == RPC_ERROR_FUNCTION_NOT_REGISTERED) {
...
void RPC4::Signal(
  ...
  if (invokeLocal) {
    ...
    p.systemAddress =
        rakPeerInterface->GetInternalID(UNASSIGNED_SYSTEM_ADDRESS);
...
PluginReceiveResult RPC4::OnReceive(Packet* packet) {
  if (packet->data[0] == ID_RPC_PLUGIN) {
    ...
    if (packet->data[1] == ID_RPC4_CALL) {
      ...
      if (!isBlocking) {
        DataStructures::HashIndex skhi =
            registeredNonblockingFunctions.GetIndexOf(functionName.C_String());
        if (skhi.IsInvalid()) {
          ...
          bsOut.Write(
              ...
              (unsigned int)functionName.GetLength() + 1);
          SendUnified(
              &bsOut,
              HIGH_PRIORITY,
              RELIABLE_ORDERED,
        ...
        DataStructures::HashIndex skhi =
            registeredBlockingFunctions.GetIndexOf(functionName.C_String());
        if (skhi.IsInvalid()) {
          RakNet::BitStream bsOut;
          bsOut.Write((unsigned char)ID_RPC_REMOTE_ERROR);
          bsOut.Write((unsigned char)RPC_ERROR_FUNCTION_NOT_REGISTERED);
  ...
  if (objectExists) {
    ...
    for (index2 = 0; index2 < lc->functions.Size(); index2++) {
      ...
      DataStructures::HashIndex skhi =
          registeredNonblockingFunctions.GetIndexOf(
              lc->functions[index2].C_String());
      if (!skhi.IsInvalid()) {
        void (*fp)(RakNet::BitStream*, Packet*);
        fp = registeredNonblockingFunctions.ItemAtIndex(skhi);
        bsIn.AlignReadToByteBoundary();
