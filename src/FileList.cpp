    ...
    do {
      ...
      if ((fileInfo.attrib & (_A_HIDDEN | _A_SUBDIR | _A_SYSTEM)) == 0) {
        ...
        if (writeData && writeHash) {
          ...
          if (fp) {
            fileData = (char*)rakMalloc_Ex(
                fileInfo.size + HASH_LENGTH, _FILE_AND_LINE_);
            RakAssert(fileData);
            fread(fileData + HASH_LENGTH, fileInfo.size, 1, fp);
            fclose(fp);

            unsigned int hash =
                SuperFastHash(fileData + HASH_LENGTH, fileInfo.size);
...
void FileList::Serialize(RakNet::BitStream* outBitStream) {
  ...
  for (i = 0; i < fileList.Size(); i++) {
    ...
    StringCompressor::Instance()->EncodeString(
        fileList[i].filename.C_String(), MAX_FILENAME_LENGTH, outBitStream);
...
bool FileList::Deserialize(RakNet::BitStream* inBitStream) {
  ...
  for (i = 0; i < fileListSize; i++) {
    ...
    StringCompressor::Instance()->DecodeString(
        (char*)filename, MAX_FILENAME_LENGTH, inBitStream);
  ...
  for (thisIndex = 0; thisIndex < fileList.Size(); thisIndex++) {
    ...
    if (dirSubsetLen > 0 &&
        (localPathLen < dirSubsetLen ||
         _strnicmp(
             fileList[thisIndex].filename.C_String(),
             dirSubset,
             dirSubsetLen) != 0 ||
         (localPathLen > dirSubsetLen &&
          IsSlash(fileList[thisIndex].filename[dirSubsetLen]) == false)))
      continue;
    ...
    for (inputIndex = 0; inputIndex < input->fileList.Size(); inputIndex++) {
      ...
      if (_stricmp(
        ...
        match = true;
        if (input->fileList[inputIndex].fileLengthBytes ==
                fileList[thisIndex].fileLengthBytes &&
            input->fileList[inputIndex].dataLengthBytes ==
                fileList[thisIndex].dataLengthBytes &&
            memcmp(
        ...
        if (writeFileHash) {
          if (writeFileData) {
            ...
            fileList[i].data = (char*)rakMalloc_Ex(
                fileList[i].fileLengthBytes + HASH_LENGTH, _FILE_AND_LINE_);
            RakAssert(fileList[i].data);
            fread(
                fileList[i].data + HASH_LENGTH,
            ...
            //						sha1.Final();
            unsigned int hash = SuperFastHash(
                fileList[i].data + HASH_LENGTH, fileList[i].fileLengthBytes);
            if (RakNet::BitStream::DoEndianSwap())
              RakNet::BitStream::ReverseBytesInPlace(
                  (unsigned char*)&hash, sizeof(hash));
            ...
            if (fileList[i].fileLengthBytes < HASH_LENGTH)
              fileList[i].data =
                  (char*)rakMalloc_Ex(HASH_LENGTH, _FILE_AND_LINE_);
            else
              fileList[i].data = (char*)rakMalloc_Ex(
          ...
          fileList[i].data =
              (char*)rakMalloc_Ex(fileList[i].fileLengthBytes, _FILE_AND_LINE_);
          RakAssert(fileList[i].data);
          fread(fileList[i].data, fileList[i].fileLengthBytes, 1, fp);
        }
...
void FileList::WriteDataToDisk(const char* applicationDirectory) {
  ...
  for (i = 0; i < fileList.Size(); i++) {
    ...
    for (j = 1; j < fileList[i].filename.GetLength(); j++) {
      if (fileList[i].filename[j] == '.' &&
          fileList[i].filename[j - 1] == '.') {
#ifdef _DEBUG
        RakAssert(0);
#endif
    ...
    WriteFileWithDirectories(
        fullPath, fileList[i].data, (unsigned int)fileList[i].dataLengthBytes);
  }
}

#ifdef _MSC_VER
#pragma warning( \
    disable : 4996) // unlink declared deprecated by Microsoft in order to make it harder to be cross platform.  I don't agree it's deprecated.
#endif
void FileList::DeleteFiles(const char* applicationDirectory) {
  ...
  for (i = 0; i < fileList.Size(); i++) {
    ...
    if (result != 0) {
      RAKNET_DEBUG_PRINTF(
          "FileList::DeleteFiles: unlink (%s) failed.\n", fullPath);
    }
  }
}
...
bool FileList::FixEndingSlash(char* str) {
  ...
  if (str[strlen(str) - 1] != '/' && str[strlen(str) - 1] != '\\') {
    strcat(
        str,
        "\\"); // Only \ works with system commands, used by AutopatcherClient
    return true;
  }
#else
  if (str[strlen(str) - 1] != '\\' && str[strlen(str) - 1] != '/') {
