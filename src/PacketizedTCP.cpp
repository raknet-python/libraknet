  ...
  while (incomingPacket) {
    ...
    if (incomingPacket->deleteData) {
      ...
      if (index < connections.Size()) {
        ...
        // Buffer data
        bq->WriteBytes(
            (const char*)incomingPacket->data,
        ...
        if (bq->GetBytesWritten() >= dataLength + sizeof(PTCPHeader)) {
          ...
          unsigned int oldWritten =
              bq->GetBytesWritten() - incomingPacket->length;
          ...
          if (newWritten / 65536 != oldWritten / 65536) {
            ...
            outgoingPacket->length = sizeof(MessageID) +
                sizeof(unsigned int) * 2 + sizeof(unsigned int) + 65536;
            ...
            memcpy(
                outgoingPacket->data + sizeof(MessageID),
                &partIndex,
                sizeof(unsigned int));
            memcpy(
                outgoingPacket->data + sizeof(MessageID) +
            ...
            bq->IncrementReadOffset(sizeof(PTCPHeader));
            bq->ReadBytes(
                (char*)outgoingPacket->data + sizeof(MessageID) +
