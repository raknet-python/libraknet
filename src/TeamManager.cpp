...
int TM_World::JoinRequestHelperComp(
...
TeamSelection::TeamSelection(JoinTeamType itt, TM_Team* param)
    : joinTeamType(itt) {
...
TeamSelection::TeamSelection(JoinTeamType itt, NoTeamId param)
    : joinTeamType(itt) {
...
bool TM_TeamMember::RequestTeam(TeamSelection teamSelection) {
  if (teamSelection.joinTeamType == JOIN_NO_TEAM) {
    ...
    if (teams.Size() == 0 &&
        noTeamSubcategory == teamSelection.teamParameter.noTeamSubcategory) {
      // No change
      return false;
    }
    ...
    bsOut.Write(world->GetWorldId());
    bsOut.Write(networkId);
    world->GetTeamManager()->SendUnified(
        &bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, world->GetHost(), false);
  } else {
...
bool TM_TeamMember::RequestTeamSwitch(
    TM_Team* teamToJoin,
    TM_Team* teamToLeave) {
...
void TM_TeamMember::GetRequestedSpecificTeams(
    DataStructures::List<TM_Team*>& requestedTeams) const {
...
void TM_TeamMember::GetCurrentTeams(
    DataStructures::List<TM_Team*>& _teams) const {
  _teams = teams;
}
...
void TM_TeamMember::SerializeConstruction(BitStream* constructionBitstream) {
  ...
  for (unsigned int i = 0; i < teamsRequested.Size(); i++) {
    ...
    if (teamsRequested[i].teamToLeave) {
      ...
      constructionBitstream->Write(
          teamsRequested[i].teamToLeave->GetNetworkID());
    } else {
...
bool TM_TeamMember::DeserializeConstruction(
...
bool TM_TeamMember::JoinSpecificTeamCheck(
...
bool TM_TeamMember::SwitchSpecificTeamCheck(
    TM_Team* teamToJoin,
    TM_Team* teamToLeave,
    bool ignoreRequested) const {
...
void TM_TeamMember::AddToRequestedTeams(
    TM_Team* teamToJoin,
    TM_Team* teamToLeave) {
  RemoveFromRequestedTeams(teamToJoin);
...
void TM_Team::GetTeamMembers(
    DataStructures::List<TM_TeamMember*>& _teamMembers) const {
...
bool TM_Team::DeserializeConstruction(
    TeamManager* teamManager,
    BitStream* constructionBitstream) {
  WorldId worldId;
  constructionBitstream->Read(worldId);
...
void TM_World::AddParticipant(RakNetGUID rakNetGUID) {
  ...
  if (GetTeamManager()->GetMyGUIDUnified() == GetHost()) {
    ...
    teamManager->SendUnified(
        &bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, rakNetGUID, false);
  }
}
...
void TM_World::GetParticipantList(
  ...
  for (i = 0; i < teams.Size(); i++) {
    if (teams[i] == team) {
      ...
      while (team->teamMembers.Size()) {
        team->teamMembers[team->teamMembers.Size() - 1]->LeaveTeam(
            team, noTeamSubcategory);
      }
      teams.RemoveAtIndex(i);
...
void TM_World::ReferenceTeamMember(
...
unsigned int TM_World::GetTeamMemberIndex(
    const TM_TeamMember* teamMember) const {
  unsigned int i;
  for (i = 0; i < teamMembers.Size(); i++) {
    if (teamMembers[i] == teamMember) {
      return i;
...
bool TM_World::SetBalanceTeams(bool balanceTeams, NoTeamId noTeamId) {
  ...
  bsOut.Write(noTeamId);
  GetTeamManager()->SendUnified(
      &bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, GetHost(), false);
...
void TM_World::OnClosedConnection(
...
void TM_World::KickExcessMembers(NoTeamId noTeamId) {
  ...
  for (i = 0; i < teams.Size(); i++) {
    ...
    while (team->GetMemberLimitSetting() < team->GetTeamMembersCount() ||
           (balancedTeamLimit < team->GetTeamMembersCount() &&
            team->GetBalancingApplies())) {
      TM_TeamMember* teamMember =
          team->teamMembers[team->teamMembers.Size() - 1];

      teamIndex = GetAvailableTeamIndexWithFewestMembers(
          balancedTeamLimit, ALLOW_JOIN_REBALANCING);
      if (teamIndex == (unsigned int)-1) {
        ...
        teamManager->RemoveFromTeamsRequestedAndAddTeam(
            teamMember, teamToJoin, true, team);
...
void TM_World::FillRequestedSlots() {
  ...
  DataStructures::OrderedList<
      TM_World::JoinRequestHelper,
      TM_World::JoinRequestHelper,
      JoinRequestHelperComp>
      joinRequests;
  GetSortedJoinRequests(joinRequests);
  unsigned int joinRequestIndex;

  for (joinRequestIndex = 0; joinRequestIndex < joinRequests.Size();
       joinRequestIndex++) {
    ...
    if (teamIndex != (unsigned int)-1) {
      ...
      if (teamMember->teamsRequested.Size() == 0) {
        ...
        bool isSwitch =
            teamMember->teamsRequested[indexIntoTeamsRequested].isTeamSwitch;
        TM_Team* teamToLeave;
        if (isSwitch) {
          teamToLeave =
              teamMember->teamsRequested[indexIntoTeamsRequested].teamToLeave;
          if (teamToLeave) {
...
unsigned int TM_World::GetAvailableTeamIndexWithFewestMembers(
    TeamMemberLimit secondaryLimit,
...
void TM_World::GetSortedJoinRequests(DataStructures::OrderedList<
                                     TM_World::JoinRequestHelper,
                                     TM_World::JoinRequestHelper,
                                     JoinRequestHelperComp>& joinRequests) {
  unsigned int i;
...
void TM_World::BroadcastToParticipants(
    RakNet::BitStream* bsOut,
    RakNetGUID exclusionGuid) {
...
TM_Team* TM_World::JoinAnyTeam(TM_TeamMember* teamMember, int* resultCode) {
  ...
  unsigned int idx = GetAvailableTeamIndexWithFewestMembers(
      balancedLimit, ALLOW_JOIN_ANY_AVAILABLE_TEAM);
  if (idx == (unsigned int)-1) {
    ...
    for (idx = 0; idx < teams.Size(); idx++) {
      if ((teams[idx]->GetTeamMembersCount() >= balancedLimit ||
           teams[idx]->GetTeamMembersCount() >=
               teams[idx]->GetMemberLimitSetting()) &&
          teams[idx]->GetMemberLimitSetting() != 0 &&
          (ALLOW_JOIN_ANY_AVAILABLE_TEAM & teams[idx]->GetJoinPermissions()) !=
              0) {
        // Full
        *resultCode = -2;
        return teams[idx];
...
int TM_World::JoinSpecificTeam(
  ...
  if (team->GetJoinPermissions() & ALLOW_JOIN_SPECIFIC_TEAM) {
    if (!balanceTeamsIsActive || teamsWeAreLeaving.Size() == 0) {
      if (team->GetMemberLimit() > team->GetTeamMembersCount()) {
        // Can join normally
        teamMember->StoreLastTeams();
        teamManager->RemoveFromTeamsRequestedAndAddTeam(
            teamMember, team, isTeamSwitch, teamToLeave);
        return 1;
      } else {
        // Full
        return -2;
      ...
      if (teamsWeAreLeaving.Size() == 1) {
        unsigned int j =
            team->GetMemberWithRequestedSingleTeamSwitch(teamsWeAreLeaving[0]);
        if (j != (unsigned int)-1) {
...
TeamMemberLimit TM_World::GetBalancedTeamLimit() const {
  ...
  } while (additionalTeamsExcluded && teamsCount > 0);
...
void TeamManager::EncodeTeamFull(
    RakNet::BitStream* bitStream,
    TM_TeamMember* teamMember,
    TM_Team* team) {
...
void TeamManager::DecomposeTeamFull(
    Packet* packet,
    TM_World** world,
    TM_TeamMember** teamMember,
    TM_Team** team,
  ...
  DecomposeTeamFullOrLocked(
      &bsIn,
      world,
      teamMember,
      team,
      currentMembers,
      memberLimitIncludingBalancing,
      balancingIsActive,
      joinPermissions);
}
...
void TeamManager::EncodeTeamLocked(
    RakNet::BitStream* bitStream,
    TM_TeamMember* teamMember,
    TM_Team* team) {
...
void TeamManager::EncodeTeamFullOrLocked(
    RakNet::BitStream* bitStream,
    TM_TeamMember* teamMember,
    TM_Team* team) {
...
void TeamManager::DecomposeTeamFullOrLocked(
...
void TeamManager::DecomposeTeamLocked(
    Packet* packet,
    TM_World** world,
    TM_TeamMember** teamMember,
    TM_Team** team,
...
void TeamManager::EncodeTeamAssigned(
    RakNet::BitStream* bitStream,
    TM_TeamMember* teamMember) {
  bitStream->Write(teamMember->world->GetWorldId());
  bitStream->Write(teamMember->GetNetworkID());
...
void TeamManager::ProcessTeamAssigned(RakNet::BitStream* bsIn) {
  ...
  if (teamMember) {
    ...
    for (unsigned int i = 0; i < teamsJoined.Size(); i++) {
      if (!teamMember->IsOnTeam(teamsJoined[i])) {
...
void TeamManager::DecodeTeamAssigned(
    Packet* packet,
    TM_World** world,
    TM_TeamMember** teamMember) {
  WorldId worldId;
...
void TeamManager::DecodeTeamCancelled(
    Packet* packet,
    TM_World** world,
    TM_TeamMember** teamMember,
...
void TeamManager::OnClosedConnection(
    const SystemAddress& systemAddress,
    RakNetGUID rakNetGUID,
    PI2_LostConnectionReason lostConnectionReason) {
  for (unsigned int i = 0; i < worldsList.Size(); i++) {
    worldsList[i]->OnClosedConnection(
        systemAddress, rakNetGUID, lostConnectionReason);
  }
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TeamManager::OnNewConnection(
    const SystemAddress& systemAddress,
    RakNetGUID rakNetGUID,
    bool isIncoming) {
  for (unsigned int i = 0; i < worldsList.Size(); i++) {
...
void TeamManager::RemoveFromTeamsRequestedAndAddTeam(
    TM_TeamMember* teamMember,
    TM_Team* team,
    bool isTeamSwitch,
    TM_Team* teamToLeave) {
...
void TeamManager::OnJoinRequestedTeam(Packet* packet, TM_World* world) {
  ...
  if (teamToJoin && teamMember) {
    ...
    int resultCode = world->JoinSpecificTeam(
        teamMember, teamToJoin, isTeamSwitch, teamToLeave, teamsWeAreLeaving);
...
void TeamManager::OnUpdateTeamsRequestedToNoneAndAddTeam(
    Packet* packet,
    TM_World* world) {
  BitStream bsIn(packet->data, packet->length, false);
  bsIn.IgnoreBytes(2 + sizeof(WorldId));
...
void TeamManager::OnRemoveFromTeamsRequestedAndAddTeam(
    Packet* packet,
    TM_World* world) {
  BitStream bsIn(packet->data, packet->length, false);
  bsIn.IgnoreBytes(2 + sizeof(WorldId));
