...
static const CCTimeType HISTOGRAM_RESTART_CYCLE =
    10000; // Every 10 seconds reset the histogram
#else
...
static const CCTimeType STARTING_TIME_BETWEEN_PACKETS =
...
BPSTracker::TimeAndValue2::TimeAndValue2() = default;
BPSTracker::TimeAndValue2::~TimeAndValue2() = default;

BPSTracker::TimeAndValue2::TimeAndValue2(RakNet::TimeUS t, uint64_t v1)
    : value1(v1), time(t) {}
...
int RakNet::SplitPacketChannelComp(
...
int SplitPacketIndexComp(
    SplitPacketIndexType const& key,
    InternalPacket* const& data) {
...
ReliabilityLayer::ReliabilityLayer() {
  ...
  internalPacketPool.SetPageSize(
      sizeof(InternalPacket) * INTERNAL_PACKET_PAGE_SIZE);
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::Reset(
    bool resetVariables,
    int MTUSize,
    bool _useSecurity) {
  FreeMemory(
      true); // true because making a memory reset pending in the update cycle causes resets after reconnects.  Instead, just call Reset from a single thread
  if (resetVariables) {
    InitializeVariables();
...
void ReliabilityLayer::InitializeVariables() {
  memset(
      orderedWriteIndex,
      0,
      NUMBER_OF_ORDERED_STREAMS * sizeof(OrderingIndexType));
  memset(
      sequencedWriteIndex,
      0,
      NUMBER_OF_ORDERED_STREAMS * sizeof(OrderingIndexType));
  ...
  for (i = 0; i < splitPacketChannelList.Size(); i++) {
    for (j = 0; j < splitPacketChannelList[i]->splitPacketList.Size(); j++) {
      FreeInternalPacketData(
          splitPacketChannelList[i]->splitPacketList[j], _FILE_AND_LINE_);
      ReleaseToInternalPacketPool(
          splitPacketChannelList[i]->splitPacketList[j]);
    }
#if PREALLOCATE_LARGE_MESSAGES == 1
    if (splitPacketChannelList[i]->returnedPacket) {
      FreeInternalPacketData(
          splitPacketChannelList[i]->returnedPacket, __FILE__, __LINE__);
...
bool ReliabilityLayer::HandleSocketReceiveFromConnectedPlayer(
  ...
  if (length <= 2 ||
      buffer ==
          nullptr) // Length of 1 is a connection request resend that we just ignore
  {
    for (unsigned int messageHandlerIndex = 0;
         messageHandlerIndex < messageHandlerList.Size();
         messageHandlerIndex++) {
  ...
  if (dhf.isACK) {
    ...
    for (i = 0; i < incomingAcks.ranges.Size(); i++) {
      if (incomingAcks.ranges[i].minIndex > incomingAcks.ranges[i].maxIndex ||
        ...
        RakAssert(
            incomingAcks.ranges[i].minIndex <= incomingAcks.ranges[i].maxIndex);

        for (unsigned int messageHandlerIndex = 0;
             messageHandlerIndex < messageHandlerList.Size();
             messageHandlerIndex++) {
          messageHandlerList[messageHandlerIndex]
      ...
      for (datagramNumber = incomingAcks.ranges[i].minIndex;
           ...
           datagramNumber <= incomingAcks.ranges[i].maxIndex;
           datagramNumber++) {
        CCTimeType whenSent;
        ...
        if (unreliableWithAckReceiptHistory.Size() > 0) {
          ...
          while (k < unreliableWithAckReceiptHistory.Size()) {
            if (unreliableWithAckReceiptHistory[k].datagramNumber ==
                datagramNumber) {
              ...
              memcpy(
                  ackReceipt->data + sizeof(MessageID),
        ...
        MessageNumberNode* messageNumberNode =
            GetMessageNumberNodeByDatagramIndex(datagramNumber, &whenSent);
        if (messageNumberNode) {
          ...
          while (messageNumberNode) {
            ...
            RemovePacketFromResendListAndDeleteOlderReliableSequenced(
                messageNumberNode->messageNumber,
                timeRead,
                messageHandlerList,
                systemAddress);
    ...
    for (i = 0; i < incomingNAKs.ranges.Size(); i++) {
      if (incomingNAKs.ranges[i].minIndex > incomingNAKs.ranges[i].maxIndex) {
        RakAssert(
            incomingNAKs.ranges[i].minIndex <= incomingNAKs.ranges[i].maxIndex);
        ...
        for (unsigned int messageHandlerIndex = 0;
          ...
          messageHandlerList[messageHandlerIndex]
              ->OnReliabilityLayerNotification(
                  "incomingNAKs minIndex>maxIndex",
                  BYTES_TO_BITS(length),
                  systemAddress,
                  true);
        }
      ...
      for (messageNumber = incomingNAKs.ranges[i].minIndex;
        ...
        CCTimeType timeSent;
        MessageNumberNode* messageNumberNode =
            GetMessageNumberNodeByDatagramIndex(messageNumber, &timeSent);
        while (messageNumberNode) {
          // Update timers so resends occur immediately
          InternalPacket* internalPacket = resendBuffer
              [messageNumberNode->messageNumber &
               (uint32_t)RESEND_BUFFER_ARRAY_MASK];
          if (internalPacket) {
    ...
    if (!congestionManager.OnGotPacket(
            dhf.datagramNumber,
            dhf.isContinuousSend,
            timeRead,
            length,
            &skippedMessageCount)) {
      for (unsigned int messageHandlerIndex = 0;
    ...
    for (skippedMessageOffset = skippedMessageCount; skippedMessageOffset > 0;
    ...
    InternalPacket* internalPacket =
        CreateInternalPacketFromBitStream(&socketData, timeRead);
    ...
    while (internalPacket) {
      for (unsigned int messageHandlerIndex = 0;
        ...
        messageHandlerList[messageHandlerIndex]->OnInternalPacket(
            internalPacket, receivePacketCount, systemAddress, timeRead, false);
#else
        messageHandlerList[messageHandlerIndex]->OnInternalPacket(
            internalPacket,
            receivePacketCount,
...

      {
        ...
        if (resetReceivedPackets) {
          hasReceivedPacketQueue.ClearAndForceAllocation(
              DEFAULT_HAS_RECEIVED_PACKET_QUEUE_SIZE, _FILE_AND_LINE_);
          receivedPacketsBaseIndex = 0;
        ...
        if (internalPacket->reliability == RELIABLE ||
          ...
          holeCount = (DatagramSequenceNumberType)(internalPacket
          ...
          if (holeCount == (DatagramSequenceNumberType)0) {
            ...
            if (hasReceivedPacketQueue
                    [holeCount]) // non-zero means this is a hole
            {
              ...
              // Fill in the hole
              hasReceivedPacketQueue[holeCount] =
                  false; // We got the packet at holeCount
            } else {
              bpsMetrics[(int)USER_MESSAGE_BYTES_RECEIVED_IGNORED].Push1(
                  timeRead, BITS_TO_BYTES(internalPacket->dataBitLength));
          ...
          } else // holeCount>=receivedPackets.Size()
          {
            ...
            while ((unsigned int)(holeCount) > hasReceivedPacketQueue.Size()) {
              hasReceivedPacketQueue.Push(
                  true,
                  _FILE_AND_LINE_); // time+(CCTimeType)60 * (CCTimeType)1000 * (CCTimeType)1000); // Didn't get this packet - set the time to give up waiting
            }
            hasReceivedPacketQueue.Push(
                false, _FILE_AND_LINE_); // Got the packet
#ifdef _DEBUG
            // If this assert hits then DatagramSequenceNumberType has overflowed
            RakAssert(
                hasReceivedPacketQueue.Size() <
                (unsigned int)((DatagramSequenceNumberType)(const uint32_t)(
                    -1)));
#endif
          }

          while (hasReceivedPacketQueue.Size() > 0 &&
                 !hasReceivedPacketQueue.Peek()) {
            hasReceivedPacketQueue.Pop();
            ++receivedPacketsBaseIndex;
          }
        }
        ...
        if (hasReceivedPacketQueue.AllocationSize() >
                (unsigned int)DEFAULT_HAS_RECEIVED_PACKET_QUEUE_SIZE &&
            hasReceivedPacketQueue.AllocationSize() >
                hasReceivedPacketQueue.Size() * 3) {
        ...
        if (internalPacket->splitPacketCount > 0) {
          ...
          if (internalPacket->reliability != RELIABLE_ORDERED &&
            ...
            internalPacket->orderingChannel =
                255; // Use 255 to designate not sequenced and not ordered
          }
        ...
        if (internalPacket->reliability == RELIABLE_SEQUENCED ||
          ...
          if (internalPacket->orderingIndex ==
              orderedReadIndex[internalPacket->orderingChannel]) {
            // Has current ordering index
            if (internalPacket->reliability == RELIABLE_SEQUENCED ||
                internalPacket->reliability == UNRELIABLE_SEQUENCED) {
              // Is sequenced
              if (!IsOlderOrderedPacket(
                      internalPacket->sequencingIndex,
                      highestSequencedReadIndex[internalPacket
                                                    ->orderingChannel])) {
                ...
                if (packetId == ID_USER_PACKET_ENUM + 1 && fp) {
                  fprintf(
                      ...
                      type,
                      internalPacket->orderingIndex.val,
                      internalPacket->sequencingIndex);
                  fflush(fp);
                }
              ...
              if (packetId == ID_USER_PACKET_ENUM + 1 && fp) {
                fprintf(
                    fp,
                    "outputting immediate %i, %s. OI=%i. SI=%i.",
                    receivedPacketNumber,
                    type,
                    internalPacket->orderingIndex.val,
                    internalPacket->sequencingIndex);
              ...
              while (orderingHeaps[internalPacket->orderingChannel].Size() >
                         0 &&
                     orderingHeaps[internalPacket->orderingChannel]
                             .Peek()
                             ->orderingIndex ==
                         orderedReadIndex[internalPacket->orderingChannel]) {
                internalPacket =
                    orderingHeaps[internalPacket->orderingChannel].Pop(0);
            ...
            orderingHeaps[internalPacket->orderingChannel].Push(
                weight, internalPacket, _FILE_AND_LINE_);

#ifdef PRINT_TO_FILE_RELIABLE_ORDERED_TEST
            if (packetId == ID_USER_PACKET_ENUM + 1 && fp) {
              fprintf(
                  fp,
                  "Heap push %i, %s, weight=%" PRINTF_64_BIT_MODIFIER
                  "u. OI=%i. waiting on %i. SI=%i.\n",
                  receivedPacketNumber,
                  type,
                  weight,
                  internalPacket->orderingIndex.val,
                  orderedReadIndex[internalPacket->orderingChannel].val,
                  internalPacket->sequencingIndex);
              fflush(fp);
            }
#endif
...
bool ReliabilityLayer::Send(
  ...
  auto numberOfBytesToSend = (unsigned int)BITS_TO_BYTES(numberOfBitsToSend);
  ...
  bpsMetrics[(int)USER_MESSAGE_BYTES_PUSHED].Push1(
      currentTime, numberOfBytesToSend);

  internalPacket->creationTime = currentTime;

  if (makeDataCopy) {
    AllocInternalPacketData(
        internalPacket, numberOfBytesToSend, true, _FILE_AND_LINE_);
  ...
  outgoingPacketBuffer.Push(
      GetNextWeight(internalPacket->priority), internalPacket, _FILE_AND_LINE_);
  RakAssert(
      outgoingPacketBuffer.Size() == 0 ||
      outgoingPacketBuffer.Peek()->dataBitLength <
          BYTES_TO_BITS(MAXIMUM_MTU_SIZE));
...
#pragma warning( \
...
#else
  if (statistics.messagesInResendBuffer != 0 &&
      AckTimeout(RakNet::TimeMS(time / (CCTimeType)1000)))
#endif
  {
  ...
  if (NAKs.Size() > 0) {
    ...
    NAKs.Serialize(
        &updateBitStream, GetMaxDatagramSizeExcludingMessageHeaderBits(), true);
  ...
  const bool hasDataToSendOrResend =
      !IsResendQueueEmpty() || bandwidthExceededStatistic;
  RakAssert(NUMBER_OF_PRIORITIES == 4);
  congestionManager.Update(time, hasDataToSendOrResend);

  statistics.BPSLimitByOutgoingBandwidthLimit =
      BITS_TO_BYTES(bitsPerSecondLimit);
  statistics.BPSLimitByCongestionControl =
      congestionManager.GetBytesPerSecondLimitByCongestionControl();
  ...
  if (unreliableWithAckReceiptHistory.Size() > 0) {
    ...
    while (i < unreliableWithAckReceiptHistory.Size()) {
      ...
      if (time - unreliableWithAckReceiptHistory[i].nextActionTime <
          (((CCTimeType)-1) / 2)) {
        InternalPacket* ackReceipt = AllocateFromInternalPacketPool();
        AllocInternalPacketData(ackReceipt, 5, false, _FILE_AND_LINE_);
        ackReceipt->dataBitLength = BYTES_TO_BITS(5);
        ...
        memcpy(
            ackReceipt->data + sizeof(MessageID),
            &unreliableWithAckReceiptHistory[i].sendReceiptSerial,
            sizeof(uint32_t));
        outputQueue.Push(ackReceipt, _FILE_AND_LINE_);
  ...
  if (hasDataToSendOrResend) {
    ...
    int transmissionBandwidth = congestionManager.GetTransmissionBandwidth(
    ...
    if (retransmissionBandwidth > 0 || transmissionBandwidth > 0) {
      ...
      while ((int)BITS_TO_BYTES(allDatagramSizesSoFar) <
             retransmissionBandwidth) {
        ...
        while (!IsResendQueueEmpty()) {
          ...
          if (time - internalPacket->nextActionTime < (((CCTimeType)-1) / 2)) {
            nextPacketBitLength =
                internalPacket->headerLength + internalPacket->dataBitLength;
            if (datagramSizeSoFar + nextPacketBitLength >
            ...
            CC_DEBUG_PRINTF_2(
                "Rs %i ", internalPacket->reliableMessageNumber.val);

            bpsMetrics[(int)USER_MESSAGE_BYTES_RESENT].Push1(
                time, BITS_TO_BYTES(internalPacket->dataBitLength));
    ...
    if ((int)BITS_TO_BYTES(allDatagramSizesSoFar) < transmissionBandwidth) {
      ...
      while (
        ...
        statistics.isLimitedByOutgoingBandwidthLimit =
            ...
            BITS_TO_BYTES(bitsPerSecondLimit) <
                bpsMetrics[USER_MESSAGE_BYTES_SENT].GetBPS1(time);
        ...
        //while ( sendPacketSet[ i ].Size() )
        {
          ...
          internalPacket->headerLength =
              GetMessageHeaderLengthBits(internalPacket);
          nextPacketBitLength =
              internalPacket->headerLength + internalPacket->dataBitLength;
          if (datagramSizeSoFar + nextPacketBitLength >
              GetMaxDatagramSizeExcludingMessageHeaderBits()) {
...
*/
            ...
            if (resendBuffer
                    [internalPacket->reliableMessageNumber &
                     (uint32_t)RESEND_BUFFER_ARRAY_MASK] != nullptr) {
              //								bool overflow = ResendBufferOverflow();
              RakAssert(0);
            }
            resendBuffer
                [internalPacket->reliableMessageNumber &
                 (uint32_t)RESEND_BUFFER_ARRAY_MASK] = internalPacket;
          ...
          bpsMetrics[(int)USER_MESSAGE_BYTES_SENT].Push1(
              time, BITS_TO_BYTES(internalPacket->dataBitLength));
    ...
    for (unsigned int datagramIndex = 0;
         datagramIndex < packetsToSendThisUpdateDatagramBoundaries.Size();
         datagramIndex++) {
      if (datagramIndex > 0) {
      ...
      dhf.datagramNumber =
          congestionManager.GetAndIncrementNextDatagramSequenceNumber();
      dhf.isPacketPair = datagramsToSendThisUpdateIsPair[datagramIndex];

      //printf("%p pushing datagram %i\n", this, dhf.datagramNumber.val);

      bool isSecondOfPacketPair = dhf.isPacketPair && datagramIndex > 0 &&
          datagramsToSendThisUpdateIsPair[datagramIndex - 1];
      ...
      while (msgIndex < msgTerm) {
        ...
        if (packetsToSendThisUpdate[msgIndex]->reliability != UNRELIABLE &&
            packetsToSendThisUpdate[msgIndex]->reliability !=
                UNRELIABLE_SEQUENCED) {
          if (messageNumberNode == nullptr) {
            messageNumberNode = AddFirstToDatagramHistory(
                dhf.datagramNumber,
                packetsToSendThisUpdate[msgIndex]->reliableMessageNumber,
                time);
          } else {
        ...
        RakAssert(
            updateBitStream.GetNumberOfBytesUsed() <=
            MAXIMUM_MTU_SIZE - UDP_HEADER_SIZE);
        WriteToBitStreamFromInternalPacket(
            &updateBitStream, packetsToSendThisUpdate[msgIndex], time);
        RakAssert(
            updateBitStream.GetNumberOfBytesUsed() <=
            MAXIMUM_MTU_SIZE - UDP_HEADER_SIZE);
        msgIndex++;
      ...
      if (isSecondOfPacketPair) {
        ...
        RakAssert(
            updateBitStream.GetNumberOfBytesUsed() <=
            MAXIMUM_MTU_SIZE - UDP_HEADER_SIZE);
        updateBitStream.PadWithZeroToByteLength(
            datagramSizesInBytes[datagramIndex - 1]);
        RakAssert(
            updateBitStream.GetNumberOfBytesUsed() <=
            MAXIMUM_MTU_SIZE - UDP_HEADER_SIZE);
      }
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SendBitStream(
    RakNetSocket2* s,
    SystemAddress& systemAddress,
  ...
  if (minExtraPing > 0 || extraPingVariance > 0) {
    ...
    if (delay > 0) {
      auto* dat = RakNet::OP_NEW<DataAndTime>(__FILE__, __LINE__);
      memcpy(dat->data, (char*)bitStream->GetData(), length);
      dat->s = s;
      dat->length = length;
  ...
  if (useSecurity) {
    unsigned char* buffer =
        reinterpret_cast<unsigned char*>(bitStream->GetData());
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::ApplyNetworkSimulator(
    double _packetloss,
...
//-------------------------------------------------------------------------------------------------------
unsigned
ReliabilityLayer::RemovePacketFromResendListAndDeleteOlderReliableSequenced(
  ...
  for (unsigned int messageHandlerIndex = 0;
       ...
       messageHandlerIndex++) {
#if CC_TIME_TYPE_BYTES == 4
    messageHandlerList[messageHandlerIndex]->OnAck(
        messageNumber, systemAddress, time);
#else
    messageHandlerList[messageHandlerIndex]->OnAck(
        messageNumber,
        systemAddress,
  ...
  if (internalPacket &&
      internalPacket->reliableMessageNumber == messageNumber) {
    //	ValidateResendList();
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SendAcknowledgementPacket(
    const DatagramSequenceNumberType messageNumber,
    CCTimeType time) {
  // REMOVEME
...
//-------------------------------------------------------------------------------------------------------
BitSize_t ReliabilityLayer::GetMessageHeaderLengthBits(
  ...
  if (internalPacket->reliability == RELIABLE ||
      ...
      internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT) {
    bitLength += 8 *
        3; // bitStream->Write(internalPacket->reliableMessageNumber); // Message sequence number
  }
  ...
  if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
      ...
      internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT) {
    bitLength += 8 *
        3; // bitStream->Write(internalPacket->orderingIndex); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED.
    bitLength += 8 *
        1; // tempChar=internalPacket->orderingChannel; bitStream->WriteAlignedVar8((const char*)& tempChar); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED. 5 bits needed, write one byte
  }
  if (internalPacket->splitPacketCount > 0) {
    bitLength += 8 *
        4; // bitStream->WriteAlignedVar32((const char*)& internalPacket->splitPacketCount); RakAssert(sizeof(SplitPacketIndexType)==4); // Only needed if splitPacketCount>0. 4 bytes
    bitLength +=
        8 *
        sizeof(
            SplitPacketIdType); // bitStream->WriteAlignedVar16((const char*)& internalPacket->splitPacketId); RakAssert(sizeof(SplitPacketIdType)==2); // Only needed if splitPacketCount>0.
    bitLength += 8 *
        4; // bitStream->WriteAlignedVar32((const char*)& internalPacket->splitPacketIndex); // Only needed if splitPacketCount>0. 4 bytes
  }
...
//-------------------------------------------------------------------------------------------------------
BitSize_t ReliabilityLayer::WriteToBitStreamFromInternalPacket(
    RakNet::BitStream* bitStream,
    const InternalPacket* const internalPacket,
    CCTimeType curTime) {
  (void)curTime;
  ...
  bitStream->WriteBits(
      (const unsigned char*)&tempChar, 3, true); // 3 bits to write reliability.

  bool hasSplitPacket = internalPacket->splitPacketCount > 0;
  bitStream->Write(
      hasSplitPacket); // Write 1 bit to indicate if splitPacketCount>0
  bitStream->AlignWriteToByteBoundary();
  RakAssert(internalPacket->dataBitLength < 65535);
  unsigned short s;
  ...
  if (internalPacket->reliability == RELIABLE ||
      ...
      internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT) {
    bitStream->Write(
        internalPacket->reliableMessageNumber); // Used for all reliable types
  }
  bitStream->AlignWriteToByteBoundary(); // Potentially nothing else to write
  ...
  if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
    ...
    bitStream->Write(
        internalPacket
            ->sequencingIndex); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED.
  }
  ...
  if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
    ...
    bitStream->Write(
        internalPacket
            ->orderingIndex); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED.
    tempChar = internalPacket->orderingChannel;
    bitStream->WriteAlignedVar8((
        const char*)&tempChar); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED. 5 bits needed, write one byte
  }
  ...
  if (internalPacket->splitPacketCount > 0) {
    ...
    bitStream->WriteAlignedVar32(
        (const char*)&internalPacket->splitPacketCount);
    RakAssert(
        sizeof(SplitPacketIndexType) ==
        4); // Only needed if splitPacketCount>0. 4 bytes
    bitStream->WriteAlignedVar16((const char*)&internalPacket->splitPacketId);
    RakAssert(
        sizeof(SplitPacketIdType) == 2); // Only needed if splitPacketCount>0.
    bitStream->WriteAlignedVar32(
        (const char*)&internalPacket
            ->splitPacketIndex); // Only needed if splitPacketCount>0. 4 bytes
...
//-------------------------------------------------------------------------------------------------------
InternalPacket* ReliabilityLayer::CreateInternalPacketFromBitStream(
    RakNet::BitStream* bitStream,
    CCTimeType time) {
  ...
  if (bitStream->GetNumberOfUnreadBits() <
      (int)sizeof(internalPacket->reliableMessageNumber) * 8) {
  ...
  readSuccess = bitStream->Read(
      hasSplitPacket); // Read 1 bit to indicate if splitPacketCount>0
  ...
  if (internalPacket->reliability == RELIABLE ||
    ...
    bitStream->Read(
        internalPacket->reliableMessageNumber); // Message sequence number
  } else {
    internalPacket->reliableMessageNumber =
        (MessageNumberType)(const uint32_t)-1;
  }
  ...
  if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
    ...
    bitStream->Read(
        internalPacket
            ->orderingIndex); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED. 4 bytes.
    readSuccess = bitStream->ReadAlignedVar8(
        (char*)&internalPacket
            ->orderingChannel); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED. 5 bits needed, Read one byte
  } else {
  ...
  if (hasSplitPacket) {
    ...
    bitStream->ReadAlignedVar32(
        (char*)&internalPacket
            ->splitPacketCount); // Only needed if splitPacketCount>0. 4 bytes
    bitStream->ReadAlignedVar16(
        (char*)&internalPacket
            ->splitPacketId); // Only needed if splitPacketCount>0.
  ...
  if (internalPacket->data == nullptr) {
    RakAssert(
        "Out of memory in ReliabilityLayer::CreateInternalPacketFromBitStream" &&
        0);
    notifyOutOfMemory(_FILE_AND_LINE_);
    ReleaseToInternalPacketPool(internalPacket);
    return nullptr;
  ...
  bitStreamSucceeded = bitStream->ReadAlignedBytes(
      (unsigned char*)internalPacket->data,
      BITS_TO_BYTES(internalPacket->dataBitLength));
...
// Check the SHA1 code
//-------------------------------------------------------------------------------------------------------
bool ReliabilityLayer::CheckSHA1(
    char code[SHA1_LENGTH],
    unsigned char* const buffer,
    unsigned int nbytes) {
...
//-------------------------------------------------------------------------------------------------------
bool ReliabilityLayer::IsOlderOrderedPacket(
  ...
  if (waitingForPacketOrderingIndex > maxRange / (OrderingIndexType)2) {
    if (newPacketOrderingIndex >= waitingForPacketOrderingIndex -
                maxRange / (OrderingIndexType)2 + (OrderingIndexType)1 &&
...
void ReliabilityLayer::SplitPacket(InternalPacket* internalPacket) {
  ...
  auto headerLength =
      (unsigned int)BITS_TO_BYTES(GetMessageHeaderLengthBits(internalPacket));
  auto dataByteLength =
      (unsigned int)BITS_TO_BYTES(internalPacket->dataBitLength);
  int maximumSendBlockBytes, byteOffset, bytesToSend;
  SplitPacketIndexType splitPacketIndex;
  int i;
  InternalPacket** internalPacketArray;

  maximumSendBlockBytes = GetMaxDatagramSizeExcludingMessageHeaderBytes() -
      BITS_TO_BYTES(GetMaxMessageHeaderLengthBits());

  // Calculate how many packets we need to create
  internalPacket->splitPacketCount =
      ((dataByteLength - 1) / (maximumSendBlockBytes) + 1);
  ...
  if (sizeof(InternalPacket*) * internalPacket->splitPacketCount <
    ...
    internalPacketArray = (InternalPacket**)alloca(
        sizeof(InternalPacket*) * internalPacket->splitPacketCount);
    usedAlloca = true;
  } else {
#endif
    internalPacketArray = (InternalPacket**)rakMalloc_Ex(
        sizeof(InternalPacket*) * internalPacket->splitPacketCount,
        _FILE_AND_LINE_);
  }
  ...
  InternalPacketRefCountedData* refCounter = nullptr;
  ...
  do {
    ...
    AllocInternalPacketData(
        internalPacketArray[splitPacketIndex],
        &refCounter,
        internalPacket->data,
    ...
    if (bytesToSend != maximumSendBlockBytes) {
      internalPacketArray[splitPacketIndex]->dataBitLength =
          internalPacket->dataBitLength -
          splitPacketIndex * (maximumSendBlockBytes << 3);
    } else {
    ...
    internalPacketArray[splitPacketIndex]->splitPacketCount =
        internalPacket->splitPacketCount;
    RakAssert(
        internalPacketArray[splitPacketIndex]->dataBitLength <
        BYTES_TO_BITS(MAXIMUM_MTU_SIZE));
  ...
  for (i = 0; i < (int)internalPacket->splitPacketCount; i++) {
    ...
    RakAssert(
        internalPacketArray[i]->dataBitLength <
        BYTES_TO_BITS(MAXIMUM_MTU_SIZE));
    ...
    outgoingPacketBuffer.PushSeries(
        GetNextWeight(internalPacketArray[i]->priority),
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::InsertIntoSplitPacketList(
    InternalPacket* internalPacket,
    CCTimeType time) {
  bool objectExists;
  unsigned index;
  ...
  index = splitPacketChannelList.GetIndexFromKey(
      internalPacket->splitPacketId, &objectExists);
  if (!objectExists) {
    auto* newChannel = RakNet::OP_NEW<SplitPacketChannel>(__FILE__, __LINE__);
#if PREALLOCATE_LARGE_MESSAGES == 1
    index = splitPacketChannelList.Insert(
        internalPacket->splitPacketId, newChannel, true, __FILE__, __LINE__);
    newChannel->returnedPacket =
        CreateInternalPacketCopy(internalPacket, 0, 0, time);
    newChannel->gotFirstPacket = false;
    newChannel->splitPacketsArrived = 0;
    AllocInternalPacketData(
        newChannel->returnedPacket,
        BITS_TO_BYTES(
            internalPacket->dataBitLength * internalPacket->splitPacketCount),
        false,
        __FILE__,
        __LINE__);
    ...
    newChannel->splitPacketList.Preallocate(
        internalPacket->splitPacketCount, __FILE__, __LINE__);
  ...
  splitPacketChannelList[index]->returnedPacket->dataBitLength +=
      internalPacket->dataBitLength;

  bool dealloc;
  if (internalPacket->splitPacketIndex == 0) {
    splitPacketChannelList[index]->gotFirstPacket = true;
    splitPacketChannelList[index]->stride =
        BITS_TO_BYTES(internalPacket->dataBitLength);

    for (unsigned int j = 0;
         j < splitPacketChannelList[index]->splitPacketList.Size();
         j++) {
      memcpy(
          splitPacketChannelList[index]->returnedPacket->data +
              internalPacket->splitPacketIndex *
                  splitPacketChannelList[index]->stride,
          internalPacket->data,
          (size_t)BITS_TO_BYTES(internalPacket->dataBitLength));
      FreeInternalPacketData(
          splitPacketChannelList[index]->splitPacketList[j],
          __FILE__,
          __LINE__);
      ReleaseToInternalPacketPool(
          splitPacketChannelList[index]->splitPacketList[j]);
    ...
    memcpy(
        ...
        internalPacket->data,
        (size_t)BITS_TO_BYTES(internalPacket->dataBitLength));
    splitPacketChannelList[index]->splitPacketList.Clear(
    ...
    if (splitPacketChannelList[index]->gotFirstPacket == true) {
      ...
      splitPacketChannelList[index]->splitPacketList.Push(
          internalPacket, __FILE__, __LINE__);
      dealloc = false;
    }
  }
  ...
  if (splitPacketChannelList[index]->gotFirstPacket == true &&
    ...
    const unsigned int len =
        sizeof(MessageID) + sizeof(unsigned int) * 2 + sizeof(unsigned int) + l;
    ...
    memcpy(
        progressIndicator->data + sizeof(MessageID) + sizeof(unsigned int) * 3,
        splitPacketChannelList[index]->returnedPacket->data,
        (size_t)BITS_TO_BYTES(l));
  }
  ...
  splitPacketChannelList[index]->splitPacketList.Insert(
      internalPacket, __FILE__, __LINE__);
  splitPacketChannelList[index]->lastUpdateTime = time;
  ...
  if (splitMessageProgressInterval &&
    ...
    InternalPacket* progressIndicator = AllocateFromInternalPacketPool();
    unsigned int length =
        sizeof(MessageID) + sizeof(unsigned int) * 2 + sizeof(unsigned int) +
        (unsigned int)BITS_TO_BYTES(
            splitPacketChannelList[index]->firstPacket->dataBitLength);
    AllocInternalPacketData(
        progressIndicator, length, false, __FILE__, __LINE__);
    ...
    memcpy(
        ...
        &temp,
        sizeof(unsigned int));
    temp = (unsigned int)BITS_TO_BYTES(
        splitPacketChannelList[index]->firstPacket->dataBitLength);
    memcpy(
        progressIndicator->data + sizeof(MessageID) + sizeof(unsigned int) * 2,
        &temp,
...
// Optimized version
//-------------------------------------------------------------------------------------------------------
InternalPacket* ReliabilityLayer::BuildPacketFromSplitPacketList(
  ...
  // Reconstruct
  internalPacket = CreateInternalPacketCopy(
      splitPacketChannel->splitPacketList[0], 0, 0, time);
  internalPacket->dataBitLength = 0;
  for (j = 0; j < splitPacketChannel->splitPacketList.Size(); j++) {
    internalPacket->dataBitLength +=
        splitPacketChannel->splitPacketList[j]->dataBitLength;
  }
  // splitPacketPartLength=BITS_TO_BYTES(splitPacketChannel->firstPacket->dataBitLength);

  internalPacket->data = (unsigned char*)rakMalloc_Ex(
      (size_t)BITS_TO_BYTES(internalPacket->dataBitLength), _FILE_AND_LINE_);
  ...
  for (j = 0; j < splitPacketChannel->splitPacketList.Size(); j++) {
    ...
    memcpy(
        internalPacket->data + BITS_TO_BYTES(offset),
        splitPacket->data,
        (size_t)BITS_TO_BYTES(
            splitPacketChannel->splitPacketList[j]->dataBitLength));
  ...
  for (j = 0; j < splitPacketChannel->splitPacketList.Size(); j++) {
    FreeInternalPacketData(
        splitPacketChannel->splitPacketList[j], _FILE_AND_LINE_);
...
#if PREALLOCATE_LARGE_MESSAGES == 1
  if (splitPacketChannel->splitPacketsArrived ==
      splitPacketChannel->returnedPacket->splitPacketCount)
#else
  if (splitPacketChannel->splitPacketList.Size() ==
      splitPacketChannel->splitPacketList[0]->splitPacketCount)
#endif
  {
...
//-------------------------------------------------------------------------------------------------------
InternalPacket* ReliabilityLayer::CreateInternalPacketCopy(
    InternalPacket* original,
    int dataByteOffset,
    int dataByteLength,
    CCTimeType time) {
  ...
  if (dataByteLength > 0) {
    AllocInternalPacketData(
        copy, BITS_TO_BYTES(dataByteLength), false, _FILE_AND_LINE_);
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::InsertPacketIntoResendList(
    InternalPacket* internalPacket,
    CCTimeType time,
    bool firstResend,
...
RakNetStatistics* ReliabilityLayer::GetStatistics(RakNetStatistics* rns) {
  ...
  if (rns->valueOverLastSecond[USER_MESSAGE_BYTES_SENT] +
  ...
  if (uint64Denominator != 0 &&
      rns->runningTotal[USER_MESSAGE_BYTES_SENT] / uint64Denominator > 0) {
    doubleDenominator =
        ((double)rns->runningTotal[USER_MESSAGE_BYTES_SENT] +
         (double)rns->runningTotal[USER_MESSAGE_BYTES_RESENT]);
    if (doubleDenominator != 0) {
      rns->packetlossTotal =
          (float)((double)rns->runningTotal[USER_MESSAGE_BYTES_RESENT] /
                  doubleDenominator);
    }
  }
...
bool ReliabilityLayer::AckTimeout(RakNet::Time curTime) {
  ...
  return (timeLastDatagramArrived - curTime) > 10000 &&
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::PushPacket(
    CCTimeType time,
    InternalPacket* internalPacket,
    bool isReliable) {
  BitSize_t bitsForThisPacket = BYTES_TO_BITS(
      BITS_TO_BYTES(internalPacket->dataBitLength) +
      BITS_TO_BYTES(internalPacket->headerLength));
  datagramSizeSoFar += bitsForThisPacket;
  RakAssert(
      BITS_TO_BYTES(datagramSizeSoFar) < MAXIMUM_MTU_SIZE - UDP_HEADER_SIZE);
  allDatagramSizesSoFar += bitsForThisPacket;
  packetsToSendThisUpdate.Push(internalPacket, _FILE_AND_LINE_);
  packetsToDeallocThisUpdate.Push(!isReliable, _FILE_AND_LINE_);
  RakAssert(
      internalPacket->headerLength ==
      GetMessageHeaderLengthBits(internalPacket));
...
void ReliabilityLayer::PushDatagram() {
  if (datagramSizeSoFar > 0) {
    ...
    RakAssert(
        BITS_TO_BYTES(datagramSizeSoFar) < MAXIMUM_MTU_SIZE - UDP_HEADER_SIZE);
    datagramSizesInBytes.Push(
        BITS_TO_BYTES(datagramSizeSoFar), _FILE_AND_LINE_);
    datagramSizeSoFar = 0;
...
bool ReliabilityLayer::TagMostRecentPushAsSecondOfPacketPair() {
  if (datagramsToSendThisUpdateIsPair.Size() >= 2) {
    ...
    datagramsToSendThisUpdateIsPair
        [datagramsToSendThisUpdateIsPair.Size() - 1] = true;
    return true;
  }
  return false;
}
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::RemoveFromList(
    InternalPacket* internalPacket,
    bool modifyUnacknowledgedBytes) {
  ...
  if (modifyUnacknowledgedBytes) {
    RakAssert(
        ...
        BITS_TO_BYTES(
            internalPacket->headerLength + internalPacket->dataBitLength));
    unacknowledgedBytes -= BITS_TO_BYTES(
        internalPacket->headerLength + internalPacket->dataBitLength);
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::AddToListTail(
    InternalPacket* internalPacket,
    bool modifyUnacknowledgedBytes) {
  if (modifyUnacknowledgedBytes) {
    unacknowledgedBytes += BITS_TO_BYTES(
...
void ReliabilityLayer::PopListHead(bool modifyUnacknowledgedBytes) {
  RakAssert(resendLinkedListHead != nullptr);
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::RemoveFromUnreliableLinkedList(
    InternalPacket* internalPacket) {
  if (internalPacket->reliability == UNRELIABLE ||
      internalPacket->reliability == UNRELIABLE_SEQUENCED ||
      internalPacket->reliability == UNRELIABLE_WITH_ACK_RECEIPT
    ...
    internalPacket->unreliableNext->unreliablePrev =
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::AddToUnreliableLinkedList(
    InternalPacket* internalPacket) {
  if (internalPacket->reliability == UNRELIABLE ||
      internalPacket->reliability == UNRELIABLE_SEQUENCED ||
      internalPacket->reliability == UNRELIABLE_WITH_ACK_RECEIPT
...
ReliabilityLayer::GetMessageNumberNodeByDatagramIndex(
    DatagramSequenceNumberType index,
    CCTimeType* timeSent) {
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::RemoveFromDatagramHistory(
    DatagramSequenceNumberType index) {
  DatagramSequenceNumberType offsetIntoList = index - datagramHistoryPopCount;
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::AddFirstToDatagramHistory(
    DatagramSequenceNumberType datagramNumber,
    CCTimeType timeSent) {
  (void)datagramNumber;
  ...
  datagramHistory.Push(DatagramHistoryNode(nullptr, timeSent), _FILE_AND_LINE_);
  // printf("%p Pushed empty DatagramHistoryNode to datagram history at index %i\n", this, datagramHistory.Size()-1);
}

//-------------------------------------------------------------------------------------------------------
ReliabilityLayer::MessageNumberNode*
ReliabilityLayer::AddFirstToDatagramHistory(
    DatagramSequenceNumberType datagramNumber,
    DatagramSequenceNumberType messageNumber,
    CCTimeType timeSent) {
  (void)datagramNumber;
...
//-------------------------------------------------------------------------------------------------------
ReliabilityLayer::MessageNumberNode*
ReliabilityLayer::AddSubsequentToDatagramHistory(
    MessageNumberNode* messageNumberNode,
    DatagramSequenceNumberType messageNumber) {
  messageNumberNode->next =
      datagramHistoryMessagePool.Allocate(_FILE_AND_LINE_);
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::AllocInternalPacketData(
    InternalPacket* internalPacket,
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::FreeInternalPacketData(
    InternalPacket* internalPacket,
    const char* file,
    unsigned int line) {
  if (internalPacket == nullptr) {
