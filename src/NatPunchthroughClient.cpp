...
void NatPunchthroughDebugInterface_PacketLogger::OnClientMessage(
    const char* msg) {
  if (pl) {
...
void NatPunchthroughClient::FindRouterPortStride(
  ...
  if (natPunchthroughDebugInterface) {
    natPunchthroughDebugInterface->OnClientMessage(RakString(
        "Calculating port stride from %s", facilitator.ToString(true)));
  }

  RakNet::BitStream outgoingBs;
  outgoingBs.Write((MessageID)ID_NAT_REQUEST_BOUND_ADDRESSES);
  rakPeerInterface->Send(
      &outgoingBs, HIGH_PRIORITY, RELIABLE_ORDERED, 0, facilitator, false);
}
bool NatPunchthroughClient::OpenNAT(
...
*/
void NatPunchthroughClient::SetDebugInterface(
...
void NatPunchthroughClient::Update() {
  ...
  if (sp.nextActionTime && sp.nextActionTime < time) {
    ...
    if (sp.testMode == SendPing::TESTING_INTERNAL_IPS) {
      SendOutOfBand(
          sp.internalIds[sp.attemptCount], ID_NAT_ESTABLISH_UNIDIRECTIONAL);
      ...
      if (sp.attemptCount >= pc.MAXIMUM_NUMBER_OF_INTERNAL_IDS_TO_CHECK) {
        ...
        sp.nextActionTime =
            time + pc.TIME_BETWEEN_PUNCH_ATTEMPTS_INTERNAL - delta;
      }
    } else if (sp.testMode == SendPing::WAITING_FOR_INTERNAL_IPS_RESPONSE) {
      sp.testMode =
          SendPing::TESTING_EXTERNAL_IPS_FACILITATOR_PORT_TO_FACILITATOR_PORT;
      sp.attemptCount = 0;
      sp.sentTTL = false;
    }
    /*
      ...
      if (!sp.sentTTL) {
        ...
        if (natPunchthroughDebugInterface) {
          natPunchthroughDebugInterface->OnClientMessage(
              RakString("Send with TTL 2 to %s", sa.ToString(true)));
        }
        ...
        if (sp.attemptCount > pc.MAX_PREDICTIVE_PORT_RANGE) {
          ...
          sp.nextActionTime =
              time + pc.EXTERNAL_IP_WAIT_AFTER_ALL_ATTEMPTS - delta;
        ...
        sp.testMode ==
        SendPing::TESTING_EXTERNAL_IPS_1024_TO_FACILITATOR_PORT) {
      SystemAddress sa = sp.targetAddress;
      if (sp.targetGuid < rakPeerInterface->GetGuidFromSystemAddress(
      ...
      if (sp.attemptCount > pc.MAX_PREDICTIVE_PORT_RANGE) {
        ...
        sp.testMode ==
        SendPing::TESTING_EXTERNAL_IPS_FACILITATOR_PORT_TO_1024) {
      SystemAddress sa = sp.targetAddress;
      if (sp.targetGuid > rakPeerInterface->GetGuidFromSystemAddress(
                              UNASSIGNED_SYSTEM_ADDRESS)) {
        sa.SetPortHostOrder((unsigned short)(1024 + sp.attemptCount));
      ...
      if (sp.attemptCount > pc.MAX_PREDICTIVE_PORT_RANGE) {
        if (natPunchthroughDebugInterface) {
          ...
          natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
              "Likely bidirectional punchthrough failure to guid %s, system address %s.",
              guidString,
              ipAddressString));
        }
    ...
    if (sp.testMode == SendPing::PUNCHING_FIXED_PORT) {
      ...
      if (++sp.retryCount >= sp.punchingFixedPortAttempts) {
        if (natPunchthroughDebugInterface) {
          ...
          sp.targetGuid.ToString(guidString);
          natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
              "Likely unidirectional punchthrough failure to guid %s, system address %s.",
              guidString,
              ipAddressString));
        }
        ...
        if ((sp.retryCount % pc.UDP_SENDS_PER_PORT_EXTERNAL) == 0) {
          ...
          sp.nextActionTime =
              time + pc.TIME_BETWEEN_PUNCH_ATTEMPTS_EXTERNAL - delta;
        }
      }
    }
  }
...
void NatPunchthroughClient::OnPunchthroughFailure() {
  if (!pc.retryOnFailure) {
    if (natPunchthroughDebugInterface) {
      ...
      sp.targetGuid.ToString(guidString);
      natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
          "Failed punchthrough once. Returning failure to guid %s, system address %s to user.",
          guidString,
          ipAddressString));
    }
  ...
  for (i = 0; i < failedAttemptList.Size(); i++) {
    if (failedAttemptList[i].guid == sp.targetGuid) {
      if (natPunchthroughDebugInterface) {
        ...
        sp.targetGuid.ToString(guidString);
        natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
            "Failed punchthrough twice. Returning failure to guid %s, system address %s to user.",
            guidString,
            ipAddressString));
      }
  ...
  if (rakPeerInterface->GetConnectionState(sp.facilitator) != IS_CONNECTED) {
    if (natPunchthroughDebugInterface) {
      ...
      sp.targetGuid.ToString(guidString);
      natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
          "Not connected to facilitator, so cannot retry punchthrough after first failure. Returning failure onj guid %s, system address %s to user.",
          guidString,
          ipAddressString));
    }
  ...
  if (natPunchthroughDebugInterface) {
    ...
    sp.targetGuid.ToString(guidString);
    natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
        "First punchthrough failure on guid %s, system address %s. Reattempting.",
        guidString,
        ipAddressString));
  }
  ...
  switch (packet->data[0]) {
    ...
    case ID_NAT_RESPOND_BOUND_ADDRESSES: {
      ...
      if (boundAddressCount < 2) {
        if (natPunchthroughDebugInterface) {
          natPunchthroughDebugInterface->OnClientMessage(RakString(
              "INCAPABLE_PORT_STRIDE. My external ID is %s",
              rakPeerInterface->GetExternalID(packet->systemAddress)
                  .ToString()));
        }
      ...
      for (int i = 0;
           i < boundAddressCount && i < MAXIMUM_NUMBER_OF_INTERNAL_IDS;
           i++) {
        ...
        if (boundAddresses[i] != packet->systemAddress) {
          ...
          rakPeerInterface->SendOutOfBand(
              (const char*)boundAddresses[i].ToString(false),
              boundAddresses[i].GetPort(),
    ...
    case ID_OUT_OF_BAND_INTERNAL:
      if (packet->length >= 2 && packet->data[1] == ID_NAT_PONG) {
        ...
        if (natPunchthroughDebugInterface) {
          natPunchthroughDebugInterface->OnClientMessage(RakString(
              "HAS_PORT_STRIDE %i. First external port %i. Second external port %i.",
              portStride,
              externalPort,
              externalPort2));
        }
        ...
        if (packet->data[1] == ID_NAT_ESTABLISH_UNIDIRECTIONAL &&
          ...
          if (sp.testMode != SendPing::PUNCHING_FIXED_PORT) {
            ...
            sp.punchingFixedPortAttempts = pc.UDP_SENDS_PER_PORT_EXTERNAL *
                (pc.MAX_PREDICTIVE_PORT_RANGE + 1);

            if (natPunchthroughDebugInterface) {
              char guidString[128];
              sp.targetGuid.ToString(guidString);
              natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
                  "PUNCHING_FIXED_PORT: Received ID_NAT_ESTABLISH_UNIDIRECTIONAL from guid %s, system address %s.",
                  guidString,
                  ipAddressString));
            }
          } else {
            if (natPunchthroughDebugInterface) {
              ...
              sp.targetGuid.ToString(guidString);
              natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
                  "Received ID_NAT_ESTABLISH_UNIDIRECTIONAL from guid %s, system address %s.",
                  guidString,
                  ipAddressString));
            }
          }
          ...
          if (mostRecentExternalPort == 0) {
            ...
            if (natPunchthroughDebugInterface) {
              natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
                  "ID_NAT_ESTABLISH_BIDIRECTIONAL mostRecentExternalPort first time set to %i",
            ...
            if (sp.testMode != SendPing::TESTING_INTERNAL_IPS &&
              ...
              if (hasPortStride != HAS_PORT_STRIDE) {
                ...
                if (natPunchthroughDebugInterface) {
                  natPunchthroughDebugInterface->OnClientMessage(RakString(
                      "ID_NAT_ESTABLISH_BIDIRECTIONAL: Estimated port stride from incoming connection at %i. ourExternalPort=%i mostRecentExternalPort=%i",
                      portStride,
                      ourExternalPort,
                      mostRecentExternalPort));
                }
              ...
              if (natPunchthroughDebugInterface) {
                natPunchthroughDebugInterface->OnClientMessage(RakString(
                    "ID_NAT_ESTABLISH_BIDIRECTIONAL: New mostRecentExternalPort %i",
                    mostRecentExternalPort));
              }
            }
          }
          ...
          if (natPunchthroughDebugInterface) {
            ...
            if (removedFromFailureQueue) {
              natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
                  "Punchthrough to guid %s, system address %s succeeded on 2nd attempt.",
                  guidString,
                  ipAddressString));
            } else {
              natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
                  "Punchthrough to guid %s, system address %s succeeded on 1st attempt.",
                  guidString,
                  ipAddressString));
            }
          }
    ...
    case ID_NAT_ALREADY_IN_PROGRESS: {
      ...
      if (natPunchthroughDebugInterface) {
        ...
        natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
            "Punchthrough retry to guid %s failed due to ID_NAT_ALREADY_IN_PROGRESS. Returning failure.",
            guidString));
      }
    ...
    case ID_NAT_TARGET_UNRESPONSIVE: {
      ...
      for (i = 0; i < failedAttemptList.Size(); i++) {
        if (failedAttemptList[i].guid == targetGuid) {
          if (natPunchthroughDebugInterface) {
            ...
            targetGuid.ToString(guidString);
            natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
                "Punchthrough retry to guid %s failed due to %s.",
                guidString,
                reason));
          }
      ...
      if (natPunchthroughDebugInterface) {
        ...
        targetGuid.ToString(guidString);
        natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
            "Punchthrough attempt to guid %s failed due to %s.",
            guidString,
            reason));
      }
    ...
    case ID_TIMESTAMP:
      if (packet->data[sizeof(MessageID) + sizeof(RakNet::Time)] ==
...
const char* TestModeToString(NatPunchthroughClient::SendPing::TestMode tm) {
  switch (tm) {
    ...
    case NatPunchthroughClient::SendPing::WAITING_FOR_INTERNAL_IPS_RESPONSE:
      ...
      // 		break;
    case NatPunchthroughClient::SendPing::
        TESTING_EXTERNAL_IPS_FACILITATOR_PORT_TO_FACILITATOR_PORT:
...
void NatPunchthroughClient::SendOutOfBand(SystemAddress sa, MessageID oobId) {
  ...
  if (natPunchthroughDebugInterface) {
    ...
    RakNet::Time clockDifferential =
        rakPeerInterface->GetClockDifferential(sp.facilitator);
    RakNet::Time serverTime = RakNet::GetTime() + clockDifferential;

    if (oobId == ID_NAT_ESTABLISH_UNIDIRECTIONAL) {
      natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
          "%I64d: %s: OOB ID_NAT_ESTABLISH_UNIDIRECTIONAL to guid %s, system address %s.\n",
          serverTime,
          TestModeToString(sp.testMode),
          guidString,
          ipAddressString));
    } else {
      natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
          "%I64d: %s: OOB ID_NAT_ESTABLISH_BIDIRECTIONAL to guid %s, system address %s.\n",
          serverTime,
          TestModeToString(sp.testMode),
          guidString,
          ipAddressString));
...
void NatPunchthroughClient::OnNewConnection(
  ...
  if (ourExternalId != UNASSIGNED_SYSTEM_ADDRESS &&
    ...
    if (natPunchthroughDebugInterface) {
      natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
          "OnNewConnection mostRecentExternalPort first time set to %i",
          mostRecentExternalPort));
    }
  }
...
void NatPunchthroughClient::OnClosedConnection(
    const SystemAddress& systemAddress,
    RakNetGUID rakNetGUID,
  ...
  if (sp.facilitator == systemAddress) {
    ...
    while (i < failedAttemptList.Size()) {
      if (sp.nextActionTime != 0 &&
          sp.targetGuid == failedAttemptList[i].guid) {
        i++;
        continue;
      }
...
void NatPunchthroughClient::GetUPNPPortMappings(
    char* externalPort,
    char* internalPort,
  ...
  if (mostRecentExternalPort == 0) {
    mostRecentExternalPort =
        rakPeerInterface->GetExternalID(natPunchthroughServerAddress).GetPort();
  }
...
void NatPunchthroughClient::OnGetMostRecentPort(Packet* packet) {
  ...
  if (mostRecentExternalPort == 0) {
    ...
    if (natPunchthroughDebugInterface) {
      natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
          "OnGetMostRecentPort mostRecentExternalPort first time set to %i",
          mostRecentExternalPort));
    }
  }
...
*/
void NatPunchthroughClient::QueueOpenNAT(
    RakNetGUID destination,
    const SystemAddress& facilitator) {
  DSTAndFac daf;
...
NatPunchthroughClient::GetPunchthroughConfiguration() {
  return &pc;
}
...
void NatPunchthroughClient::IncrementExternalAttemptCount(
    RakNet::Time time,
    RakNet::Time delta) {
