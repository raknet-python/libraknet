...
bool ReplicaManager3::PushConnection(
  ...
  if (index == (unsigned int)-1) {
    ...
    Connection_RM3::ConstructionMode constructionMode =
        newConnection->QueryConstructionMode();
    if (constructionMode == Connection_RM3::QUERY_REPLICA_FOR_CONSTRUCTION ||
        constructionMode ==
            Connection_RM3::QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION) {
...
void ReplicaManager3::DeallocReplicaNoBroadcastDestruction(
...
RakNet::Connection_RM3* ReplicaManager3::PopConnection(
    unsigned int index,
    WorldId worldId) {
  ...
  for (index2 = 0; index2 < replicaList.Size(); index2++) {
    ...
    if (action == RM3AOPC_DELETE_REPLICA) {
      ...
      for (unsigned int index3 = 0;
           index3 < connection->queryToSerializeReplicaList.Size();
           index3++) {
        LastSerializationResult* lsr =
            connection->queryToSerializeReplicaList[index3];
...
void ReplicaManager3::Reference(RakNet::Replica3* replica3, WorldId worldId) {
  ...
  if (index != (unsigned int)-1) {
    ...
    for (pushIdx = 0; pushIdx < world->connectionList.Size(); pushIdx++) {
      Connection_RM3::ConstructionMode constructionMode =
          world->connectionList[pushIdx]->QueryConstructionMode();
      if (constructionMode == Connection_RM3::QUERY_REPLICA_FOR_CONSTRUCTION ||
          constructionMode ==
              Connection_RM3::QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION) {
...
void ReplicaManager3::DereferenceList(
    DataStructures::List<Replica3*>& replicaListIn,
    WorldId worldId) {
  unsigned int index;
...
void ReplicaManager3::GetReplicasCreatedByMe(
...
PluginReceiveResult ReplicaManager3::OnReceive(Packet* packet) {
  ...
  switch (packetIdentifier) {
    ...
    case ID_REPLICA_MANAGER_SCOPE_CHANGE: {
      ...
      if (connection && !connection->isValidated) {
...
void Connection_RM3::AutoConstructByQuery(
    ReplicaManager3* replicaManager3,
    WorldId worldId) {
  ...
  if (constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
    ...
    while (index < queryToConstructReplicaList.Size()) {
      ...
      constructionState =
          lsr->replica->QueryConstruction(this, replicaManager3);
    ...
    if (constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION) {
      ...
      while (index < queryToDestructReplicaList.Size()) {
        ...
        destructionState =
            lsr->replica->QueryDestruction(this, replicaManager3);
    ...
    for (idx2 = 0; idx2 < destroyedReplicasCulled.Size(); idx2++) {
      exists = false;
      bool objectExists;
      idx1 = constructedReplicaList.GetIndexFromKey(
          destroyedReplicasCulled[idx2], &objectExists);
      if (objectExists) {
        constructedReplicaList.RemoveAtIndex(idx1);

        unsigned int j;
        for (j = 0; j < queryToSerializeReplicaList.Size(); j++) {
          if (queryToSerializeReplicaList[j]->replica ==
              destroyedReplicasCulled[idx2]) {
  ...
  SendConstruction(
      ...
      replicaManager3->rakPeerInterface,
      worldId,
      replicaManager3);
}
      ...
      for (index = 0; index < world->connectionList.Size(); index++) {
        ...
        if (connection->QuerySerializationList(replicasToSerialize)) {
          ...
          while (index2 < replicasToSerialize.Size()) {
            ...
            ssicr = connection->SendSerializeIfChanged(
...
PluginReceiveResult ReplicaManager3::OnConstruction(
    Packet* packet,
...
PluginReceiveResult ReplicaManager3::OnSerialize(
    Packet* packet,
    unsigned char* packetData,
    int packetDataLength,
    RakNetGUID senderGuid,
...
PluginReceiveResult ReplicaManager3::OnDownloadStarted(
  ...
  if (connection->QueryGroupDownloadMessages() &&
      ...
      !connection->groupConstructionAndSerialize) {
...
PluginReceiveResult ReplicaManager3::OnDownloadComplete(
    Packet* packet,
    unsigned char* packetData,
    int packetDataLength,
    RakNetGUID senderGuid,
...
Replica3* ReplicaManager3::GetReplicaByNetworkID(
    NetworkID networkId,
    WorldId worldId) {
  RM3World* world = worldsArray[worldId];
...
void ReplicaManager3::BroadcastDestructionList(
...
void ReplicaManager3::BroadcastDestruction(
    Replica3* replica,
    const SystemAddress& exclusionAddress) {
  DataStructures::List<Replica3*> replicaList;
...
Connection_RM3::Connection_RM3(
    const SystemAddress& _systemAddress,
    RakNetGUID _guid)
...
void Connection_RM3::GetConstructedReplicas(
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void Connection_RM3::SendSerializeHeader(
  ...
  for (channelIndex = 0; channelIndex < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS;
    ...
    if (channelIndex == 0) {
      ...
      for (int channelIndex2 = channelIndex;
           channelIndex2 < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS;
           channelIndex2++) {
      ...
      for (int channelIndex2 = 0; channelIndex2 < channelIndex;
           channelIndex2++) {
        bitsPerChannel[channelIndex2] = 0;
        out.Write(false);
      }
    ...
    channelHasData = indicesToSend[channelIndex] && bitsUsed > 0;
...
SendSerializeIfChangedResult Connection_RM3::SendSerializeIfChanged(
  ...
  if (replica->forceSendUntilNextUpdate) {
    ...
    return SendSerialize(
        replica,
        replica->lastSentSerialization.indicesToSend,
  ...
  if (serializationResult == RM3SR_SERIALIZED_ALWAYS) {
    ...
    for (int z = 0; z < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; z++) {
      ...
      lsr->AllocBS();
      lsr->lastSerializationResultBS->bitStream[z].Reset();
      lsr->lastSerializationResultBS->bitStream[z].Write(
          &sp->outputBitstream[z]);
      sp->outputBitstream[z].ResetReadPointer();
    }
    return SendSerialize(
        replica,
  ...
  if (serializationResult == RM3SR_SERIALIZED_ALWAYS_IDENTICALLY) {
    for (int z = 0; z < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; z++) {
      replica->lastSentSerialization.indicesToSend[z] =
          sp->outputBitstream[z].GetNumberOfBitsUsed() > 0;
      sp->bitsWrittenSoFar += sp->outputBitstream[z].GetNumberOfBitsUsed();
      replica->lastSentSerialization.bitStream[z].Reset();
      replica->lastSentSerialization.bitStream[z].Write(
          &sp->outputBitstream[z]);
      sp->outputBitstream[z].ResetReadPointer();
  ...
  if (serializationResult == RM3SR_BROADCAST_IDENTICALLY ||
    ...
    for (int z = 0; z < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; z++) {
      if (sp->outputBitstream[z].GetNumberOfBitsUsed() > 0 &&
          (sp->outputBitstream[z].GetNumberOfBitsUsed() !=
               lsr->lastSerializationResultBS->bitStream[z]
                   .GetNumberOfBitsUsed() ||
           memcmp(
               sp->outputBitstream[z].GetData(),
               lsr->lastSerializationResultBS->bitStream[z].GetData(),
               sp->outputBitstream[z].GetNumberOfBytesUsed()) != 0)) {
        indicesToSend[z] = true;
        sp->bitsWrittenSoFar += sp->outputBitstream[z].GetNumberOfBitsUsed();
        lsr->lastSerializationResultBS->bitStream[z].Reset();
...
void Connection_RM3::OnLocalReference(
  ...
  ConstructionMode constructionMode = QueryConstructionMode();
  RakAssert(
      constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
      constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION);
  RakAssert(replica3);
  (void)replicaManager;
  ...
  if (constructedReplicaList.HasData(replica3)) {
    RakAssert(
        "replica added to queryToConstructReplicaList when already in constructedReplicaList" &&
        0);
  }
#endif
...
void Connection_RM3::OnDownloadFromThisSystem(
  ...
  if (constructedReplicaList.Insert(lsr->replica, lsr, true, _FILE_AND_LINE_) !=
      (unsigned)-1) {
...
void Connection_RM3::OnDownloadFromOtherSystem(
    Replica3* replica3,
    ReplicaManager3* replicaManager) {
  ConstructionMode constructionMode = QueryConstructionMode();
  if (constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
...
void Connection_RM3::OnNeverConstruct(
  ...
  ValidateLists(replicaManager);
  LastSerializationResult* lsr =
      queryToConstructReplicaList[queryToConstructIdx];
...
void Connection_RM3::OnConstructToThisConnection(
    unsigned int queryToConstructIdx,
    ReplicaManager3* replicaManager) {
  ConstructionMode constructionMode = QueryConstructionMode();
  RakAssert(
...
void Connection_RM3::SendConstruction(
    DataStructures::List<Replica3*>& newObjects,
  ...
  if (isFirstConstruction) {
    ...
    rakPeer->Send(
        ...
        RELIABLE_ORDERED,
        sendParameters.orderingChannel,
        systemAddress,
        false,
  ...
  for (newListIndex = 0; newListIndex < newObjects.Size(); newListIndex++) {
    ...
    RM3ConstructionState cs =
        newObjects[newListIndex]->QueryConstruction(this, replicaManager3);
    ...
    if (actuallyCreateObject) {
      ...
      bsOut.Write(
          offsetStart2); // overwritten to point to after the call to WriteAllocationID
      bsOut
          .AlignWriteToByteBoundary(); // Give the user an aligned bitStream in case they use memcpy
  ...
  for (oldListIndex = 0; oldListIndex < deletedObjects.Size(); oldListIndex++) {
    ...
    offsetStart = bsOut.GetWriteOffset();
    bsOut.Write(offsetStart);
    deletedObjects[oldListIndex]->deletingSystemGUID =
...
RM3ConstructionState Replica3::QueryConstruction_ClientConstruction(
...
bool Replica3::QueryRemoteConstruction_ClientConstruction(
...
RM3ConstructionState Replica3::QueryConstruction_ServerConstruction(
    RakNet::Connection_RM3* destinationConnection,
    bool isThisTheServer) {
  (void)destinationConnection;
...
bool Replica3::QueryRemoteConstruction_ServerConstruction(
    RakNet::Connection_RM3* sourceConnection,
    bool isThisTheServer) {
  (void)sourceConnection;
  return !isThisTheServer;
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

RM3ConstructionState Replica3::QueryConstruction_PeerToPeer(
    RakNet::Connection_RM3* destinationConnection,
...
bool Replica3::QueryRemoteConstruction_PeerToPeer(
    RakNet::Connection_RM3* sourceConnection) {
  (void)sourceConnection;
...
RM3QuerySerializationResult Replica3::QuerySerialization_PeerToPeer(
    RakNet::Connection_RM3* destinationConnection,
    Replica3P2PMode p2pMode) {
  (void)destinationConnection;
...
RM3ActionOnPopConnection Replica3::QueryActionOnPopConnection_Client(
...
RM3ActionOnPopConnection Replica3::QueryActionOnPopConnection_Server(
    RakNet::Connection_RM3* droppedConnection) const {
  (void)droppedConnection;
