...
void TableSerializer::SerializeColumns(
  ...
  for (i = 0; i < columns.Size(); i++) {
    StringCompressor::Instance()->EncodeString(
        columns[i].columnName, _TABLE_MAX_COLUMN_NAME_LENGTH, out);
...
void TableSerializer::SerializeRow(
    ...
    unsigned keyIn,
    const DataStructures::List<DataStructures::Table::ColumnDescriptor>&
        columns,
    RakNet::BitStream* out) {
  unsigned cellIndex;
  out->Write(keyIn);
...
void TableSerializer::SerializeCell(
  ...
  if (!cell->isEmpty) {
    if (columnType == DataStructures::Table::NUMERIC) {
      ...
      out->WriteAlignedBytes(
          (const unsigned char*)cell->c, (const unsigned int)cell->i);
    }
  }
}
...
void TableSerializer::SerializeFilterQuery(
  ...
  StringCompressor::Instance()->EncodeString(
      query->columnName, _TABLE_MAX_COLUMN_NAME_LENGTH, in, 0);
  ...
  if (!query->cellValue->isEmpty) {
    ...
    in->WriteAlignedBytesSafe(
        ...
        (const unsigned int)query->cellValue->i,
...
bool TableSerializer::DeserializeFilterQuery(
  ...
  StringCompressor::Instance()->DecodeString(
      query->columnName, _TABLE_MAX_COLUMN_NAME_LENGTH, out, 0);
