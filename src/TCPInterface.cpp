...
#if !defined(WINDOWS_STORE_RT)
bool TCPInterface::CreateListenSocket(
    unsigned short port,
...
#if defined(WINDOWS_STORE_RT)
bool TCPInterface::CreateListenSocket_WinStore8(
    unsigned short port,
    unsigned short maxIncomingConnections,
    unsigned short socketFamily,
    const char* bindAddress) {
...
bool TCPInterface::Start(
...
#if defined(WINDOWS_STORE_RT)
    CreateListenSocket_WinStore8(
        port, maxIncomingConnections, socketFamily, bindAddress);
#else
  ...
  errorCode =
      RakNet::RakThread::Create(UpdateTCPInterfaceLoop, this, threadPriority);
...
SystemAddress TCPInterface::Connect(
  ...
  if (block) {
    ...
    __TCPSOCKET__ sockfd =
        SocketConnect(buffout, remotePort, socketFamily, bindAddress);
    // Windows RT TODO
#if !defined(WINDOWS_STORE_RT)
    if (sockfd == 0)
#endif
    ...
    completedConnectionAttempts.Push(
        remoteClients[newRemoteClientIndex].systemAddress, _FILE_AND_LINE_);
    ...
    errorCode =
        RakNet::RakThread::Create(ConnectionAttemptLoop, s, threadPriority);
  ...
  if (systemAddress.systemIndex < remoteClientsLength &&
    ...
    for (int i = 0; i < remoteClientsLength; i++) {
      ...
      if (remoteClients[i].isActive &&
          remoteClients[i].systemAddress == systemAddress) {
        remoteClients[systemAddress.systemIndex].SetActive(false);
  ...
  if (sysAddr != UNASSIGNED_SYSTEM_ADDRESS) {
    ...
    for (i = 0; i < messageHandlerList.Size(); i++) {
      ...
      messageHandlerList[i]->OnFailedConnectionAttempt(
          &p, FCAR_CONNECTION_ATTEMPT_FAILED);
    }
  }
...
unsigned int TCPInterface::GetOutgoingDataBufferSize(
  ...
  if (systemAddress.systemIndex < remoteClientsLength &&
    ...
    bytesWritten =
        remoteClients[systemAddress.systemIndex].outgoingData.GetBytesWritten();
...
RAK_THREAD_DECLARATION(RakNet::ConnectionAttemptLoop) {
  ...
  __TCPSOCKET__ sockfd = tcpInterface->SocketConnect(
      str1, systemAddress.GetPort(), socketFamily, s->bindAddress);
  if (sockfd == 0) {
  ...
  tcpInterface->remoteClients[newRemoteClientIndex].systemAddress =
      systemAddress;
...
RAK_THREAD_DECLARATION(RakNet::UpdateTCPInterfaceLoop) {
  ...
  while (sts->isStarted.GetValue() > 0) {
      ...
      if (sslSystemAddress->systemIndex >= 0 &&
        ...
        for (int i = 0; i < sts->remoteClientsLength; i++) {
          ...
          if (sts->remoteClients[i].isActive &&
              sts->remoteClients[i].systemAddress == *sslSystemAddress) {
    ...
    __TCPSOCKET__ largestDescriptor =
        0; // see select__()'s first parameter's documentation under linux
...
#pragma warning( \
    disable : 4244) // warning C4127: conditional expression is constant
      ...
      if (sts->listenSocket != 0 && FD_ISSET(sts->listenSocket, &readFD)) {
        newSock = accept__(
            sts->listenSocket, (sockaddr*)&sockAddr, (socklen_t*)&sockAddrSize);
        ...
        if (newSock != 0) {
          ...
          for (newRemoteClientIndex = 0;
            ...
            if (!sts->remoteClients[newRemoteClientIndex].isActive) {
              sts->remoteClients[newRemoteClientIndex].socket = newSock;

#if RAKNET_SUPPORT_IPV6 != 1
              sts->remoteClients[newRemoteClientIndex]
                  .systemAddress.address.addr4.sin_addr.s_addr =
                  sockAddr.sin_addr.s_addr;
              sts->remoteClients[newRemoteClientIndex]
                  .systemAddress.SetPortNetworkOrder(sockAddr.sin_port);
              sts->remoteClients[newRemoteClientIndex]
                  .systemAddress.systemIndex = newRemoteClientIndex;
#else
              ...
              SystemAddress* newConnectionSystemAddress =
                  sts->newIncomingConnections.Allocate(_FILE_AND_LINE_);
              *newConnectionSystemAddress =
                  sts->remoteClients[newRemoteClientIndex].systemAddress;
          ...
          sts->listenSocket != 0 && FD_ISSET(sts->listenSocket, &exceptionFD)) {
#ifdef _DO_PRINTF
        int err;
...

      {
        ...
        while (i < (unsigned int)sts->remoteClientsLength) {
          ...
          if (FD_ISSET(socketCopy, &exceptionFD)) {
            ...
            SystemAddress* lostConnectionSystemAddress =
                sts->lostConnections.Allocate(_FILE_AND_LINE_);
            *lostConnectionSystemAddress = sts->remoteClients[i].systemAddress;
            ...
            if (FD_ISSET(socketCopy, &readFD)) {
              ...
              if (len > 0) {
                incomingMessage =
                    sts->incomingMessages.Allocate(_FILE_AND_LINE_);
                incomingMessage->data =
                    (unsigned char*)rakMalloc_Ex(len + 1, _FILE_AND_LINE_);
                memcpy(incomingMessage->data, data, len);
                incomingMessage->data[len] =
                    0; // Null terminate this so we can print it out as regular strings.  This is different from RakNet which does not do this.
                // printf("RECV: %s\n",incomingMessage->data);
                /*
								if (1)
								{
                ...
                incomingMessage->systemAddress =
                    sts->remoteClients[i].systemAddress;
                sts->incomingMessages.Push(incomingMessage);
              } else {
                // Connection lost gracefully
                SystemAddress* lostConnectionSystemAddress =
                    sts->lostConnections.Allocate(_FILE_AND_LINE_);
                *lostConnectionSystemAddress =
                    sts->remoteClients[i].systemAddress;
                sts->lostConnections.Push(lostConnectionSystemAddress);
                sts->remoteClients[i].isActiveMutex.Lock();
                sts->remoteClients[i].SetActive(false);
            ...
            if (FD_ISSET(socketCopy, &writeFD)) {
              ...
              if (bytesInBuffer > 0) {
                ...
                char* contiguousBytesPointer =
                    rc->outgoingData.PeekContiguousBytes(&contiguousLength);
                if (contiguousLength < (unsigned int)BUFF_SIZE &&
                    contiguousLength < bytesInBuffer) {
                  ...
                  bytesSent =
                      rc->Send(contiguousBytesPointer, contiguousLength);
                }
