  ...
  if (packet->data[0] == ID_UDP_PROXY_GENERAL && packet->length > 1) {
    ...
    switch (packet->data[1]) {
      case ID_UDP_PROXY_FORWARDING_REQUEST_FROM_COORDINATOR_TO_SERVER:
        if (loggedInCoordinators.GetIndexFromKey(
                packet->systemAddress, &objectExists) != (unsigned int)-1) {
      ...
      case ID_UDP_PROXY_LOGIN_SUCCESS_FROM_COORDINATOR_TO_SERVER: {
        unsigned int removalIndex = loggingInCoordinators.GetIndexFromKey(
            packet->systemAddress, &objectExists);
        if (objectExists) {
...
void UDPProxyServer::OnForwardingRequestFromCoordinatorToServer(
  ...
  outgoingBs.Write((MessageID)ID_UDP_PROXY_GENERAL);
  outgoingBs.Write(
      (MessageID)ID_UDP_PROXY_FORWARDING_REPLY_FROM_SERVER_TO_COORDINATOR);
