...
static const unsigned int MAX_OFFLINE_DATA_LENGTH =
    400; // I set this because I limit ID_CONNECTION_REQUEST to 512 bytes, and the password is appended to that packet.
...
Packet*
RakPeer::AllocPacket(unsigned dataSize, const char* file, unsigned int line) {
...
RakPeer::RakPeer() {
  ...
  packetAllocationPoolMutex.Lock();
  packetAllocationPool.SetPageSize(
      sizeof(DataStructures::MemoryPool<Packet>::MemoryWithPage) * 32);
  packetAllocationPoolMutex.Unlock();

  remoteSystemIndexPool.SetPageSize(
      sizeof(DataStructures::MemoryPool<RemoteSystemIndex>::MemoryWithPage) *
      32);
...
StartupResult RakPeer::Startup(
  ...
  for (i = 0; i < socketDescriptorCount; i++) {
    ...
    if (r2->IsBerkleySocket()) {
      ...
      bbp.remotePortRakNetWasStartedOn_PS3_PS4_PSP2 =
          socketDescriptors[i].remotePortRakNetWasStartedOn_PS3_PSP2;
  ...
  for (i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++) {
    ...
    if (socketList[0]->IsBerkleySocket()) {
      unsigned short port =
          ((RNS2_Berkley*)socketList[0])->GetBoundAddress().GetPort();
  ...
  if (maximumNumberOfPeers == 0) {
    ...
    remoteSystemList = RakNet::OP_NEW_ARRAY<RemoteSystemStruct>(
    ...
    activeSystemList = RakNet::OP_NEW_ARRAY<RemoteSystemStruct*>(
        maximumNumberOfPeers, _FILE_AND_LINE_);
...
#ifdef _DEBUG
      remoteSystemList[i].reliabilityLayer.ApplyNetworkSimulator(
          _packetloss, _minExtraPing, _extraPingVariance);
#endif
    ...
    for (unsigned int i = 0; i < (unsigned int)maximumNumberOfPeers *
             REMOTE_SYSTEM_LOOKUP_HASH_MULTIPLE;
         i++) {
  ...
  if (endThreads) {
    ...
    if (!isMainLoopThreadActive) {
      ...
      errorCode =
          RakNet::RakThread::Create(UpdateNetworkLoop, this, threadPriority);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::InitializeSecurity(
  ...
  if (ip == nullptr) {
    ...
    while (i < securityExceptionList.Size()) {
      if (securityExceptionList[i].IPAddressMatch(ip)) {
        securityExceptionList[i] =
            securityExceptionList[securityExceptionList.Size() - 1];
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SetIncomingPassword(
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
ConnectionAttemptResult RakPeer::Connect(
    const char* host,
  ...
  if (host == nullptr || endThreads ||
      connectionSocketIndex >= socketList.Size()) {
  ...
  connectionSocketIndex =
      GetRakNetSocketFromUserConnectionSocketIndex(connectionSocketIndex);
...
ConnectionAttemptResult RakPeer::ConnectWithSocket(
    const char* host,
    unsigned short remotePort,
    const char* passwordData,
    int passwordDataLength,
    ...
    unsigned timeBetweenSendConnectionAttemptsMS,
    RakNet::TimeMS timeoutTime) {
  if (host == nullptr || endThreads || socket == nullptr) {
    return INVALID_PARAMETER;
  }
    ...
    for (i = 0; i < systemListSize; i++) {
      ...
      if (remoteSystemList[i].isActive) {
        NotifyAndFlagForShutdown(
            remoteSystemList[i].systemAddress,
            false,
            orderingChannel,
...
//	RakNet::TimeMS timeout;
  ...
  for (i = 0; i < systemListSize; i++) {
    ...
    remoteSystemList[i].reliabilityLayer.Reset(
        false, remoteSystemList[i].MTUSize, false);
    remoteSystemList[i].rakNetSocket = nullptr;
  }
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::GetConnectionList(
...
#ifdef _MSC_VER
#pragma warning( \
    disable : 4701) // warning C4701: local variable <variable name> may be used without having been initialized
#endif
  ...
  do {
    ...
    for (i = 0; i < pluginListTS.Size(); i++) {
      ...
      if (pluginResult == RR_STOP_PROCESSING_AND_DEALLOCATE) {
        ...
        packet = nullptr; // Will do the loop again and get another packet
...
void RakPeer::CloseConnection(
  ...
  if (!sendDisconnectionNotification &&
      GetConnectionState(target) == IS_CONNECTED) {
    Packet* packet = AllocPacket(sizeof(char), _FILE_AND_LINE_);
    packet->data[0] = ID_CONNECTION_LOST; // DeadConnection
    packet->guid = target.rakNetGuid == UNASSIGNED_RAKNET_GUID
        ? GetGuidFromSystemAddress(target.systemAddress)
        : target.rakNetGuid;
    ...
    packet->systemAddress.systemIndex =
        (SystemIndex)GetIndexFromSystemAddress(packet->systemAddress);
...
#if LIBCAT_SECURITY == 1
      CAT_AUDIT_PRINTF(
          "AUDIT: Deleting requestedConnectionQueue %i client_handshake %x\n",
          i,
          requestedConnectionQueue[i]->client_handshake);
      RakNet::OP_DELETE(
          requestedConnectionQueue[i]->client_handshake, _FILE_AND_LINE_);
#endif
...
int RakPeer::GetIndexFromSystemAddress(
    const SystemAddress systemAddress) const {
    ...
    if (remoteSystemList[index].isActive &&
        remoteSystemList[index].connectMode ==
            RakPeer::RemoteSystemStruct::
                CONNECTED) { // Don't give the user players that aren't fully connected, since sends will fail
  ...
  for (i = 0; i < activeSystemListSize; i++) {
    if ((activeSystemList[i])->isActive &&
        (activeSystemList[i])->connectMode ==
            RakPeer::RemoteSystemStruct::CONNECTED) {
...
void RakPeer::AddToBanList(const char* IP, RakNet::TimeMS milliseconds) {
  ...
  auto* banStruct = RakNet::OP_NEW<BanStruct>(_FILE_AND_LINE_);
...
bool RakPeer::Ping(
  ...
  bsp.systemAddress.FromStringExplicitPort(
      host,
      remotePort,
  ...
  unsigned i;
  for (i = 0; i < pluginListNTS.Size(); i++) {
    pluginListNTS[i]->OnDirectSocketSend(
        (const char*)bitStream.GetData(),
        bitStream.GetNumberOfBitsUsed(),
        bsp.systemAddress);
  }
...
int RakPeer::GetAveragePing(const AddressOrGUID systemIdentifier) {
  ...
  RemoteSystemStruct* remoteSystem =
      GetRemoteSystem(systemIdentifier, false, false);
...
int RakPeer::GetLastPing(const AddressOrGUID systemIdentifier) const {
  ...
  if (remoteSystem->pingAndClockDifferentialWriteIndex == 0) {
    return remoteSystem->pingAndClockDifferential[PING_TIMES_ARRAY_SIZE - 1]
        .pingTime;
  } else {
    return remoteSystem
        ->pingAndClockDifferential
            [remoteSystem->pingAndClockDifferentialWriteIndex - 1]
        .pingTime;
  }
}
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RakNet::Time RakPeer::GetClockDifferential(
    const AddressOrGUID systemIdentifier) {
  RemoteSystemStruct* remoteSystem =
      GetRemoteSystem(systemIdentifier, false, false);
  if (remoteSystem == nullptr) {
...
RakNet::Time RakPeer::GetClockDifferentialInt(
    RemoteSystemStruct* remoteSystem) const {
  ...
  for (counter = 0; counter < PING_TIMES_ARRAY_SIZE; counter++) {
    ...
    if (remoteSystem->pingAndClockDifferential[counter].pingTime <
        lowestPingSoFar) {
      clockDifferential =
          remoteSystem->pingAndClockDifferential[counter].clockDifferential;
      lowestPingSoFar =
          remoteSystem->pingAndClockDifferential[counter].pingTime;
    }
  }
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SetOfflinePingResponse(
    const char* data,
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
SystemAddress RakPeer::GetInternalID(
  ...
  if (systemAddress == UNASSIGNED_SYSTEM_ADDRESS) {
    ...
    RemoteSystemStruct* remoteSystem =
        GetRemoteSystemFromSystemAddress(systemAddress, false, true);
    if (remoteSystem == nullptr) {
      return UNASSIGNED_SYSTEM_ADDRESS;
    }
    ...
    if (remoteSystemList[i].systemAddress == target) {
      if (remoteSystemList[i].isActive) {
        return remoteSystemList[i].myExternalSystemAddress;
          ...
          remoteSystemList[i].myExternalSystemAddress !=
...
const RakNetGUID& RakPeer::GetGuidFromSystemAddress(
    const SystemAddress input) const {
  if (input == UNASSIGNED_SYSTEM_ADDRESS) {
    return myGuid;
  }

  if (input.systemIndex != (SystemIndex)-1 &&
      input.systemIndex < maximumNumberOfPeers &&
...
bool RakPeer::GetClientPublicKeyFromSystemAddress(
...
void RakPeer::SetTimeoutTime(
  ...
  if (target == UNASSIGNED_SYSTEM_ADDRESS) {
    ...
    RemoteSystemStruct* remoteSystem =
        GetRemoteSystemFromSystemAddress(target, false, true);
...
void RakPeer::SetSplitMessageProgressInterval(int interval) {
  ...
  for (unsigned short i = 0; i < maximumNumberOfPeers; i++) {
    remoteSystemList[i].reliabilityLayer.SetSplitMessageProgressInterval(
...
void RakPeer::SetUnreliableTimeout(RakNet::TimeMS timeoutMS) {
  ...
  for (unsigned short i = 0; i < maximumNumberOfPeers; i++) {
    remoteSystemList[i].reliabilityLayer.SetUnreliableTimeout(
        unreliableTimeout);
  }
}
...
void RakPeer::SendTTL(
  ...
  if (socketList[realIndex]->IsBerkleySocket()) {
    ...
    for (i = 0; i < pluginListNTS.Size(); i++) {
      pluginListNTS[i]->OnDirectSocketSend(
          (const char*)bsp.data, BYTES_TO_BITS(bsp.length), bsp.systemAddress);
    }
    socketList[realIndex]->Send(&bsp, _FILE_AND_LINE_);
  }
#endif
...
void RakPeer::DetachPlugin(PluginInterface2* plugin) {
  ...
  plugin->OnDetach();
  plugin->SetRakPeerInterface(nullptr);
}
...
void RakPeer::PushBackPacket(Packet* packet, bool pushAtHead) {
  ...
  unsigned i;
  for (i = 0; i < pluginListTS.Size(); i++) {
    pluginListTS[i]->OnPushBackPacket(
        (const char*)packet->data, packet->bitSize, packet->systemAddress);
  }
  for (i = 0; i < pluginListNTS.Size(); i++) {
    pluginListNTS[i]->OnPushBackPacket(
        (const char*)packet->data, packet->bitSize, packet->systemAddress);
  }
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::ChangeSystemAddress(
    RakNetGUID guid,
    const SystemAddress& systemAddress) {
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::ApplyNetworkSimulator(
    float packetloss,
  ...
  if (remoteSystemList) {
    ...
    for (i = 0; i < maximumNumberOfPeers; i++) {
      //for (i=0; i < remoteSystemListSize; i++)
      remoteSystemList[i].reliabilityLayer.ApplyNetworkSimulator(
          packetloss, minExtraPing, extraPingVariance);
    }
  }
...
void RakPeer::SetPerConnectionOutgoingBandwidthLimit(
...
bool RakPeer::SendOutOfBand(
  ...
  RakAssert(
      dataLength <=
      (MAX_OFFLINE_DATA_LENGTH + sizeof(unsigned char) + sizeof(RakNet::Time) +
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RakNetStatistics* RakPeer::GetStatistics(
    const SystemAddress systemAddress,
...
int RakPeer::GetIndexFromSystemAddress(
  ...
  if (systemAddress.systemIndex != (SystemIndex)-1 &&
      systemAddress.systemIndex < maximumNumberOfPeers &&
  ...
  if (calledFromNetworkThread) {
    ...
    for (i = 0; i < maximumNumberOfPeers; i++) {
      if (remoteSystemList[i].isActive &&
          remoteSystemList[i].systemAddress == systemAddress) {
        return i;
      }
    }
...
int RakPeer::GetIndexFromGuid(const RakNetGUID guid) {
  ...
  if (guid.systemIndex != (SystemIndex)-1 &&
      guid.systemIndex < maximumNumberOfPeers &&
      remoteSystemList[guid.systemIndex].guid == guid &&
      remoteSystemList[guid.systemIndex].isActive) {
  ...
  switch (publicKey->publicKeyMode) {
    ...
    case PKM_ACCEPT_ANY_PUBLIC_KEY:
      ...
      rcs->client_handshake =
          RakNet::OP_NEW<cat::ClientEasyHandshake>(_FILE_AND_LINE_);
    ...
    case PKM_USE_TWO_WAY_AUTHENTICATION:
      ...
      if (!rcs->client_handshake->Initialize(
              publicKey->remoteServerPublicKey) ||
          !rcs->client_handshake->SetIdentity(
              publicKey->myPublicKey, publicKey->myPrivateKey) ||
...
ConnectionAttemptResult RakPeer::SendConnectionRequest(
  ...
  RakAssert(remotePort != 0);
  SystemAddress systemAddress;
  if (!systemAddress.FromStringExplicitPort(
          host,
          remotePort,
  ...
  auto* rcs = RakNet::OP_NEW<RequestedConnectionStruct>(_FILE_AND_LINE_);
  ...
  rcs->timeBetweenSendConnectionAttemptsMS =
      timeBetweenSendConnectionAttemptsMS;
...
RakPeer::RemoteSystemStruct* RakPeer::GetRemoteSystemFromGUID(
  ...
  for (i = 0; i < maximumNumberOfPeers; i++) {
    if (remoteSystemList[i].guid == guid &&
        (!onlyActive || remoteSystemList[i].isActive)) {
      return remoteSystemList + i;
    }
  }
  return nullptr;
...
void RakPeer::ParseConnectionRequestPacket(
  ...
  if (_using_security) {
    ...
    if (doSecurity != 1 ||
        !remoteSystem->reliabilityLayer.GetAuthenticatedEncryption())
      return;

    // Validate client proof of key
    unsigned char proof[cat::EasyHandshake::PROOF_BYTES];
    bs.ReadAlignedBytes(proof, sizeof(proof));
    if (!remoteSystem->reliabilityLayer.GetAuthenticatedEncryption()
             ->ValidateProof(proof, sizeof(proof))) {
    ...
    if (doClientKey == 1) {
      ...
      if (_require_client_public_key) {
        ...
        if (!_server_handshake->VerifyInitiatorIdentity(
                remoteSystem->answer, ident, remoteSystem->client_public_key)) {
          RakNet::BitStream bitStream;
          bitStream.Write(
              (MessageID)
                  ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY); // Report an error since the client is not providing an identity when it is necessary to connect
          bitStream.Write(
              (unsigned char)2); // Indicate client identity is invalid
          SendImmediate(
              ...
              RakNet::GetTimeUS(),
              0);
          remoteSystem->connectMode =
              RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY;
          return;
        }
      }
      ...
      if (_require_client_public_key) {
        ...
        bitStream.Write(
            (MessageID)
                ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY); // Report an error since the client is not providing an identity when it is necessary to connect
        bitStream.Write(
            (unsigned char)1); // Indicate client identity is missing
        SendImmediate(
            (char*)bitStream.GetData(),
            bitStream.GetNumberOfBytesUsed(),
            IMMEDIATE_PRIORITY,
...
void RakPeer::OnConnectionRequest(
  ...
  auto systemIndex =
      (SystemIndex)GetIndexFromSystemAddress(remoteSystem->systemAddress, true);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::NotifyAndFlagForShutdown(
    const SystemAddress systemAddress,
  ...
  if (performImmediate) {
    SendImmediate(
        ...
        0);
    RemoteSystemStruct* rss =
        GetRemoteSystemFromSystemAddress(systemAddress, true, true);
    rss->connectMode = RemoteSystemStruct::DISCONNECT_ASAP;
  } else {
    SendBuffered(
        (const char*)temp.GetData(),
        temp.GetNumberOfBitsUsed(),
        disconnectionNotificationPriority,
        RELIABLE_ORDERED,
        orderingChannel,
        systemAddress,
        false,
        RemoteSystemStruct::DISCONNECT_ASAP,
        0);
  }
}
...
RakPeer::RemoteSystemStruct* RakPeer::AssignSystemAddressToRemoteSystemList(
  ...
  *thisIPConnectedRecently = false;
  for (assignedIndex = 0; assignedIndex < maximumNumberOfPeers;
       assignedIndex++) {
    if (!remoteSystemList[assignedIndex].isActive) {
      ...
      remoteSystem->isActive =
          true; // This one line causes future incoming packets to go through the reliability layer
      // Reserve this reliability layer for ourselves.
      ...
      remoteSystem->reliabilityLayer.Reset(
          true, remoteSystem->MTUSize, useSecurity);
      remoteSystem->reliabilityLayer.SetSplitMessageProgressInterval(
          splitMessageProgressInterval);
      ...
      if (incomingRakNetSocket->GetBoundAddress() == bindingAddress) {
        ...
        for (ipListIndex = 0; ipListIndex < MAXIMUM_NUMBER_OF_INTERNAL_IDS;
             ipListIndex++) {
        ...
        if (true || foundIndex == (unsigned int)-1) {
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::ShiftIncomingTimestamp(
    unsigned char* data,
    const SystemAddress& systemAddress) const {
#ifdef _DEBUG
  RakAssert(IsActive());
...
RakNet::Time RakPeer::GetBestClockDifferential(
    const SystemAddress systemAddress) const {
  RemoteSystemStruct* remoteSystem =
      GetRemoteSystemFromSystemAddress(systemAddress, true, true);
...
void RakPeer::ReferenceRemoteSystem(
  ...
  SystemAddress oldAddress =
      remoteSystemList[remoteSystemListIndex].systemAddress;
  ...
  if (remoteSystemLookup[hashIndex] == nullptr) {
    rsi->next = nullptr;
...
*/
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::IsLoopbackAddress(
    const AddressOrGUID& systemIdentifier,
  ...
  for (int i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS &&
       ipList[i] != UNASSIGNED_SYSTEM_ADDRESS;
       i++) {
    if (matchPort) {
    ...
    if (performImmediate) {
      ...
      if (index != (unsigned int)-1) {
        if (remoteSystemList[index].isActive) {
          ...
          remoteSystemList[index].reliabilityLayer.Reset(
              false, remoteSystemList[index].MTUSize, false);

          // Not using this socket
          remoteSystemList[index].rakNetSocket = nullptr;
        }
      }
    } else {
      BufferedCommandStruct* bcs;
...
void RakPeer::SendBuffered(
  ...
  bcs = bufferedCommands.Allocate(_FILE_AND_LINE_);
  bcs->data = (char*)rakMalloc_Ex(
      (size_t)BITS_TO_BYTES(numberOfBitsToSend),
      _FILE_AND_LINE_); // Making a copy doesn't lose efficiency because I tell the reliability layer to use this allocation for its own copy
  if (bcs->data == nullptr) {
...
void RakPeer::SendBufferedList(
  ...
  char* dataAggregate;
  dataAggregate = (char*)rakMalloc_Ex(
      (size_t)totalLength,
      _FILE_AND_LINE_); // Making a copy doesn't lose efficiency because I tell the reliability layer to use this allocation for its own copy
...
bool RakPeer::SendImmediate(
  ...
  unsigned int remoteSystemIndex,
      sendListIndex; // Iterates into the list of remote systems
  ...
  if (systemIdentifier.systemAddress != UNASSIGNED_SYSTEM_ADDRESS) {
    remoteSystemIndex =
        GetIndexFromSystemAddress(systemIdentifier.systemAddress, true);
...
//			int debugIndex = GetRemoteSystemIndex(systemIdentifier.systemAddress);
    ...
    if (remoteSystemList[remoteSystemIndex].isActive &&
        ...
        remoteSystemList[remoteSystemIndex].connectMode !=
            RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY &&
        remoteSystemList[remoteSystemIndex].connectMode !=
            RemoteSystemStruct::DISCONNECT_ON_NO_ACK) {
...
#else
    sendList = (unsigned*)rakMalloc_Ex(
        sizeof(unsigned) * maximumNumberOfPeers, _FILE_AND_LINE_);
#endif
    ...
    for (idx = 0; idx < maximumNumberOfPeers; idx++) {
      ...
      if (remoteSystemList[idx].isActive &&
          remoteSystemList[idx].systemAddress != UNASSIGNED_SYSTEM_ADDRESS) {
  ...
  for (sendListIndex = 0; sendListIndex < sendListSize; sendListIndex++) {
    ...
    bool useData = useCallerDataAllocation && !callerDataAllocationUsed &&
    ...
    if (reliability == RELIABLE || reliability == RELIABLE_ORDERED ||
...
void RakPeer::OnConnectedPong(
  ...
  remoteSystem
      ->pingAndClockDifferential[remoteSystem
  ...
  remoteSystem
      ->pingAndClockDifferential[remoteSystem
                                     ->pingAndClockDifferentialWriteIndex]
      .clockDifferential = sendPongTime - (time / 2 + sendPingTime / 2);

  if (remoteSystem->lowestPing == (unsigned short)-1 ||
      remoteSystem->lowestPing > (int)ping) {
    remoteSystem->lowestPing = (unsigned short)ping;
  }

  if (++(remoteSystem->pingAndClockDifferentialWriteIndex) ==
...
uint64_t RakPeerInterface::Get64BitUniqueRandomNumber() {
  ...
  for (j = 0; j < 8; j++) {
    ...
    auto diff4Bits = (unsigned int)(diff & 15);
    diff4Bits <<= 32 - 4;
    diff4Bits >>= j * 4;
    ((char*)&g)[j] ^= diff4Bits;
  }
...
bool ProcessOfflineNetworkPacket(
  ...
  if (rakPeer->IsBanned(str1)) {
    for (i = 0; i < rakPeer->pluginListNTS.Size(); i++) {
      rakPeer->pluginListNTS[i]->OnDirectSocketReceive(
          data, length * 8, systemAddress);
    }
    ...
    for (i = 0; i < rakPeer->pluginListNTS.Size(); i++) {
      rakPeer->pluginListNTS[i]->OnDirectSocketSend(
          (char*)bs.GetData(), bs.GetNumberOfBitsUsed(), systemAddress);
    }
  ...
  if (length <= 2) {
    *isOfflineMessage = true;
      ...
      length >= sizeof(unsigned char) + sizeof(RakNet::Time) +
              sizeof(OFFLINE_MESSAGE_DATA_ID)) {
    *isOfflineMessage = memcmp(
      ...
      (unsigned char)data[0] == ID_UNCONNECTED_PONG &&
      (size_t)length >= sizeof(unsigned char) + sizeof(RakNet::TimeMS) +
              RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID)) {
    *isOfflineMessage = memcmp(
                            data + sizeof(unsigned char) +
                                sizeof(RakNet::Time) + RakNetGUID::size(),
                            OFFLINE_MESSAGE_DATA_ID,
                            sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
  } else if (
      (unsigned char)data[0] == ID_OUT_OF_BAND_INTERNAL &&
      (size_t)length >= sizeof(MessageID) + RakNetGUID::size() +
              sizeof(OFFLINE_MESSAGE_DATA_ID)) {
    *isOfflineMessage = memcmp(
                            data + sizeof(MessageID) + RakNetGUID::size(),
                            OFFLINE_MESSAGE_DATA_ID,
                            sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
  } else if (
    ...
    *isOfflineMessage = memcmp(
              ...
              (size_t)length ==
                  sizeof(MessageID) * 2 + RakNetGUID::size() +
                      sizeof(OFFLINE_MESSAGE_DATA_ID))) {
    *isOfflineMessage = memcmp(
                            data + sizeof(MessageID) * 2,
                            OFFLINE_MESSAGE_DATA_ID,
                            sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
  } else {
  ...
  if (*isOfflineMessage) {
    ...
    if (((unsigned char)data[0] == ID_UNCONNECTED_PING_OPEN_CONNECTIONS ||
          ...
          rakPeer->AllowIncomingConnections()) // Open connections with players
      {
        ...
        outBitStream.Write(
            (MessageID)
                ID_UNCONNECTED_PONG); // Should be named ID_UNCONNECTED_PONG eventually
        outBitStream.Write(sendPingTime);
        outBitStream.Write(rakPeer->myGuid);
        outBitStream.WriteAlignedBytes(
            (const unsigned char*)OFFLINE_MESSAGE_DATA_ID,
            sizeof(OFFLINE_MESSAGE_DATA_ID));
        ...
        for (i = 0; i < rakPeer->pluginListNTS.Size(); i++) {
          rakPeer->pluginListNTS[i]->OnDirectSocketSend(
              (const char*)outBitStream.GetData(),
        ...
        packet->systemAddress.systemIndex =
            (SystemIndex)rakPeer->GetIndexFromSystemAddress(
                systemAddress, true);
        packet->guid.systemIndex = packet->systemAddress.systemIndex;
    ...
    // UNCONNECTED MESSAGE Pong with no data.
        ...
        (unsigned char)data[0] == ID_UNCONNECTED_PONG &&
        (size_t)length >= sizeof(unsigned char) + sizeof(RakNet::Time) +
                RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID) &&
        (size_t)length < sizeof(unsigned char) + sizeof(RakNet::Time) +
                RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID) +
                MAX_OFFLINE_DATA_LENGTH) {
      packet = rakPeer->AllocPacket(
          (unsigned int)(length - sizeof(OFFLINE_MESSAGE_DATA_ID) -
                         RakNetGUID::size() - sizeof(RakNet::Time) +
      ...
      RakNet::BitStream bsOut(
          (unsigned char*)packet->data, packet->length, false);
      bsOut.ResetWritePointer();
      bsOut.Write((unsigned char)ID_UNCONNECTED_PONG);
      auto pingMS = (RakNet::TimeMS)ping;
      bsOut.Write(pingMS);
      bsOut.WriteAlignedBytes(
          (const unsigned char*)data + sizeof(unsigned char) +
              sizeof(RakNet::Time) + RakNetGUID::size() +
              sizeof(OFFLINE_MESSAGE_DATA_ID),
          length - sizeof(unsigned char) - sizeof(RakNet::Time) -
              RakNetGUID::size() - sizeof(OFFLINE_MESSAGE_DATA_ID));
      ...
      packet->systemAddress = systemAddress;
      packet->systemAddress.systemIndex =
          (SystemIndex)rakPeer->GetIndexFromSystemAddress(systemAddress, true);
      packet->guid.systemIndex = packet->systemAddress.systemIndex;
      rakPeer->AddPacketToProducer(packet);
    } else if (
        (unsigned char)data[0] == ID_OUT_OF_BAND_INTERNAL &&
        (size_t)length > sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) +
                RakNetGUID::size() &&
        (size_t)length < MAX_OFFLINE_DATA_LENGTH +
                sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) +
                RakNetGUID::size()) {
      auto dataLength =
          (unsigned int)(length - sizeof(OFFLINE_MESSAGE_DATA_ID) -
      ...
      if (data
              [sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) +
               RakNetGUID::size()] == ID_ADVERTISE_SYSTEM) {
        packet->length--;
        packet->bitSize = BYTES_TO_BITS(packet->length);
        packet->data[0] = ID_ADVERTISE_SYSTEM;
        memcpy(
            packet->data + 1,
            data + sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) * 2 +
                RakNetGUID::size(),
            dataLength - 1);
      } else {
        ...
        memcpy(
            packet->data + 1,
            data + sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) +
                RakNetGUID::size(),
            dataLength);
      }
      ...
      rakPeer->AddPacketToProducer(packet);
        ...
        (unsigned char)(data)[0] == (MessageID)ID_OPEN_CONNECTION_REPLY_1) {
      for (i = 0; i < rakPeer->pluginListNTS.Size(); i++) {
        rakPeer->pluginListNTS[i]->OnDirectSocketReceive(
            data, length * 8, systemAddress);
      }
          ...
          if (serverHasSecurity) {
            ...
            if (rcs->publicKeyMode == PKM_ACCEPT_ANY_PUBLIC_KEY) {
              ...
              if (!rcs->client_handshake->Initialize(public_key) ||
                  !rcs->client_handshake->GenerateChallenge(
                      rcs->handshakeChallenge)) {
                CAT_AUDIT_PRINTF(
            ...
            if (cat::SecureEqual(
              ...
              packet->data[0] =
                  ID_PUBLIC_KEY_MISMATCH; // Attempted a connection and couldn't
            ...
            if (rcs->client_handshake == 0) {
              ...
              bsOut.WriteAlignedBytes(
                  (const unsigned char*)rcs->handshakeChallenge,
    ...
    // Message does not contain a challenge
            ...
            if (rcs->client_handshake != 0) {
              ...
              packet = rakPeer->AllocPacket(sizeof(char), _FILE_AND_LINE_);
              packet->data[0] =
                  ID_OUR_SYSTEM_REQUIRES_SECURITY; // Attempted a connection and couldn't
              packet->bitSize = (sizeof(char) * 8);
              packet->systemAddress = rcs->systemAddress;
              packet->guid = serverGuid;
              rakPeer->AddPacketToProducer(packet);
      ...
      rakPeer->requestedConnectionQueueMutex.Unlock();
        ...
        (unsigned char)(data)[0] == (MessageID)ID_OPEN_CONNECTION_REPLY_2) {
      for (i = 0; i < rakPeer->pluginListNTS.Size(); i++) {
        rakPeer->pluginListNTS[i]->OnDirectSocketReceive(
            data, length * 8, systemAddress);
      }
        ...
        if (rcs->systemAddress == systemAddress) {
          ...
          if (doSecurity) {
            if (rcs->client_handshake == 0) {
              ...
              packet->data[0] =
                  ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY; // Attempted a connection and couldn't
          ...
          RakAssert(
              rcs->actionToTake == RakPeer::RequestedConnectionStruct::CONNECT);
          ...
          if (!thisIPConnectedRecently) {
            ...
            if (remoteSystem) {
              ...
              remoteSystem->connectMode =
                  RakPeer::RemoteSystemStruct::REQUESTED_CONNECTION;
              ...
              if (doSecurity) {
                ...
                remoteSystem->reliabilityLayer.GetAuthenticatedEncryption()
                    ->GenerateProof(proof, sizeof(proof));
              ...
              if (rcs->outgoingPasswordLength > 0) {
                temp.Write(
                    (char*)rcs->outgoingPassword, rcs->outgoingPasswordLength);
              }
              ...
              packet = rakPeer->AllocPacket(sizeof(char), _FILE_AND_LINE_);
              packet->data[0] =
                  ID_CONNECTION_ATTEMPT_FAILED; // Attempted a connection and couldn't
              packet->bitSize = (sizeof(char) * 8);
              packet->systemAddress = rcs->systemAddress;
              packet->guid = guid;
              rakPeer->AddPacketToProducer(packet);
          ...
          rakPeer->requestedConnectionQueueMutex.Lock();
          for (unsigned int k = 0; k < rakPeer->requestedConnectionQueue.Size();
               k++) {
            if (rakPeer->requestedConnectionQueue[k]->systemAddress ==
                systemAddress) {
      ...
      return true;
        ...
        (unsigned char)(data)[0] == (MessageID)ID_CONNECTION_ATTEMPT_FAILED ||
        (unsigned char)(data)[0] ==
            (MessageID)ID_NO_FREE_INCOMING_CONNECTIONS ||
      ...
      if (rakPeer->_using_security) {
        ...
        // Write cookie
        uint32_t cookie = rakPeer->_cookie_jar->Generate(
            &systemAddress.address, sizeof(systemAddress.address));
        CAT_AUDIT_PRINTF(
      ...
      if (rakPeer->_using_security) {
        ...
        requiresSecurityOfThisClient =
            rakPeer->IsInSecurityExceptionList(str1) == false;

        uint32_t cookie;
        bs.Read(cookie);
        CAT_AUDIT_PRINTF(
            "AUDIT: Got cookie %i from %i:%i\n", cookie, systemAddress);
        if (rakPeer->_cookie_jar->Verify(
                &systemAddress.address,
                sizeof(systemAddress.address),
                cookie) == false) {
          return true;
        }
        ...
        if (clientWroteChallenge) {
          bs.ReadAlignedBytes(
              (unsigned char*)remoteHandshakeChallenge,
              cat::EasyHandshake::CHALLENGE_BYTES);
#ifdef CAT_AUDIT
      ...
      if (IPAddrInUse & GUIDInUse) {
        if (rssFromSA == rssFromGuid &&
            rssFromSA->connectMode ==
                RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER) {
      ...
      if (outcome == 1) {
        ...
        for (i = 0; i < rakPeer->pluginListNTS.Size(); i++) {
          rakPeer->pluginListNTS[i]->OnDirectSocketSend(
              (const char*)bsAnswer.GetData(),
              bsAnswer.GetNumberOfBitsUsed(),
              systemAddress);
        }
      ...
      if (requiresSecurityOfThisClient) {
        ...
        bsAnswer.WriteAlignedBytes(
            (const unsigned char*)rssFromSA->answer, sizeof(rssFromSA->answer));
      }
#endif // LIBCAT_SECURITY
...
void ProcessNetworkPacket(
  ...
  bool isOfflineMessage;
  if (ProcessOfflineNetworkPacket(
          systemAddress,
          data,
          length,
          rakPeer,
...
bool RakPeer::RunUpdateCycle(BitStream& updateBitStream) {
...
#elif defined(_WIN32)
  if (socketList[0]->GetSocketType() == RNS2T_WINDOWS &&
      ((RNS2_Windows*)socketList[0])->GetSocketLayerOverride()) {
    int len;
    SystemAddress sender;
    char dataOut[MAXIMUM_MTU_SIZE];
    do {
      len = ((RNS2_Windows*)socketList[0])
                ->GetSocketLayerOverride()
                ->RakNetRecvFrom(dataOut, &sender, true);
      if (len > 0) {
        ProcessNetworkPacket(
            sender,
            dataOut,
  ...
  while ((bcs = bufferedCommands.PopInaccurate()) != nullptr) {
    if (bcs->command == BufferedCommandStruct::BCS_SEND) {
      ...
      callerDataAllocationUsed = SendImmediate(
          (char*)bcs->data,
          bcs->numberOfBitsToSend,
          bcs->priority,
          bcs->reliability,
          bcs->orderingChannel,
          bcs->systemIdentifier,
          bcs->broadcast,
          true,
          timeNS,
          bcs->receipt);
      ...
      // Reroute
      RakPeer::RemoteSystemStruct* rssFromGuid =
          GetRemoteSystem(bcs->systemIdentifier.rakNetGuid, true, true);
      if (rssFromGuid != nullptr) {
        unsigned int existingSystemIndex =
            GetRemoteSystemIndex(rssFromGuid->systemAddress);
        ReferenceRemoteSystem(
            bcs->systemIdentifier.systemAddress, existingSystemIndex);
      }
  ...
  if (!requestedConnectionQueue.IsEmpty()) {
    ...
    while (requestedConnectionQueueIndex < requestedConnectionQueue.Size()) {
      ...
      if (rcs->nextRequestTime < timeMS) {
        ...
        condition2 =
            (bool)((rcs->systemAddress == UNASSIGNED_SYSTEM_ADDRESS) == 1);
        ...
        if (condition1 || condition2) {
          ...
          int MTUSizeIndex = rcs->requestsMade /
              (rcs->sendConnectionAttemptCount / NUM_MTU_SIZES);
          if (MTUSizeIndex >= NUM_MTU_SIZES) {
            MTUSizeIndex = NUM_MTU_SIZES - 1;
          }
          rcs->requestsMade++;
          rcs->nextRequestTime =
              timeMS + rcs->timeBetweenSendConnectionAttemptsMS;
          ...
          // if (SocketLayer::SendTo( socketToUse, (const char*) bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), rcs->systemAddress, _FILE_AND_LINE_ )==-10040)
          {
            ...
            rcs->requestsMade =
                (unsigned char)((MTUSizeIndex + 1) *
                                (rcs->sendConnectionAttemptCount /
                                 NUM_MTU_SIZES));
            ...
            if (sendToEnd - sendToStart > 100) {
              // Drop to lowest MTU
              int lowestMtuIndex = rcs->sendConnectionAttemptCount /
                  NUM_MTU_SIZES * (NUM_MTU_SIZES - 1);
              if (lowestMtuIndex > rcs->requestsMade) {
                rcs->requestsMade = (unsigned char)lowestMtuIndex;
                rcs->nextRequestTime = timeMS;
              } else {
                rcs->requestsMade =
                    (unsigned char)(rcs->sendConnectionAttemptCount + 1);
              }
            }
          }
  ...
  // remoteSystemList in network thread
  for (activeSystemListIndex = 0; activeSystemListIndex < activeSystemListSize;
  ...
  //for ( remoteSystemIndex = 0; remoteSystemIndex < remoteSystemListSize; ++remoteSystemIndex )
  {
    ...
    if (timeMS > remoteSystem->lastReliableSend &&
        timeMS - remoteSystem->lastReliableSend >
            remoteSystem->reliabilityLayer.GetTimeoutTime() / 2 &&
    ...
    if (remoteSystem->reliabilityLayer.IsDeadConnection() ||
        ...
        (remoteSystem->connectMode ==
             RemoteSystemStruct::DISCONNECT_ON_NO_ACK &&
         (!remoteSystem->reliabilityLayer.AreAcksWaiting() ||
          remoteSystem->reliabilityLayer.AckTimeout(timeMS))) ||
        (((remoteSystem->connectMode ==
               RemoteSystemStruct::REQUESTED_CONNECTION ||
           remoteSystem->connectMode ==
               RemoteSystemStruct::HANDLING_CONNECTION_REQUEST ||
           remoteSystem->connectMode ==
               RemoteSystemStruct::UNVERIFIED_SENDER) &&
...
#ifdef _DO_PRINTF
      RAKNET_DEBUG_PRINTF(
          "Connection dropped for player %i:%i\n", systemAddress);
#endif
    ...
    if (remoteSystem->connectMode == RemoteSystemStruct::CONNECTED &&
        timeMS > remoteSystem->nextPingTime &&
      ...
      if (remoteSystem->connectMode == RemoteSystemStruct::UNVERIFIED_SENDER) {
        if ((unsigned char)(data)[0] == ID_CONNECTION_REQUEST) {
          ParseConnectionRequestPacket(
              remoteSystem, systemAddress, (const char*)data, byteSize);
        ...
        if ((unsigned char)(data)[0] == ID_CONNECTION_REQUEST) {
          ...
          rakFree_Ex(data, _FILE_AND_LINE_);
            ...
            (unsigned char)data[0] == ID_NEW_INCOMING_CONNECTION &&
          ...
          if (remoteSystem->connectMode ==
            ...
            RakNet::BitStream inBitStream(
                (unsigned char*)data, byteSize, false);
            ...
            if (firstExternalID == UNASSIGNED_SYSTEM_ADDRESS) {
              ...
              firstExternalID.debugPort =
                  ntohs(firstExternalID.address.addr4.sin_port);
            }
            ...
            (unsigned char)data[0] == ID_CONNECTED_PONG &&
          ...
          rakFree_Ex(data, _FILE_AND_LINE_);
            ...
            (unsigned char)data[0] == ID_CONNECTED_PING &&
          ...
          //	AddPacketToProducer(packet);
            ...
            (unsigned char)(data)[0] == ID_DETECT_LOST_CONNECTIONS &&
          ...
          if (byteSize > sizeof(MessageID) + sizeof(unsigned int) +
                  sizeof(unsigned short) + sizeof(SystemIndex) +
            ...
            allowConnection = remoteSystem->connectMode ==
                    RemoteSystemStruct::HANDLING_CONNECTION_REQUEST ||
                remoteSystem->connectMode ==
                    RemoteSystemStruct::REQUESTED_CONNECTION ||
                allowConnectionResponseIPMigration;

            alreadyConnected = remoteSystem->connectMode ==
                RemoteSystemStruct::HANDLING_CONNECTION_REQUEST;
            ...
            if (allowConnection) {
              ...
              for (unsigned int i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS;
                   i++) {
                inBitStream.Read(remoteSystem->theirInternalSystemAddress[i]);
              }
              ...
              packet->systemAddress = systemAddress;
              packet->systemAddress.systemIndex =
                  (SystemIndex)GetIndexFromSystemAddress(systemAddress, true);
              packet->guid = remoteSystem->guid;
              packet->guid.systemIndex = packet->systemAddress.systemIndex;
              AddPacketToProducer(packet);
...
void RakPeer::OnRNS2Recv(RNS2RecvStruct* recvStruct) {
  if (incomingDatagramEventHandler) {
    if (!incomingDatagramEventHandler(recvStruct)) {
      return;
    }
  }
...
RAK_THREAD_DECLARATION(RakNet::UpdateNetworkLoop) {
  auto* rakPeer = (RakPeer*)arguments;
...
void RakPeer::CallPluginCallbacks(
    DataStructures::List<PluginInterface2*>& pluginList,
    Packet* packet) {
  for (unsigned int i = 0; i < pluginList.Size(); i++) {
    switch (packet->data[0]) {
      case ID_DISCONNECTION_NOTIFICATION:
        pluginList[i]->OnClosedConnection(
      ...
      case ID_CONNECTION_LOST:
        pluginList[i]->OnClosedConnection(
            packet->systemAddress, packet->guid, LCR_CONNECTION_LOST);
        break;
      case ID_NEW_INCOMING_CONNECTION:
        pluginList[i]->OnNewConnection(
            packet->systemAddress, packet->guid, true);
        break;
      case ID_CONNECTION_REQUEST_ACCEPTED:
        pluginList[i]->OnNewConnection(
            packet->systemAddress, packet->guid, false);
        break;
      case ID_CONNECTION_ATTEMPT_FAILED:
        pluginList[i]->OnFailedConnectionAttempt(
            packet, FCAR_CONNECTION_ATTEMPT_FAILED);
        break;
      case ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY:
        pluginList[i]->OnFailedConnectionAttempt(
            packet, FCAR_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY);
        break;
      case ID_OUR_SYSTEM_REQUIRES_SECURITY:
        pluginList[i]->OnFailedConnectionAttempt(
            packet, FCAR_OUR_SYSTEM_REQUIRES_SECURITY);
        break;
      case ID_PUBLIC_KEY_MISMATCH:
        pluginList[i]->OnFailedConnectionAttempt(
            packet, FCAR_PUBLIC_KEY_MISMATCH);
        break;
      case ID_ALREADY_CONNECTED:
        pluginList[i]->OnFailedConnectionAttempt(
            packet, FCAR_ALREADY_CONNECTED);
        break;
      case ID_NO_FREE_INCOMING_CONNECTIONS:
        pluginList[i]->OnFailedConnectionAttempt(
            packet, FCAR_NO_FREE_INCOMING_CONNECTIONS);
        break;
      case ID_CONNECTION_BANNED:
        pluginList[i]->OnFailedConnectionAttempt(
            packet, FCAR_CONNECTION_BANNED);
        break;
      ...
      case ID_INCOMPATIBLE_PROTOCOL_VERSION:
        pluginList[i]->OnFailedConnectionAttempt(
            packet, FCAR_INCOMPATIBLE_PROTOCOL);
        break;
      case ID_IP_RECENTLY_CONNECTED:
        pluginList[i]->OnFailedConnectionAttempt(
            packet, FCAR_IP_RECENTLY_CONNECTED);
        break;
    }
  }
}
...
void RakPeer::FillIPList() {
  ...
  int startingIdx = 0;
  while (startingIdx < MAXIMUM_NUMBER_OF_INTERNAL_IDS - 1 &&
         ipList[startingIdx] != UNASSIGNED_SYSTEM_ADDRESS) {
    int lowestIdx = startingIdx;
    for (int curIdx = startingIdx + 1;
         curIdx < MAXIMUM_NUMBER_OF_INTERNAL_IDS - 1 &&
         ipList[curIdx] != UNASSIGNED_SYSTEM_ADDRESS;
         curIdx++) {
