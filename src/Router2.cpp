...
bool Router2::ConnectInternal(
  ...
  if (largestPing < 0) {
    ...
    if (debugInterface) {
      debugInterface->ShowFailure(FormatStringTS(
          buff,
          "Router2: ConnectInternal(%I64d) failed at %s:%i\n",
          endpointGuid.g,
          __FILE__,
          __LINE__));
}
  ...
  for (unsigned int i = 0; i < guids.Size(); i++) {
    ...
    if (guids[i] != endpointGuid) {
      ...
      uint32_t pack_id = rakPeerInterface->Send(
          &bsOut, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, crs.guid, false);

      if (debugInterface) {
        char buff[512];
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff,
            "Router2::ConnectInternal: at %s:%i, pack_id = %d",
            __FILE__,
            __LINE__,
            pack_id));
      }
      ...
      if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff,
            "Router2::ConnectInternal: at %s:%i [else ..].: %I64d==%I64d",
            __FILE__,
            __LINE__,
            guids[i].g,
            endpointGuid.g));
  ...
  if (debugInterface) {
    ...
    debugInterface->ShowDiagnostic(FormatStringTS(
        buff,
        "Broadcasting ID_ROUTER_2_QUERY_FORWARDING to %I64d at %s:%i\n",
        endpointGuid.g,
        __FILE__,
        __LINE__));
  }
...
PluginReceiveResult Router2::OnReceive(Packet* packet) {
  ...
  if (packet->data[0] == ID_ROUTER_2_INTERNAL) {
    switch (packet->data[1]) {
      ...
      case ID_ROUTER_2_REQUEST_FORWARDING: {
        if (debugInterface) {
          ...
          debugInterface->ShowDiagnostic(FormatStringTS(
              buff,
              "Got ID_ROUTER_2_REQUEST_FORWARDING on ip %s from %I64d, ",
              buff2,
              packet->guid.g));
        }
      ...
      case ID_ROUTER_2_INCREASE_TIMEOUT: {
        ...
        rakPeerInterface->SetTimeoutTime(
            rakPeerInterface->GetTimeoutTime(packet->systemAddress) + 10000,
            packet->systemAddress);
        return RR_STOP_PROCESSING_AND_DEALLOCATE;
      }
    }
  } else if (
      packet->data[0] == ID_OUT_OF_BAND_INTERNAL && packet->length >= 2) {
    switch (packet->data[1]) {
      case ID_ROUTER_2_REPLY_TO_SENDER_PORT: {
        RakNet::BitStream bsOut;
        bsOut.Write(packet->guid);
        SendOOBFromRakNetPort(
            ID_ROUTER_2_MINI_PUNCH_REPLY, &bsOut, packet->systemAddress);
    ...
    for (forwardingIndex = 0; forwardingIndex < forwardedConnectionList.Size();
         forwardingIndex++) {
      if (forwardedConnectionList[forwardingIndex].endpointGuid ==
              packet->guid &&
    ...
    if (forwardingIndex < forwardedConnectionList.Size()) {
      ...
      rakPeerInterface->Send(
          &bsOut, HIGH_PRIORITY, RELIABLE, 0, packet->guid, false);
...
void Router2::Update() {
  ...
  while (i < miniPunchesInProgress.Size()) {
    if (miniPunchesInProgress[i].timeout < curTime) {
      SendFailureOnCannotForward(
          miniPunchesInProgress[i].sourceGuid,
          miniPunchesInProgress[i].endpointGuid);
...
void Router2::OnClosedConnection(
  ...
  while (forwardedConnectionIndex < forwardedConnectionList.Size()) {
    if (forwardedConnectionList[forwardedConnectionIndex].endpointGuid ==
        rakNetGUID) {
      if (debugInterface) {
        char buff[512];
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff,
            "Closed connection to the %I64d, removing forwarding from list at %s:%i\n",
            rakNetGUID.g,
            __FILE__,
            __LINE__));
      }
      ...
      forwardedConnectionList.RemoveAtIndexFast(forwardedConnectionIndex);
        ...
        forwardedConnectionList[forwardedConnectionIndex].intermediaryGuid ==
            rakNetGUID &&
        forwardedConnectionList[forwardedConnectionIndex]
            .weInitiatedForwarding) {
      ...
      connectionRequestsMutex.Lock();
      unsigned int pos = GetConnectionRequestIndex(
          forwardedConnectionList[forwardedConnectionIndex].endpointGuid);
      ...
      ConnectInternal(
          forwardedConnectionList[forwardedConnectionIndex].endpointGuid, true);
      ...
      if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff,
            "Closed connection %I64d, restarting forwarding at %s:%i\n",
            rakNetGUID.g,
            __FILE__,
            __LINE__));
      }
    ...
    if (connectionRequestGuidIndex != (unsigned int)-1) {
      ...
      if (!UpdateForwarding(cr)) // If returns false, no connection request systems left
      {
        if (debugInterface) {
          ...
          debugInterface->ShowDiagnostic(FormatStringTS(
              buff,
              "Aborted connection to the %I64d, aborted forwarding at %s:%i\n",
              rakNetGUID.g,
              __FILE__,
              __LINE__));
        }
      ...
      } else // Else a system in the connection request list dropped. If cr->requestState==R2RS_REQUEST_STATE_QUERY_FORWARDING then we are still waiting for other systems to respond.
      {
        if (debugInterface) {
          ...
          debugInterface->ShowDiagnostic(FormatStringTS(
              buff,
              "Aborted connection attempt to %I64d, restarting forwarding to %I64d at %s:%i\n",
              rakNetGUID.g,
              cr->endpointGuid.g,
              __FILE__,
              __LINE__));
  ...
  while (i < miniPunchesInProgress.Size()) {
    if (miniPunchesInProgress[i].sourceGuid == rakNetGUID ||
        miniPunchesInProgress[i].endpointGuid == rakNetGUID) {
      if (miniPunchesInProgress[i].sourceGuid != rakNetGUID) {
        SendFailureOnCannotForward(
            miniPunchesInProgress[i].sourceGuid,
            miniPunchesInProgress[i].endpointGuid);
...
void Router2::OnFailedConnectionAttempt(
    Packet* packet,
  ...
  forwardedConnectionListMutex.Lock();
  while (forwardedConnectionIndex < forwardedConnectionList.Size()) {
    if (forwardedConnectionList[forwardedConnectionIndex].intermediaryAddress ==
        packet->systemAddress) {
      if (debugInterface) {
        char buff[512];
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff,
            "Failed connection attempt to forwarded system (%I64d : %s) at %s:%i\n",
...
bool Router2::UpdateForwarding(ConnnectRequest* connectionRequest) {
  ...
  if (connectionRequest->connectionRequestSystems.Size() == 0) {
    ...
    if (connectionRequest->returnConnectionLostOnFailure) {
      ReturnToUser(
          ...
          true); // This is a connection which was previously established. Rerouting is not possible.
    ...
    if (debugInterface) {
      ...
      debugInterface->ShowDiagnostic(FormatStringTS(
          buff,
          "Forwarding failed, no remaining systems at %s:%i\n",
          _FILE_AND_LINE_));
    }

    forwardedConnectionListMutex.Lock();

    for (unsigned int forwardedConnectionIndex = 0;
         forwardedConnectionIndex < forwardedConnectionList.Size();
         forwardedConnectionIndex++) {
      if (forwardedConnectionList[forwardedConnectionIndex].endpointGuid ==
              connectionRequest->endpointGuid &&
          forwardedConnectionList[forwardedConnectionIndex]
  ...
  if (connectionRequest->requestState == R2RS_REQUEST_STATE_QUERY_FORWARDING) {
    ...
    for (unsigned int i = 0;
         i < connectionRequest->connectionRequestSystems.Size();
         i++) {
...
void Router2::RemoveConnectionRequest(unsigned int connectionRequestIndex) {
  RakNet::OP_DELETE(
      connectionRequests[connectionRequestIndex], _FILE_AND_LINE_);
  connectionRequests.RemoveAtIndexFast(connectionRequestIndex);
}
int ConnectionRequestSystemComp(
    const Router2::ConnectionRequestSystem& key,
...
void Router2::RequestForwarding(ConnnectRequest* connectionRequest) {
  RakAssert(
      connectionRequest->requestState == R2RS_REQUEST_STATE_QUERY_FORWARDING);
  connectionRequest->requestState = REQUEST_STATE_REQUEST_FORWARDING;

  if (connectionRequest->GetGuidIndex(
          connectionRequest->lastRequestedForwardingSystem) !=
      (unsigned int)-1) {
    char buff[512];
    if (debugInterface) {
      debugInterface->ShowFailure(
  ...
  DataStructures::OrderedList<
      ...
      ConnectionRequestSystem,
      ConnectionRequestSystemComp>
      commandList;
  unsigned int connectionRequestGuidIndex;
  connectionRequest->connectionRequestSystemsMutex.Lock();
  for (connectionRequestGuidIndex = 0; connectionRequestGuidIndex <
       connectionRequest->connectionRequestSystems.Size();
       connectionRequestGuidIndex++) {
    RakAssert(
...
void Router2::SendFailureOnCannotForward(
  ...
  bsOut.Write(false);
  rakPeerInterface->Send(
      &bsOut, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, sourceGuid, false);
}
int Router2::ReturnFailureOnCannotForward(
    RakNetGUID sourceGuid,
    RakNetGUID endpointGuid) {
  ...
  if (udpForwarder == nullptr ||
    ...
    if (debugInterface) {
      debugInterface->ShowFailure(FormatStringTS(
          buff,
          "Router2 failed (%I64d -> %I64d) at %s:%i\n",
          sourceGuid.g,
          endpointGuid.g,
          __FILE__,
          __LINE__));
  ...
  for (unsigned int i = 0; i < forwardedConnectionList.Size(); i++) {
    if ((forwardedConnectionList[i].endpointGuid == endpointGuid) ||
        (forwardedConnectionList[i].endpointGuid ==
         sourceGuid)) // sourceGuid is here so you do not respond to routing requests from systems you are already routing through.
    {
      forwardedConnectionListMutex.Unlock();
...
void Router2::OnQueryForwarding(Packet* packet) {
  ...
  rakPeerInterface->Send(
      &bsOut, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, packet->guid, false);

  if (debugInterface) {
    char buff[512];
    debugInterface->ShowDiagnostic(FormatStringTS(
        buff,
        "Sending ID_ROUTER_2_REPLY_FORWARDING to the %I64d at %s:%i\n",
        packet->guid.g,
        __FILE__,
        __LINE__));
  }
...
void Router2::OnQueryForwardingReply(Packet* packet) {
  ...
  connectionRequests[connectionRequestIndex]
      ->connectionRequestSystemsMutex.Lock();
  unsigned int connectionRequestGuidIndex =
      connectionRequests[connectionRequestIndex]->GetGuidIndex(packet->guid);
  if (connectionRequestGuidIndex == (unsigned int)-1) {
    connectionRequests[connectionRequestIndex]
        ->connectionRequestSystemsMutex.Unlock();
    connectionRequestsMutex.Unlock();
    char buff[512];
    if (debugInterface) {
      debugInterface->ShowFailure(FormatStringTS(
  ...
  if (debugInterface) {
    ...
    debugInterface->ShowDiagnostic(FormatStringTS(
        buff,
        "Got ID_ROUTER_2_REPLY_FORWARDING on address %s(%I64d -> %I64d) canForward=%i at %s:%i\n",
        buff2,
        packet->guid.g,
        endpointGuid.g,
        canForward,
  ...
  if (canForward) {
    ...
    connectionRequests[connectionRequestIndex]
        ->connectionRequestSystems.RemoveAtIndex(connectionRequestGuidIndex);
  }
  connectionRequests[connectionRequestIndex]
      ->connectionRequestSystemsMutex.Unlock();

  if (!UpdateForwarding(connectionRequests[connectionRequestIndex])) {
    RemoveConnectionRequest(connectionRequestIndex);
  }
  connectionRequestsMutex.Unlock();
}
void Router2::SendForwardingSuccess(
    MessageID messageId,
    RakNetGUID sourceGuid,
    RakNetGUID endpointGuid,
  ...
  if (debugInterface) {
    ...
    if (messageId == ID_ROUTER_2_FORWARDING_ESTABLISHED) {
      debugInterface->ShowDiagnostic(FormatStringTS(
          buff,
          "Sending ID_ROUTER_2_FORWARDING_ESTABLISHED at %s:%i\n",
          _FILE_AND_LINE_));
    } else {
      debugInterface->ShowDiagnostic(FormatStringTS(
...
void Router2::SendOOBFromRakNetPort(
  ...
  rakPeerInterface->SendOutOfBand(
      (const char*)ipAddressString,
      sa.GetPort(),
      (const char*)oobBs.GetData(),
      oobBs.GetNumberOfBytesUsed());
}
void Router2::SendOOBFromSpecifiedSocket(
    OutOfBandIdentifiers oob,
    SystemAddress sa,
  ...
  if (sa.address.addr4.sin_family == AF_INET) {
    sendto__(
        ...
        0,
        (const sockaddr*)&sa.address.addr4,
        sizeof(sockaddr_in));
  } else {
    ...
    sendto__(
        ...
        bs.GetNumberOfBytesUsed(),
        0,
        (const sockaddr*)&sa.address.addr6,
        sizeof(sockaddr_in6));
#endif
  }
}
void Router2::SendOOBMessages(Router2::MiniPunchRequest* mpr) {
  ...
  SendOOBFromSpecifiedSocket(
      ID_ROUTER_2_REPLY_TO_SENDER_PORT,
      mpr->sourceAddress,
      mpr->forwardingSocket);
  ...
  if (debugInterface) {
    ...
    debugInterface->ShowDiagnostic(FormatStringTS(
        buff, "call SendOOBFromSpecifiedSocket(...,%s,...)", buff2));
  ...
  SendOOBFromRakNetPort(
      ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT, &extraData, mpr->sourceAddress);
  ...
  SendOOBFromRakNetPort(
      ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT, &extraData, mpr->endpointAddress);
}
void Router2::OnRequestForwarding(Packet* packet) {
  ...
  if (pingToEndpoint == -1) {
    ...
    if (debugInterface) {
      debugInterface->ShowFailure(FormatStringTS(
          buff,
          "Router2 failed (packet->guid =%I64d, endpointGuid = %I64d) at %s:%i\n",
          packet->guid.g,
          endpointGuid.g,
          __FILE__,
          __LINE__));
...
void Router2::OnMiniPunchReplyBounce(Packet* packet) {
  ...
  if (debugInterface) {
    ...
    debugInterface->ShowDiagnostic(FormatStringTS(
        buff,
        "Got ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE from guid=%I64d (miniPunchesInProgress.Size() = %d)",
        packet->guid.g,
        miniPunchesInProgress.Size()));
  }
  ...
  while (i < miniPunchesInProgress.Size()) {
    if (miniPunchesInProgress[i].sourceGuid == packet->guid ||
      ...
      if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff,
            "Processing ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE, gotReplyFromSource=%i gotReplyFromEndpoint=%i at %s:%i\n",
            miniPunchesInProgress[i].gotReplyFromSource,
            miniPunchesInProgress[i].gotReplyFromEndpoint,
            __FILE__,
            __LINE__));
      }

      if (miniPunchesInProgress[i].gotReplyFromEndpoint &&
          miniPunchesInProgress[i].gotReplyFromSource) {
        SendForwardingSuccess(
            ID_ROUTER_2_REROUTED,
            miniPunchesInProgress[i].endpointGuid,
            miniPunchesInProgress[i].sourceGuid,
            miniPunchesInProgress[i].forwardingPort);
        SendForwardingSuccess(
            ID_ROUTER_2_FORWARDING_ESTABLISHED,
            miniPunchesInProgress[i].sourceGuid,
            miniPunchesInProgress[i].endpointGuid,
...
void Router2::OnMiniPunchReply(Packet* packet) {
  ...
  if (debugInterface) {
    ...
    debugInterface->ShowDiagnostic(FormatStringTS(
        buff,
        "Sending ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE (%s) at %s:%i\n",
        buff2,
        __FILE__,
        __LINE__));
  }
  ...
  if (forwardingIndex < forwardedConnectionList.Size()) {
    ...
    if (debugInterface) {
      ...
      debugInterface->ShowDiagnostic(FormatStringTS(
          ...
          endpointGuid.g,
          intermediaryAddress.ToString(true),
          __FILE__,
          __LINE__));
    }
  } else {
    ...
    if (debugInterface) {
      ...
      debugInterface->ShowDiagnostic(FormatStringTS(
          buff,
          "Got ID_ROUTER_2_REROUTE, returning ID_ROUTER_2_REROUTED, Calling RakPeer::ChangeSystemAddress at %s:%i\n",
          __FILE__,
          __LINE__));
    }
  }
...
bool Router2::OnForwardingSuccess(Packet* packet) {
  ...
  if (forwardingIndex < forwardedConnectionList.Size()) {
    ...
    if (debugInterface) {
      ...
      debugInterface->ShowDiagnostic(FormatStringTS(
          buff,
          "Got ID_ROUTER_2_FORWARDING_ESTABLISHED, returning ID_ROUTER_2_REROUTED, Calling RakPeer::ChangeSystemAddress at %s:%i\n",
          _FILE_AND_LINE_));
    }
    ...
    unsigned int connectionRequestIndex =
        GetConnectionRequestIndex(endpointGuid);
    fc.returnConnectionLostOnFailure =
        connectionRequests[connectionRequestIndex]
    ...
    if (debugInterface) {
      ...
      debugInterface->ShowDiagnostic(FormatStringTS(
          buff,
          "Got and returning to user ID_ROUTER_2_FORWARDING_ESTABLISHED at %s:%i\n",
          _FILE_AND_LINE_));
    }
  }
  return true; // Return packet to user
...
int Router2::GetLargestPingAmongConnectedSystems() const {
  ...
  for (index = 0; index < rakPeerInterface->GetMaximumNumberOfPeers();
    ...
    if (g != UNASSIGNED_RAKNET_GUID) {
      avePing = rakPeerInterface->GetAveragePing(
