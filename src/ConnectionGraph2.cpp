...
int RakNet::ConnectionGraph2::SystemAddressAndGuidComp(
...
bool ConnectionGraph2::GetConnectionListForRemoteSystem(
  ...
  bool objectExists;
  unsigned int idx =
      remoteSystems.GetIndexFromKey(remoteSystemGuid, &objectExists);
...
uint16_t ConnectionGraph2::GetPingBetweenSystems(RakNetGUID g1, RakNetGUID g2)
  ...
  sag.guid = g2;
  unsigned int idx2 =
      remoteSystems[idx]->remoteConnections.GetIndexFromKey(sag, &objectExists);
  if (objectExists == false) {
    return (uint16_t)-1;
  }
...
RakNetGUID ConnectionGraph2::GetLowestAveragePingSystem() const {
  ...
  for (idx = 0; idx < remoteSystems.Size(); idx++) {
    ...
    if (count > 0 &&
        (lowestPing == -1.0f || thisAvePing / count < lowestPing)) {
      lowestPing = thisAvePing / count;
      lowestPingIdx = idx;
    }
  }
