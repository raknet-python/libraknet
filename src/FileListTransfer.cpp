...
FileListReceiver::FileListReceiver() {
  ...
  partLength = 1;
  DataStructures::Map<unsigned int, FLR_MemoryBlock>::
...
unsigned short FileListTransfer::SetupReceive(
    FileListTransferCBInterface* handler,
    bool deleteHandler,
    SystemAddress allowedSender) {
  if (rakPeerInterface &&
...
void FileListTransfer::Send(
  ...
  if (anythingToWrite) {
    ...
    for (i = 0; i < fileList->fileList.Size(); i++) {
      sendReference = fileList->fileList[i].isAReference &&
          _incrementalReadInterface != nullptr;
      if (sendReference) {
        ...
        fileToPush->fileListNode.fullPathToFile =
            fileList->fileList[i].fullPathToFile;
        fileToPush->fileListNode.fileLengthBytes =
            fileList->fileList[i].fileLengthBytes;
        fileToPush->fileListNode.dataLengthBytes =
            fileList->fileList[i].dataLengthBytes;
        ...
        StringCompressor::Instance()->EncodeString(
            fileList->fileList[i].filename, 512, &outBitstream);

        outBitstream.WriteCompressed(i);
        outBitstream.WriteCompressed(
            fileList->fileList[i].dataLengthBytes); // Original length in bytes
...
bool FileListTransfer::DecodeSetHeader(Packet* packet) {
  ...
  if (anythingToWrite) {
    ...
    if (inBitStream.ReadCompressed(fileListReceiver->setTotalFinalLength)) {
      fileListReceiver->setTotalCompressedTransmissionLength =
          fileListReceiver->setTotalFinalLength;
...
bool FileListTransfer::DecodeFile(Packet* packet, bool isTheFullFile) {
  ...
  if (!StringCompressor::Instance()->DecodeString(
          onFileStruct.fileName, 512, &inBitStream)) {
#ifdef _DEBUG
    RakAssert(0);
#endif
    return false;
  ...
  switch (packet->data[0]) {
    ...
    case ID_DOWNLOAD_PROGRESS:
      if (packet->length > sizeof(MessageID) + sizeof(unsigned int) * 3) {
        if (packet->data[sizeof(MessageID) + sizeof(unsigned int) * 3] ==
...
void FileListTransfer::OnReferencePush(Packet* packet, bool isTheFullFile) {
  ...
  if (!fileListReceiver->pushedFiles.Has(onFileStruct.fileIndex)) {
    mb.flrMemoryBlock =
        (char*)rakMalloc_Ex(onFileStruct.byteLengthOfThisFile, _FILE_AND_LINE_);
  ...
  if (isTheFullFile) {
    ...
    onFileStruct.bytesDownloadedForThisSet =
        fileListReceiver->setTotalDownloadedLength + partCount * partLength;
    fps.iriDataChunk = (char*)inBitStream.GetData() +
        BITS_TO_BYTES(inBitStream.GetReadOffset());
  }
  ...
  if (finished) {
    ...
    if (isTheFullFile) {
      ...
      totalNotifications =
          onFileStruct.byteLengthOfThisFile / fileListReceiver->partLength + 1;
      if (!isTheFullFile) {
        currentNotificationIndex =
            (offset + partCount * fileListReceiver->partLength) /
            fileListReceiver->partLength;
      } else {
        currentNotificationIndex =
...
int SendIRIToAddressCB(
    FileListTransfer::ThreadData threadData,
    bool* returnOutput,
    void* perThreadData) {
  (void)perThreadData;
  ...
  *returnOutput = false;
  ...
  for (ftpIndex = 0;
    ...
    if (ftpr->systemAddress == systemAddress && ftpr->setId == setId) {
      ...
      bytesRead = ftp->incrementalReadInterface->GetFilePart(
          ftp->fileListNode.fullPathToFile,
          ftp->currentOffset,
          ftp->chunkSize,
          buff,
      ...
      while (done && ftp->currentOffset == 0 &&
...
void FileListTransfer::SendIRIToAddress(
