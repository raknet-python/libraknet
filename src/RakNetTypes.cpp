...
void SystemAddress::ToString(bool writePort, char* dest, char portDelineator)
    const {
#if RAKNET_SUPPORT_IPV6 != 1
...
#ifdef _MSC_VER
#pragma warning( \
    disable : 4996) // The POSIX name for this item is deprecated. Instead, use the ISO C++ conformant name: _strnicmp. See online help for details.
#endif
...
bool SystemAddress::FromString(
  ...
  if (servinfo->ai_family == AF_INET) {
    ...
    memcpy(
        &address.addr6,
        (struct sockaddr_in6*)servinfo->ai_addr,
        sizeof(struct sockaddr_in6));
  }
#else
