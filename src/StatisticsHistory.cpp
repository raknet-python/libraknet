...
bool StatisticsHistory::AddValueByObjectID(
    uint64_t objectId,
    RakString key,
    SHValueType val,
    Time curTime,
...
void StatisticsHistory::AddValueByIndex(
    unsigned int index,
    RakString key,
    SHValueType val,
    Time curTime,
...
bool StatisticsHistory::GetHistorySorted(
    uint64_t objectId,
  ...
  for (unsigned int idx = 0; idx < objects.Size(); idx++) {
    ...
    if (!hi.IsInvalid()) {
      ...
      TimeAndValueQueue::MergeSets(
          tavqOutput, dataCategory, tavqInput, dataCategory, tavqOutput);
    }
  }
}

void StatisticsHistory::GetUniqueKeyList(
...
SHValueType StatisticsHistory::TimeAndValueQueue::GetRecentStandardDeviation()
  ...
  SHValueType meanOfSquares =
      GetRecentSumOfSquares() / (SHValueType)values.Size();
...
SHValueType StatisticsHistory::TimeAndValueQueue::GetLongTermLowest() const {
  return longTermLowest;
}

SHValueType StatisticsHistory::TimeAndValueQueue::GetLongTermHighest() const {
...
void StatisticsHistory::TimeAndValueQueue::MergeSets(
    const TimeAndValueQueue* lhs,
    SHDataCategory lhsDataCategory,
    const TimeAndValueQueue* rhs,
    SHDataCategory rhsDataCategory,
  ...
  if (lhsDataCategory == StatisticsHistory::DC_DISCRETE &&
      rhsDataCategory == StatisticsHistory::DC_DISCRETE) {
    ...
    while (rhsIndex < rhs->values.Size() && lhsIndex < lhs->values.Size()) {
      if (rhs->values[rhsIndex].time < lhs->values[lhsIndex].time) {
        timeSinceOppositeValue =
            rhs->values[rhsIndex].time - lastTimeAndValueLhs.time;
        newTimeAndValue.val = rhs->values[rhsIndex].val +
            lastTimeAndValueLhs.val + lastSlopeLhs * timeSinceOppositeValue;
        ...
        timeSinceOppositeValue =
            lhs->values[lhsIndex].time - lastTimeAndValueRhs.time;
        newTimeAndValue.val = lhs->values[lhsIndex].val +
            lastTimeAndValueRhs.val + lastSlopeRhs * timeSinceOppositeValue;
        ...
        newTimeAndValue.val =
            lhs->values[lhsIndex].val + rhs->values[rhsIndex].val;
        newTimeAndValue.time = lhs->values[lhsIndex].time;
        lastTimeAndValueRhs = rhs->values[rhsIndex];
        lastTimeAndValueLhs = lhs->values[lhsIndex];
        if (rhsIndex > 0 &&
  ...
  while (timeBoundary <= endTime) {
    ...
    if (dataCategory == DC_CONTINUOUS) {
      if (dataIndex > 0 && dataIndex < values.Size() &&
          ...
          values[dataIndex].time > timeBoundary) {
        SHValueType interpolatedValue =
            Interpolate(values[dataIndex - 1], values[dataIndex], timeBoundary);
    ...
    if (histogram.PeekTail().time < timeClipStart) {
      ...
      do {
        ...
        if (histogram.Peek().time == timeClipStart) {
          ...
          tav2.val = StatisticsHistory::TimeAndValueQueue::Interpolate(
              tav, histogram.Peek(), timeClipStart);
  ...
  if (timeClipEnd != 0 && histogram.Size() >= 1) {
    ...
    if (histogram.Peek().time > timeClipEnd) {
      histogram.Clear(_FILE_AND_LINE_);
        ...
        histogram.Size() >= 2 && histogram.PeekTail().time > timeClipEnd) {
      StatisticsHistory::TimeAndValue tav;
      ...
      do {
        ...
        if (histogram.PeekTail().time == timeClipEnd) {
          ...
          StatisticsHistory::TimeAndValue tav2;
          tav2.val = StatisticsHistory::TimeAndValueQueue::Interpolate(
              tav, histogram.PeekTail(), timeClipEnd);
...
SHValueType StatisticsHistory::TimeAndValueQueue::Interpolate(
  ...
  //	{
  SHValueType slope =
      (t2.val - t1.val) / ((SHValueType)t2.time - (SHValueType)t1.time);
...
StatisticsHistory::TimeAndValueQueue&
StatisticsHistory::TimeAndValueQueue::operator=(
    const TimeAndValueQueue& input) {
  values = input.values;
...
*/
void StatisticsHistoryPlugin::OnClosedConnection(
