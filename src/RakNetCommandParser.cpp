...
RakNetCommandParser::RakNetCommandParser() {
  ...
  RegisterCommand(
      1, "SetMaximumIncomingConnections", "( unsigned short numberAllowed );");
  ...
  RegisterCommand(
      1, "GetIndexFromSystemAddress", "( const SystemAddress systemAddress );");
  RegisterCommand(1, "GetSystemAddressFromIndex", "( unsigned int index );");
  RegisterCommand(
      2, "AddToBanList", "( const char *IP, RakNet::TimeMS milliseconds=0 );");
  ...
  RegisterCommand(
      1, "GetAveragePing", "( const SystemAddress systemAddress );");
  RegisterCommand(
      1, "GetLastPing", "( const SystemAddress systemAddress ) const;");
  RegisterCommand(
      1, "GetLowestPing", "( const SystemAddress systemAddress ) const;");
  ...
  RegisterCommand(
      2,
      "SetTimeoutTime",
      "( RakNet::TimeMS timeMS, const SystemAddress target );");
...
bool RakNetCommandParser::OnCommand(
  ...
  if (strcmp(command, "Startup") == 0) {
    RakNet::SocketDescriptor socketDescriptor(
        (unsigned short)atoi(parameterList[1]), parameterList[2]);
    ReturnResult(
        peer->Startup(
            (unsigned short)atoi(parameterList[0]), &socketDescriptor, 1),
        command,
        transport,
        systemAddress);
  } else if (strcmp(command, "InitializeSecurity") == 0) {
    ReturnResult(
        peer->InitializeSecurity(parameterList[0], parameterList[1]),
        command,
        transport,
        systemAddress);
    ...
    ReturnResult(
        peer->IsInSecurityExceptionList(parameterList[1]),
        command,
        transport,
        systemAddress);
    ...
    ReturnResult(
        (int)peer->GetMaximumIncomingConnections(),
        command,
        transport,
        systemAddress);
    ...
    ReturnResult(
        peer->Connect(
            parameterList[0],
            (unsigned short)atoi(parameterList[1]),
            parameterList[2],
    ...
    peer->Shutdown(
        atoi(parameterList[0]), (unsigned char)atoi(parameterList[1]));
    ReturnResult(command, transport, systemAddress);
    ...
    peer->CloseConnection(
        ...
        atoi(parameterList[1]) != 0,
        (unsigned char)atoi(parameterList[2]));
    ReturnResult(command, transport, systemAddress);
  } else if (strcmp(command, "GetConnectionState") == 0) {
    ReturnResult(
        (int)peer->GetConnectionState(SystemAddress(parameterList[0])),
        command,
        transport,
        systemAddress);
  } else if (strcmp(command, "GetIndexFromSystemAddress") == 0) {
    ReturnResult(
        peer->GetIndexFromSystemAddress(SystemAddress(parameterList[0])),
        command,
        transport,
        systemAddress);
  } else if (strcmp(command, "GetSystemAddressFromIndex") == 0) {
    ReturnResult(
        peer->GetSystemAddressFromIndex(atoi(parameterList[0])),
        command,
        transport,
        systemAddress);
    ...
    ReturnResult(
        peer->IsBanned(parameterList[0]), command, transport, systemAddress);
    ...
    peer->Ping(
        parameterList[0],
        (unsigned short)atoi(parameterList[1]),
        atoi(parameterList[2]) != 0);
    ReturnResult(command, transport, systemAddress);
  } else if (strcmp(command, "GetAveragePing") == 0) {
    ReturnResult(
        peer->GetAveragePing(SystemAddress(parameterList[0])),
        command,
        transport,
        systemAddress);
  } else if (strcmp(command, "GetLastPing") == 0) {
    ReturnResult(
        peer->GetLastPing(SystemAddress(parameterList[0])),
        command,
        transport,
        systemAddress);
  } else if (strcmp(command, "GetLowestPing") == 0) {
    ReturnResult(
        peer->GetLowestPing(SystemAddress(parameterList[0])),
        command,
        transport,
        systemAddress);
    ...
    ReturnResult(
        peer->GetExternalID(SystemAddress(parameterList[0])),
        command,
        transport,
        systemAddress);
  } else if (strcmp(command, "SetTimeoutTime") == 0) {
    peer->SetTimeoutTime(
        atoi(parameterList[0]), SystemAddress(parameterList[1]));
    ReturnResult(command, transport, systemAddress);
  }
  /*
	...
	*/
    ...
    ReturnResult(
        peer->GetMTUSize(UNASSIGNED_SYSTEM_ADDRESS),
        command,
        transport,
        systemAddress);
  } else if (strcmp(command, "GetNumberOfAddresses") == 0) {
    ReturnResult(
        (int)peer->GetNumberOfAddresses(), command, transport, systemAddress);
  } else if (strcmp(command, "GetLocalIP") == 0) {
    ReturnResult(
        (char*)peer->GetLocalIP(atoi(parameterList[0])),
        command,
        transport,
        systemAddress);
...
void RakNetCommandParser::SendHelp(
  ...
  if (peer) {
    ...
    transport->Send(
        systemAddress,
        "SystemAddresss take two parameters: send <BinaryAddress> <Port>.\r\n");
    transport->Send(systemAddress, "For bool, send 1 or 0.\r\n");
  } else {
    transport->Send(
        systemAddress, "Parser not active.  Call SetRakPeerInterface.\r\n");
  }
}
