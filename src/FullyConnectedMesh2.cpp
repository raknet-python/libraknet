...
bool FullyConnectedMesh2::IsHostSystem() const {
  return GetHostSystem() ==
      rakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
}
void FullyConnectedMesh2::GetHostOrder(
    DataStructures::List<RakNetGUID>& hostList) {
...
bool FullyConnectedMesh2::AddParticipantInternal(
  ...
  auto* participant = RakNet::OP_NEW<FCM2Participant>(_FILE_AND_LINE_);
...
void FullyConnectedMesh2::AddParticipant(RakNetGUID rakNetGuid) {
  if (rakPeerInterface->GetConnectionState(
          rakPeerInterface->GetSystemAddressFromGuid(rakNetGuid)) !=
      IS_CONNECTED) {
#ifdef DEBUG_FCM2
    printf(
        "AddParticipant to %s failed (not connected)\n", rakNetGuid.ToString());
#endif
    return;
  }
...
void FullyConnectedMesh2::GetParticipantList(
...
PluginReceiveResult FullyConnectedMesh2::OnReceive(Packet* packet) {
  switch (packet->data[0]) {
    ...
    case ID_NAT_CONNECTION_TO_TARGET_LOST: {
      ...
      b.Read(g);
      UpdateVerifiedJoinInProgressMember(
          g, UNASSIGNED_RAKNET_GUID, JIPS_FAILED);
      return RR_CONTINUE_PROCESSING;
    }
    case ID_NAT_PUNCHTHROUGH_FAILED:
      UpdateVerifiedJoinInProgressMember(
          packet->guid, UNASSIGNED_RAKNET_GUID, JIPS_FAILED);
      return RR_CONTINUE_PROCESSING;
  }
...
void FullyConnectedMesh2::OnClosedConnection(
  ...
  while (idx < joinsInProgress.Size()) {
    if (joinsInProgress[idx]->requester == rakNetGUID) {
      Packet* p =
          AllocatePacketUnified(sizeof(MessageID) + sizeof(unsigned char));
      ...
      for (unsigned int j = 0; j < joinsInProgress[idx]->vjipMembers.Size();
           j++) {
        if (joinsInProgress[idx]->vjipMembers[j].userData != nullptr) {
          RakNet::OP_DELETE(
              joinsInProgress[idx]->vjipMembers[j].userData, _FILE_AND_LINE_);
        }
      }
  ...
  UpdateVerifiedJoinInProgressMember(
      rakNetGUID, UNASSIGNED_RAKNET_GUID, JIPS_FAILED);

  for (idx = 0; idx < fcm2ParticipantList.Size(); idx++) {
    if (fcm2ParticipantList[idx]->rakNetGuid == rakNetGUID) {
      fcm2ParticipantList[idx] =
          fcm2ParticipantList[fcm2ParticipantList.Size() - 1];
#ifdef DEBUG_FCM2
      printf(
          "Popping participant %s\n",
          fcm2ParticipantList[fcm2ParticipantList.Size() - 1]
              .rakNetGuid.ToString());
#endif
...
void FullyConnectedMesh2::OnFailedConnectionAttempt(
  ...
  if (failedConnectionAttemptReason == FCAR_ALREADY_CONNECTED) {
    UpdateVerifiedJoinInProgressMember(
        packet->guid, packet->guid, JIPS_CONNECTED);
  } else {
    UpdateVerifiedJoinInProgressMember(
        packet->systemAddress, UNASSIGNED_RAKNET_GUID, JIPS_FAILED);
  }
}
void FullyConnectedMesh2::Clear() {
  ...
  for (unsigned int i = 0; i < joinsInProgress.Size(); i++) {
    for (unsigned int j = 0; j < joinsInProgress[i]->vjipMembers.Size(); j++) {
      if (joinsInProgress[i]->vjipMembers[j].userData != nullptr) {
        RakNet::OP_DELETE(
            joinsInProgress[i]->vjipMembers[j].userData, _FILE_AND_LINE_);
      }
    }
...
void FullyConnectedMesh2::PushNewHost(
...
void FullyConnectedMesh2::SendOurFCMGuid(SystemAddress addr) {
  ...
  RakAssert(
      ourFCMGuid != 0); // Can't inform others of our FCM2Guid if it's unset!
  bsOut.Write(ourFCMGuid);
  bsOut.Write(totalConnectionCount);
  bsOut.Write(myContext);
  myContext.ResetReadPointer();
  rakPeerInterface->Send(
      &bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, addr, false);
}
void FullyConnectedMesh2::SendConnectionCountResponse(
    SystemAddress addr,
...
void FullyConnectedMesh2::CalculateHost(
    RakNetGUID* rakNetGuid,
    FCM2Guid* fcm2Guid) {
  ...
  unsigned int idx;
  for (idx = 0; idx < fcm2ParticipantList.Size(); idx++) {
    if (fcm2ParticipantList[idx]->fcm2Guid != 0 &&
        fcm2ParticipantList[idx]->fcm2Guid < lowestFCMGuid) {
...
void FullyConnectedMesh2::GetParticipantCount(
    unsigned int* participantListSize) const {
...
void FullyConnectedMesh2::SetConnectOnNewRemoteConnection(
...
void FullyConnectedMesh2::ConnectToRemoteNewIncomingConnections(
    Packet* packet) {
  unsigned int count;
  RakNet::BitStream bsIn(packet->data, packet->length, false);
  bsIn.IgnoreBytes(sizeof(MessageID));
...
void FullyConnectedMesh2::StartVerifiedJoin(RakNetGUID client) {
  ...
  for (i = 0; i < fcm2ParticipantList.Size(); i++) {
    ...
    bsOut.Write(rakPeerInterface->GetSystemAddressFromGuid(
...
void FullyConnectedMesh2::RespondOnVerifiedJoinCapable(
    Packet* packet,
    bool accept,
  ...
  if (participatingMembersNotOnClient.Size() > 0) {
    ...
    for (i = 0; i < participatingMembersNotOnClient.Size(); i++) {
      ...
      bsOut.Write(rakPeerInterface->GetSystemAddressFromGuid(
          participatingMembersNotOnClient[i]));

      bool written = false;
      for (unsigned int j = 0; j < fcm2ParticipantList.Size(); j++) {
        if (fcm2ParticipantList[j]->rakNetGuid ==
            participatingMembersNotOnClient[i]) {
          written = true;
    ...
    SendUnified(
        &bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, packet->guid, false);
    return;
  }
...
void FullyConnectedMesh2::GetVerifiedJoinRejectedAdditionalData(
    Packet* packet,
    BitStream* additionalData) {
  RakNet::BitStream bsIn(packet->data, packet->length, false);
  bsIn.IgnoreBytes(sizeof(MessageID));
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinStart(Packet* packet) {
  ...
  auto* vjip = RakNet::OP_NEW<VerifiedJoinInProgress>(_FILE_AND_LINE_);
...
void FullyConnectedMesh2::DecomposeJoinCapable(
    Packet* packet,
    VerifiedJoinInProgress* vjip) {
  RakNet::BitStream bsIn(packet->data, packet->length, false);
  bsIn.IgnoreBytes(sizeof(MessageID));
...
void FullyConnectedMesh2::OnVerifiedJoinFailed(
    RakNetGUID hostGuid,
  ...
  for (unsigned int j = 0; j < joinsInProgress[curIndex]->vjipMembers.Size();
       j++) {
    if (joinsInProgress[curIndex]->vjipMembers[j].userData != nullptr) {
      RakNet::OP_DELETE(
          joinsInProgress[curIndex]->vjipMembers[j].userData, _FILE_AND_LINE_);
    }
  }
...
void FullyConnectedMesh2::OnVerifiedJoinAccepted(Packet* packet) {
  ...
  if (systemToAddGuid == rakPeerInterface->GetMyGUID()) {
    ...
    for (unsigned int j = 0; j < joinsInProgress[curIndex]->vjipMembers.Size();
      ...
      if (joinsInProgress[curIndex]->vjipMembers[j].userData != nullptr) {
        RakNet::OP_DELETE(
            joinsInProgress[curIndex]->vjipMembers[j].userData,
            _FILE_AND_LINE_);
      }
    }
...
void FullyConnectedMesh2::UpdateVerifiedJoinInProgressMember(
...
bool FullyConnectedMesh2::ProcessVerifiedJoinInProgressIfCompleted(
    VerifiedJoinInProgress* vjip) {
  //if (vjip->sentResults)
  //	return true;
  ...
  WriteVJCUserData(&bsOut);
  SendUnified(
      &bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, vjip->requester, false);

  //vjip->sentResults=true;
  return true;
}
void FullyConnectedMesh2::WriteVerifiedJoinCapable(
    RakNet::BitStream* bsOut,
    VerifiedJoinInProgress* vjip) {
  bsOut->Write((MessageID)ID_FCM2_VERIFIED_JOIN_CAPABLE);
...
void FullyConnectedMesh2::ReadVerifiedJoinInProgressMember(
...
unsigned int FullyConnectedMesh2::GetVerifiedJoinInProgressMemberIndex(
    const AddressOrGUID systemIdentifier,
    VerifiedJoinInProgress* vjip) {
  for (unsigned int j = 0; j < vjip->vjipMembers.Size(); j++) {
    if ((systemIdentifier.rakNetGuid != UNASSIGNED_RAKNET_GUID &&
        ...
        (systemIdentifier.systemAddress != UNASSIGNED_SYSTEM_ADDRESS &&
         vjip->vjipMembers[j].systemAddress ==
...
void FullyConnectedMesh2::CategorizeVJIP(
    VerifiedJoinInProgress* vjip,
    DataStructures::List<RakNetGUID>& participatingMembersOnClientSucceeded,
    DataStructures::List<RakNetGUID>& participatingMembersOnClientFailed,
    DataStructures::List<RakNetGUID>& participatingMembersNotOnClient,
  ...
  for (unsigned int i = 0; i < fcm2ParticipantList.Size(); i++) {
    unsigned int j = GetVerifiedJoinInProgressMemberIndex(
        fcm2ParticipantList[i]->rakNetGuid, vjip);
    if (j == (unsigned int)-1) {
