...
void DynDNS::UpdateHostIPAsynch(
    const char* dnsHost,
  ...
  char outputData[512];
  Base64Encoding(
      (const unsigned char*)usernameAndPassword,
      (int)strlen(usernameAndPassword),
      outputData);
...
void DynDNS::Update() {
  ...
  if (serverAddress != UNASSIGNED_SYSTEM_ADDRESS) {
    if (connectPhase == CP_CONNECTING_TO_CHECKIP) {
      ...
      tcp->Send(
          getString.C_String(),
          (unsigned int)getString.GetLength(),
          serverAddress,
          false);
    }
  ...
  if (connectPhase == CP_WAITING_FOR_CHECKIP_RESPONSE &&
