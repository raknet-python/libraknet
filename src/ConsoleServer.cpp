...
void ConsoleServer::Update() {
  ...
  while (p) {
    ...
    if (_stricmp(*parameterList, "help") == 0 && numParameters <= 2) {
      ...
      {
        for (i = 0; i < commandParserList.Size(); i++) {
          if (_stricmp(parameterList[1], commandParserList[i]->GetName()) ==
              0) {
            commandParsed = true;
      ...
      if (numParameters >= 2) // At minimum <CommandParserName> <Command>
      {
        ...
        if (**parameterList >= '0' && **parameterList <= '9') {
          commandParserIndex = atoi(
              *parameterList); // Use specified parser unless it's an invalid number
          ...
          for (i = 0; i < commandParserList.Size(); i++) {
            if (_stricmp(parameterList[0], commandParserList[i]->GetName()) ==
                0) {
        ...
        if (!failed) {
          ...
          if (commandParserIndex != (unsigned)-1) {
            ...
            tryAllParsers = false;
            if (commandParserList[commandParserIndex]->GetRegisteredCommand(
                    parameterList[1], &rc)) {
              commandParsed = true;
              if (rc.parameterCount ==
