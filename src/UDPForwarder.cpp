...
void UDPForwarder::RecvFrom(
  ...
  if (receivedDataLen < 0) {
    ...
    !defined(WINDOWS_STORE_RT)
    DWORD dwIOError = WSAGetLastError();

    if (dwIOError != WSAECONNRESET && dwIOError != WSAEINTR &&
        dwIOError != WSAETIMEDOUT && dwIOError != WSAEWOULDBLOCK) {
  ...
  bool matchConfirmed1 =
      confirmed1 && forwardEntry->addr1Confirmed == receivedAddr;
  bool matchConfirmed2 =
      confirmed2 && forwardEntry->addr2Confirmed == receivedAddr;
  bool matchUnconfirmed1 =
      forwardEntry->addr1Unconfirmed.EqualsExcludingPort(receivedAddr);
  bool matchUnconfirmed2 =
      forwardEntry->addr2Unconfirmed.EqualsExcludingPort(receivedAddr);

  if (matchConfirmed1 ||
      (!matchConfirmed2 && !confirmed1 && matchUnconfirmed1)) {
    // Forward to addr2
  ...
  while (true) {
    ...
    if (GetUsedForwardEntries() > maxForwardEntries) {
      ...
      if (sfos.result == UDPFORWARDER_RESULT_COUNT) {
        ...
        auto* fe = RakNet::OP_NEW<UDPForwarder::ForwardEntry>(_FILE_AND_LINE_);
        ...
        int ret = bind__(
            fe->socket,
            (struct sockaddr*)&listenerSocketAddress,
            sizeof(listenerSocketAddress));
        if (ret == -1) {
        ...
        for (aip = servinfo; aip != nullptr; aip = aip->ai_next) {
          ...
          // getaddrinfo() gave us.
          fe->socket =
              socket__(aip->ai_family, aip->ai_socktype, aip->ai_protocol);
...
#pragma warning( \
  ...
  while (i < forwardListNotUpdated.Size()) {
    if (curTime >
            forwardListNotUpdated[i]
                ->timeLastDatagramForwarded && // Account for timestamp wrap
        curTime > forwardListNotUpdated[i]->timeLastDatagramForwarded +
