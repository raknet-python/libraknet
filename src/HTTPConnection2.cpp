...
bool HTTPConnection2::TransmitRequest(
  ...
  if (useAddress != UNASSIGNED_SYSTEM_ADDRESS) {
    ...
    if (request->hostEstimatedAddress.FromString(host, '|', ipVersion) ==
        false) {
      RakNet::OP_DELETE(request, _FILE_AND_LINE_);
      return false;
    }
...
#else
      RakAssert(
          "HTTPConnection2::TransmitRequest needs define  RAKNET_SUPPORT_IPV6" &&
          0);
#endif
    }
  }
...
PluginReceiveResult HTTPConnection2::OnReceive(Packet* packet) {
  ...
  for (i = 0; i < sentRequests.Size(); i++) {
    ...
    if (sentRequest->hostCompletedAddress == packet->systemAddress) {
      ...
      const char* isFirstChunk =
          strstr((char*)packet->data, "Transfer-Encoding: chunked");
      if (isFirstChunk) {
        ...
        if (sentRequest->thisChunkSize == 0) {
          ...
          body_header += 2;
          ReadChunkBlock(
              sentRequest->thisChunkSize,
              sentRequest->bytesReadForThisChunk,
              body_header,
        ...
        if (sentRequest->contentLength == -1) {
          const char* length_header = strstr(
          ...
          if (length_header) {
            ...
            for (clLength = 0;
                 length_header[clLength] && length_header[clLength] >= '0' &&
            ...
            if (clLength > 0 &&
              ...
              sentRequest->contentLength =
                  RakString::ReadIntFromSubstring(length_header, 0, clLength);
            }
          }
        }
          ...
          if (sentRequest->contentLength > 0) {
            ...
            if (body_header) {
              ...
              if (slen >= (size_t)sentRequest->contentLength) {
                sentRequest->contentOffset =
                    body_header - sentRequest->stringReceived.C_String();
                completedRequestsMutex.Lock();
                completedRequests.Push(sentRequest, _FILE_AND_LINE_);
                completedRequestsMutex.Unlock();
...
void HTTPConnection2::OnNewConnection(
...
void HTTPConnection2::OnFailedConnectionAttempt(
    Packet* packet,
...
void HTTPConnection2::OnClosedConnection(
  ...
  while (i < sentRequests.Size()) {
    if (sentRequests[i]->hostCompletedAddress == systemAddress) {
      Request* sentRequest = sentRequests[i];
      if (sentRequest->chunked == false &&
          sentRequest->stringReceived.IsEmpty() == false) {
        if (strstr(
                sentRequest->stringReceived.C_String(), "Content-Length: ")) {
          char* body_header =
              strstr((char*)sentRequest->stringReceived.C_String(), "\r\n\r\n");
          if (body_header) {
            body_header += 4; // strlen("\r\n\r\n");
            sentRequest->contentOffset =
                body_header - sentRequest->stringReceived.C_String();
...
void HTTPConnection2::SendRequest(Request* request) {
  tcpInterface->Send(
      ...
      (unsigned int)request->stringToTransmit.GetLength(),
      request->hostCompletedAddress,
      false);
}
