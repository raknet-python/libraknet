...
RNS2BindResult RNS2_NativeClient::Bind(
    NativeClientBindParameters* bindParameters,
    const char* file,
    unsigned int line) {
  ...
  if (Module::Get()->GetBrowserInterface(PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4)) {
    rns2Socket =
        ((PPB_UDPSocket_Private_0_4*)Module::Get()->GetBrowserInterface(
             PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
            ->Create(bindParameters->nativeClientInstance);
    RAKNET_DEBUG_PRINTF(
        "CreateChromeSocket(%d,%s,0x%08x,%d) ==> 0x%08x\n",
        bindParameters->port,
        bindParameters->forceHostAddress ? bindParameters->forceHostAddress
                                         : "(null)",
        bindParameters->nativeClientInstance,
        bindParameters->is_ipv6,
        rns2Socket);

    // Enable the broadcast feature on the socket (must happen before the
    // bind call)
    ((PPB_UDPSocket_Private_0_4*)pp::Module::Get()->GetBrowserInterface(
         PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
        ->SetSocketFeature(
            rns2Socket, PP_UDPSOCKETFEATURE_BROADCAST, PP_MakeBool(PP_TRUE));
    ...
    if (bindParameters->forceHostAddress) {
      ...
      if (bindParameters->is_ipv6) {
        ...
        NetAddressPrivate::CreateFromIPv6Address(
            ipv6, 0, bindParameters->port, &client_addr);
      } else {
        ...
        NetAddressPrivate::CreateFromIPv4Address(
            ipv4, bindParameters->port, &client_addr);
      }
    } else {
      NetAddressPrivate::GetAnyAddress(bindParameters->is_ipv6, &client_addr);
      NetAddressPrivate::ReplacePort(
          client_addr, bindParameters->port, &client_addr);
    }

    bindState = BS_IN_PROGRESS;

    RAKNET_DEBUG_PRINTF(
        "attempting to bind to %s\n",
        NetAddressPrivate::Describe(client_addr, true).c_str());
    PP_CompletionCallback cc =
        PP_MakeCompletionCallback(RNS2_NativeClient::onSocketBound, this);
    ((PPB_UDPSocket_Private_0_4*)Module::Get()->GetBrowserInterface(
         PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
...
void RNS2_NativeClient::SendImmediate(RNS2_SendParameters_NativeClient* sp) {
  ...
  NetAddressPrivate::CreateFromIPv6Address(
      sp->systemAddress.address.addr6.sin6_addr.u.Byte,
      0,
      sp->systemAddress.GetPort(),
      &client_addr);
#else
  NetAddressPrivate::CreateFromIPv4Address(
      (const uint8_t*)&sp->systemAddress.address.addr4.sin_addr,
      sp->systemAddress.GetPort(),
      &client_addr);
#endif
...
void RNS2_NativeClient::onRecvFrom(void* pData, int32_t dataSize) {
  ...
  if (dataSize <= 0 || !pData) {
    ...
    if (dataSize == PP_ERROR_ABORTED) {
      RAKNET_DEBUG_PRINTF(
          "onRecvFrom error PP_ERROR_ABORTED, killing recvfrom loop\n");
    } else {
  ...
  bool ok = false;
  if (((PPB_UDPSocket_Private_0_4*)Module::Get()->GetBrowserInterface(
           PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
    ...
    if (family == PP_NETADDRESSFAMILY_IPV4) {
      ok = NetAddressPrivate::GetAddress(
          addr,
          &recvStruct->systemAddress.address.addr4.sin_addr,
          sizeof(in_addr));
    }
#if RAKNET_SUPPORT_IPV6 == 1
    else {
      ok = NetAddressPrivate::GetAddress(
          addr,
          &recvStruct->systemAddress.address.addr6.sin6_addr,
          sizeof(in6_addr));
    }
#endif
  }
  ...
  if (ok) {
    recvStruct->systemAddress.SetPortHostOrder(
        pp::NetAddressPrivate::GetPort(addr));
...
void RNS2_NativeClient::IssueReceiveCall(void) {
  ...
  if (recvFromStruct != NULL) {
    ...
    PP_CompletionCallback cc =
        PP_MakeCompletionCallback(onRecvFrom, recvFromStruct);
    ((PPB_UDPSocket_Private_0_4*)Module::Get()->GetBrowserInterface(
         PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
