...
void RakString::Realloc(SharedString* sharedString, size_t bytes) {
  ...
  if (oldBytes <= (size_t)smallStringSize &&
      newBytes > (size_t)smallStringSize) {
...
const RakNet::RakString operator+(
  ...
  if (RakString::freeList.Size() == 0) {
    ...
    for (i = 0; i < 128; i++) {
      ...
      ss = (RakString::SharedString*)rakMalloc_Ex(
          sizeof(RakString::SharedString), _FILE_AND_LINE_);
  ...
  if (allocatedBytes <= (size_t)smallStringSize) {
    ...
    sharedString->bigString =
        (char*)rakMalloc_Ex(sharedString->bytesUsed, _FILE_AND_LINE_);
    sharedString->c_str = sharedString->bigString;
  }
...
WCHAR* RakString::ToWideChar() {
  ...
  //
  auto* pszUTF16 = RakNet::OP_NEW_ARRAY<WCHAR>(cchUTF16, __FILE__, __LINE__);
...
size_t RakString::Find(const char* stringToFind, size_t pos) {
  ...
  if (pos >= len || stringToFind == nullptr || stringToFind[0] == 0) {
    return (size_t)-1;
  }
...
bool RakString::IsEmailAddress() const {
  ...
  if (strLen < 6) // a@b.de
    return false;
  if (sharedString->c_str[strLen - 4] != '.' &&
      sharedString->c_str[strLen - 3] != '.') // .com, .net., .org, .de
    return false;
  unsigned i;
  // Has non-printable?
  for (i = 0; i < strLen; i++) {
...
RakNet::RakString& RakString::SQLEscape() {
  ...
  for (index = 0; index < strLen; index++) {
    if (sharedString->c_str[index] == '\'' ||
        sharedString->c_str[index] == '"' || sharedString->c_str[index] == '\\')
...
RakNet::RakString RakString::FormatForPUTOrPost(
    const char* type,
    const char* uri,
...
RakString RakString::FormatForPOST(
    const char* uri,
    const char* contentType,
    const char* body,
    const char* extraHeaders) {
...
RakString RakString::FormatForPUT(
    const char* uri,
    const char* contentType,
    const char* body,
    const char* extraHeaders) {
...
RakString RakString::FormatForDELETE(
    const char* uri,
    const char* extraHeaders) {
  RakString out;
  RakString host;
...
#ifdef _WIN32
  if (fixedString.sharedString
          ->c_str[strlen(fixedString.sharedString->c_str) - 1] != '\\') {
    fixedString += '\\';
  }
#else
  if (fixedString.sharedString
