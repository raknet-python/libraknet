...
void RNS2_Berkley::SetSocketOptions(void) {
  ...
  r = setsockopt__(
      rns2Socket, SOL_SOCKET, SO_RCVBUF, (char*)&sock_opt, sizeof(sock_opt));
  RakAssert(r == 0);
  ...
  sock_opt = 0;
  r = setsockopt__(
      rns2Socket, SOL_SOCKET, SO_LINGER, (char*)&sock_opt, sizeof(sock_opt));
  ...
  sock_opt = 1024 * 16;
  r = setsockopt__(
      rns2Socket, SOL_SOCKET, SO_SNDBUF, (char*)&sock_opt, sizeof(sock_opt));
  RakAssert(r == 0);
}
...
void RNS2_Berkley::GetSystemAddressIPV4(
...
void RNS2_Berkley::GetSystemAddressIPV4And6(
  ...
  if (ss.ss_family == AF_INET) {
    memcpy(
        &systemAddressOut->address.addr4,
        (sockaddr_in*)&ss,
        sizeof(sockaddr_in));
    systemAddressOut->debugPort =
        ntohs(systemAddressOut->address.addr4.sin_port);
    ...
    memcpy(
        &systemAddressOut->address.addr6,
        (sockaddr_in6*)&ss,
        sizeof(sockaddr_in6));
    systemAddressOut->debugPort =
        ntohs(systemAddressOut->address.addr6.sin6_port);
...
#endif
RNS2BindResult RNS2_Berkley::BindSharedIPV4(
  ...
  rns2Socket = (int)socket__(
      bindParameters->addressFamily,
      bindParameters->type,
  ...
  ret = bind__(
      rns2Socket,
      (struct sockaddr*)&boundAddress.address.addr4,
      sizeof(boundAddress.address.addr4));
  ...
  if (ret <= -1) {
    ...
    switch (ret) {
      ...
      case ENOTSOCK:
        RAKNET_DEBUG_PRINTF(
            "bind__(): Argument is a descriptor for a file, not a socket.\n");
        break;

      case EINVAL:
        RAKNET_DEBUG_PRINTF(
            "bind__(): The addrlen is wrong, or the socket was not in the AF_UNIX family.\n");
        break;
      case EROFS:
        RAKNET_DEBUG_PRINTF(
            "bind__(): The socket inode would reside on a read-only file system.\n");
        break;
      case EFAULT:
        RAKNET_DEBUG_PRINTF(
            "bind__(): my_addr points outside the user's accessible address space.\n");
        break;
      case ENAMETOOLONG:
      ...
      case ENOMEM:
        RAKNET_DEBUG_PRINTF(
            "bind__(): Insufficient kernel memory was available.\n");
        break;
      case ENOTDIR:
        RAKNET_DEBUG_PRINTF(
            "bind__(): A component of the path prefix is not a directory.\n");
        break;
      case EACCES:
        // Port reserved on PS4
        RAKNET_DEBUG_PRINTF(
            "bind__(): Search permission is denied on a component of the path prefix.\n");
        break;

      case ELOOP:
        RAKNET_DEBUG_PRINTF(
            "bind__(): Too many symbolic links were encountered in resolving my_addr.\n");
        break;
...
RNS2BindResult RNS2_Berkley::BindSharedIPV4And6(
  ...
  if (bindParameters->hostAddress &&
      (_stricmp(bindParameters->hostAddress, "UNASSIGNED_SYSTEM_ADDRESS") ==
           0 ||
  ...
  for (aip = servinfo; aip != NULL; aip = aip->ai_next) {
    ...
    if (ret >= 0) {
      // Is this valid?
      memcpy(
          &boundAddress.address.addr6,
          aip->ai_addr,
          sizeof(boundAddress.address.addr6));
...
void RNS2_Berkley::RecvFromBlockingIPV4And6(RNS2RecvStruct* recvFromStruct) {
  ...
  recvFromStruct->bytesRead = recvfrom__(
      rns2Socket,
      recvFromStruct->data,
      dataOutSize,
      flag,
...

  {
    if (their_addr.ss_family == AF_INET) {
      memcpy(
          &recvFromStruct->systemAddress.address.addr4,
      ...
      memcpy(
          &recvFromStruct->systemAddress.address.addr6,
          (sockaddr_in6*)&their_addr,
          sizeof(sockaddr_in6));
      recvFromStruct->systemAddress.debugPort =
...
void RNS2_Berkley::RecvFromBlockingIPV4(RNS2RecvStruct* recvFromStruct) {
...

  {
    ...
    recvFromStruct->systemAddress.address.addr4.sin_addr.s_addr =
        sa.sin_addr.s_addr;
  }
