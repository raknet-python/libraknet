...
void NatTypeDetectionServer::Startup(
  ...
  s1p2 = CreateNonblockingBoundSocket(
      str,
#ifdef __native_client__
      chromeInstance,
#endif
  ...
  s2p3 = CreateNonblockingBoundSocket(
      nonRakNetIP2,
#ifdef __native_client__
      chromeInstance,
#endif
  ...
  s3p4 = CreateNonblockingBoundSocket(
      nonRakNetIP3,
#ifdef __native_client__
      chromeInstance,
#endif
  ...
  s4p5 = CreateNonblockingBoundSocket(
      nonRakNetIP4,
#ifdef __native_client__
      chromeInstance,
#endif
...
void NatTypeDetectionServer::Update() {
  ...
  while (recvStruct) {
    ...
    if (data[0] == NAT_TYPE_PORT_RESTRICTED && recvStruct->socket == s3p4) {
      RakNet::BitStream bsIn(
          (unsigned char*)data, recvStruct->bytesRead, false);
      ...
      if (readSuccess) {
        ...
        if (i != (unsigned int)-1) {
          ...
          rakPeerInterface->Send(
              ...
              RELIABLE,
              0,
              natDetectionAttempts[i].systemAddress,
              false);
  ...
  while (i < (int)natDetectionAttempts.Size()) {
    if (time > natDetectionAttempts[i].nextStateTime) {
      ...
      natDetectionAttempts[i].detectionState =
          (NATDetectionState)((int)natDetectionAttempts[i].detectionState + 1);
      natDetectionAttempts[i].nextStateTime =
          time + natDetectionAttempts[i].timeBetweenAttempts;
      SystemAddress saOut;
      unsigned char c;
      bs.Reset();
      switch (natDetectionAttempts[i].detectionState) {
        ...
        case STATE_TESTING_FULL_CONE_2:
          ...
          saOut = natDetectionAttempts[i].systemAddress;
          saOut.SetPortHostOrder(
              natDetectionAttempts[i].systemAddress.GetPort());
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void NatTypeDetectionServer::DeallocRNS2RecvStruct(
    RNS2RecvStruct* s,
    const char* file,
    unsigned int line) {
