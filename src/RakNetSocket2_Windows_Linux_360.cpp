...
#if (                                                              \
    defined(_WIN32) || defined(__GNUC__) || defined(__GCCXML__) || \
    defined(__S3E__)) &&                                           \
    !defined(WINDOWS_STORE_RT) && !defined(__native_client__)

RNS2SendResult RNS2_Windows_Linux_360::Send_Windows_Linux_360NoVDP(
  ...
  do {
    ...
    if (sendParameters->ttl > 0) {
      ...
      if (getsockopt__(
              rns2Socket,
              sendParameters->systemAddress.GetIPPROTO(),
              IP_TTL,
              (char*)&oldTTL,
              &opLen) != -1) {
