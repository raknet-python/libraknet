...
int RakNet::ReadyEvent::RemoteSystemCompByGuid(
...
bool ReadyEvent::SetEvent(int eventId, bool isReady) {
  bool objectExists;
  unsigned eventIndex =
      readyEventNodeList.GetIndexFromKey(eventId, &objectExists);
  ...
  if (eventExists) {
    if (guid == UNASSIGNED_RAKNET_GUID) {
      ...
      unsigned systemIndex =
          readyEventNodeList[eventIndex]->systemList.GetIndexFromKey(
              guid, &systemExists);
      if (systemExists) {
...
RakNetGUID ReadyEvent::GetFromWaitListAtIndex(int eventId, unsigned index)
    const {
  bool objectExists;
  unsigned readyIndex =
      readyEventNodeList.GetIndexFromKey(eventId, &objectExists);
...
void ReadyEvent::OnReadyEventPacketUpdate(Packet* packet) {
  ...
  if (objectExists) {
    ...
    bool systemExists;
    unsigned systemIndex =
        ren->systemList.GetIndexFromKey(packet->guid, &systemExists);
    if (systemExists) {
...
void ReadyEvent::OnReadyEventQuery(Packet* packet) {
  ...
  if (objectExists) {
    unsigned systemIndex =
        readyEventNodeList[readyIndex]->systemList.GetIndexFromKey(
            packet->guid, &objectExists);
...
void ReadyEvent::SendReadyUpdate(
  ...
  if ((ren->eventStatus != ren->systemList[systemIndex].lastSentStatus) ||
    ...
    SendUnified(
        ...
        RELIABLE_ORDERED,
        channel,
        ren->systemList[systemIndex].rakNetGuid,
        false);
...
void ReadyEvent::RemoveFromAllLists(RakNetGUID guid) {
  ...
  for (eventIndex = 0; eventIndex < readyEventNodeList.Size(); eventIndex++) {
    ...
    systemIndex = readyEventNodeList[eventIndex]->systemList.GetIndexFromKey(
        guid, &systemExists);
    if (systemExists)
      readyEventNodeList[eventIndex]->systemList.RemoveAtIndex(systemIndex);
