...
void TeamBalancer::RequestSpecificTeam(NetworkID memberId, TeamId desiredTeam) {
  ...
  for (unsigned int i = 0; i < myTeamMembers.Size(); i++) {
    if (myTeamMembers[i].memberId == memberId) {
      foundMatch = true;
      if (myTeamMembers[i].requestedTeam == desiredTeam &&
          myTeamMembers[i].currentTeam == desiredTeam) {
        return;
      } else {
...
void TeamBalancer::OnStatusUpdateToNewHost(Packet* packet) {
  ...
  for (uint8_t i = 0; i < requestedTeamChangeListSize; i++) {
    ...
    if (tm.currentTeam != UNASSIGNED_TEAM_ID &&
      ...
      RakAssert(
          "Current team out of range in TeamBalancer::OnStatusUpdateToNewHost" &&
          0);
      return;
    }
    ...
    if (tm.requestedTeam != UNASSIGNED_TEAM_ID &&
      ...
      RakAssert(
          "Requested team out of range in TeamBalancer::OnStatusUpdateToNewHost" &&
          0);
      return;
    }
      ...
      if (tm.currentTeam == UNASSIGNED_TEAM_ID) {
        ...
        if (tm.requestedTeam == UNASSIGNED_TEAM_ID) {
          ...
          if (!TeamWouldBeOverpopulatedOnAddition(
                  tm.requestedTeam, teamMembers.Size())) {
...
void TeamBalancer::OnRequestSpecificTeam(Packet* packet) {
  ...
  if (tm.requestedTeam > teamLimits.Size()) {
    RakAssert(
        "Requested team out of range in TeamBalancer::OnRequestSpecificTeam" &&
        0);
    return;
  }
  if (memberIndex == (unsigned int)-1) {
    ...
    // Assign to requested team if possible. Otherwise, assign to a default team
    if (TeamsWouldBeEvenOnSwitch(tm.requestedTeam, oldTeamThisUserWasOn)) {
...
unsigned int TeamBalancer::GetMemberIndex(NetworkID memberId, RakNetGUID guid)
    const {
  for (unsigned int i = 0; i < teamMembers.Size(); i++) {
...
void TeamBalancer::RemoveTeamMember(unsigned int index) {
  ...
  teamMemberCounts[teamMembers[index].currentTeam] =
...
unsigned int TeamBalancer::GetMemberIndexToSwitchTeams(
...
void TeamBalancer::NotifyTeamAssigment(unsigned int teamMemberIndex) {
  ...
  rakPeerInterface->Send(
      ...
      RELIABLE_ORDERED,
      0,
      teamMembers[teamMemberIndex].memberGuid,
      false);
}
...
TeamId TeamBalancer::GetNextDefaultTeam() {
  ...
  switch (defaultAssigmentAlgorithm) {
    ...
    default: {
      RakAssert(
          "TeamBalancer::GetNextDefaultTeam unknown algorithm enumeration" &&
          0);
      return UNASSIGNED_TEAM_ID;
    }
  }
}
bool TeamBalancer::TeamWouldBeOverpopulatedOnAddition(
    TeamId teamId,
...
bool TeamBalancer::TeamWouldBeUnderpopulatedOnLeave(
    TeamId teamId,
    unsigned int teamMemberSize) {
  if (forceTeamsToBeEven) {
    unsigned int minMembersOnASingleTeam =
        (teamMemberSize - 1) / teamLimits.Size();
...
TeamId TeamBalancer::GetSmallestNonFullTeam() const {
  ...
  for (idx = 0; idx < teamMemberCounts.Size(); idx++) {
    if (teamMemberCounts[idx] < smallestTeamCount &&
        teamMemberCounts[idx] < teamLimits[idx]) {
...
TeamId TeamBalancer::MoveMemberThatWantsToJoinTeamInternal(TeamId teamId) {
  ...
  if (membersThatWantToJoinTheTeam.Size() > 0) {
    TeamId oldTeam;
    unsigned int swappedMemberIndex = membersThatWantToJoinTheTeam
        [randomMT() % membersThatWantToJoinTheTeam.Size()];
...
bool TeamBalancer::TeamsWouldBeEvenOnSwitch(TeamId t1, TeamId t2) {
  ...
  return !TeamWouldBeOverpopulatedOnAddition(t1, teamMembers.Size() - 1) &&
      !TeamWouldBeUnderpopulatedOnLeave(t2, teamMembers.Size() - 1);
}
