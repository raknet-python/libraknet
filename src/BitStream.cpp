...
#if defined(_MSC_VER) && \
    _MSC_VER < 1299 // VC6 doesn't support template specialization
...
BitStream::BitStream(const unsigned int initialBytesToAllocate) {
  ...
  if (initialBytesToAllocate <= BITSTREAM_STACK_ALLOCATION_SIZE) {
    ...
    data = (unsigned char*)rakMalloc_Ex(
        (size_t)initialBytesToAllocate, _FILE_AND_LINE_);
  ...
  if (copyData) {
    if (lengthInBytes > 0) {
      if (lengthInBytes < BITSTREAM_STACK_ALLOCATION_SIZE) {
        ...
        data = (unsigned char*)rakMalloc_Ex(
            (size_t)lengthInBytes, _FILE_AND_LINE_);
      }
#ifdef _DEBUG
      RakAssert(data);
#endif
...
void BitStream::Write(
  ...
  if ((numberOfBitsUsed & 7) == 0) {
    ...
    memcpy(
        data + BITS_TO_BYTES(numberOfBitsUsed),
        inputByteArray,
...
void BitStream::Write(BitStream* bitStream, BitSize_t numberOfBits) {
  ...
  if ((bitStream->GetReadOffset() & 7) == 0 && (numberOfBitsUsed & 7) == 0) {
    ...
    memcpy(
        ...
        bitStream->GetData() + readOffsetBytes,
        numBytes);
  ...
  while (numberOfBits-- > 0 &&
         bitStream->readOffset + 1 <= bitStream->numberOfBitsUsed) {
    numberOfBitsMod8 = numberOfBitsUsed & 7;
    if (numberOfBitsMod8 == 0) {
      // New byte
      if (bitStream->data[bitStream->readOffset >> 3] &
...
void BitStream::WriteAlignedBytes(
...
void BitStream::WriteAlignedBytesSafe(
  ...
  WriteAlignedBytes(
      (const unsigned char*)inByteArray,
      inputLength < maxBytesToWrite ? inputLength : maxBytesToWrite);
}
...
bool BitStream::ReadAlignedBytes(
...
void BitStream::WriteBits(
  ...
  if (numberOfBitsUsedMod8 == 0 && (numberOfBitsToWrite & 7) == 0) {
    memcpy(
        data + (numberOfBitsUsed >> 3), inByteArray, numberOfBitsToWrite >> 3);
  ...
  //do
  {
    ...
    if (numberOfBitsUsedMod8 == 0) {
      ...
      *(data + (numberOfBitsUsed >> 3)) |=
          dataByte >> (numberOfBitsUsedMod8); // First half

      if (8 - (numberOfBitsUsedMod8) < 8 &&
          8 - (numberOfBitsUsedMod8) <
              numberOfBitsToWrite) // If we didn't write it all out in the first half (8 - (numberOfBitsUsed%8) is the number we wrote in the first half)
      {
        *(data + (numberOfBitsUsed >> 3) + 1) =
            (unsigned char)(dataByte
                            << (8 -
                                (numberOfBitsUsedMod8))); // Second half (overlaps byte boundary)
      }
    }
...
void BitStream::WriteCompressed(
    const unsigned char* inByteArray,
  ...
  while (currentByte > 0) {
    if (inByteArray[currentByte] ==
        byteMatch) // If high byte is byteMatch (0 of 0xff) then it would have the same value shifted
    {
      bool b = true;
      Write(b);
    } else {
...
bool BitStream::ReadBits(
  ...
  while (numberOfBitsToRead > 0) {
    *(inOutByteArray + offset) |= *(data + (readOffset >> 3))
        << (readOffsetMod8); // First half
...
void BitStream::AddBitsAndReallocate(const BitSize_t numberOfBitsToWrite) {
  ...
  if (numberOfBitsToWrite + numberOfBitsUsed > 0 &&
      ((numberOfBitsAllocated - 1) >> 3) <
          ((newNumberOfBitsAllocated - 1) >>
           3)) // If we need to allocate 1 or more new bytes
  {
#ifdef _DEBUG
    ...
    if (newNumberOfBitsAllocated - (numberOfBitsToWrite + numberOfBitsUsed) >
        1048576) {
    ...
    if (data == (unsigned char*)stackData) {
      if (amountToAllocate > BITSTREAM_STACK_ALLOCATION_SIZE) {
        data = (unsigned char*)rakMalloc_Ex(
            (size_t)amountToAllocate, _FILE_AND_LINE_);
        RakAssert(data);
        ...
        memcpy(
            (void*)data,
            (void*)stackData,
            (size_t)BITS_TO_BYTES(numberOfBitsAllocated));
      }
    } else {
      data = (unsigned char*)rakRealloc_Ex(
          data, (size_t)amountToAllocate, _FILE_AND_LINE_);
    }
...
void BitStream::PrintBits(char* out) const {
  ...
  for (BitSize_t counter = 0;
       counter < BITS_TO_BYTES(numberOfBitsUsed) && strIndex < 2000;
       counter++) {
    BitSize_t stop;
...
BitSize_t BitStream::CopyData(unsigned char** _data) const {
  ...
  *_data = (unsigned char*)rakMalloc_Ex(
      (size_t)BITS_TO_BYTES(numberOfBitsUsed), _FILE_AND_LINE_);
  memcpy(
      *_data,
      data,
      sizeof(unsigned char) * (size_t)(BITS_TO_BYTES(numberOfBitsUsed)));
...
bool BitStream::ReadFloat16(float& outFloat, float floatMin, float floatMax) {
  ...
  if (Read(percentile)) {
    ...
    outFloat =
        floatMin + ((float)percentile / 65535.0f) * (floatMax - floatMin);
