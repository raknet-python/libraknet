...
ref class ListenerContext sealed {
 ...
 private:
  ...
  Platform::Collections::Map<uint64_t, OutputStreamAndDataWriter ^> ^
      outputStreamMap;
...
ListenerContext::ListenerContext(
  ...
  outputStreamMap = ref new Platform::Collections::
      Map<uint64_t, OutputStreamAndDataWriter ^>();
}
...
OutputStreamAndDataWriter ^
    ListenerContext::GetOutputStreamAndDataWriter(uint64_t s_addr) {
  ...
  task<IOutputStream ^> op(listener->GetOutputStreamAsync(
      hostName, ref new Platform::String(w_char2)));
  op.wait();
  OutputStreamAndDataWriter ^ outputStreamAndDataWriter =
      ref new OutputStreamAndDataWriter;
  outputStreamAndDataWriter->outputStream = op.get();
  outputStreamAndDataWriter->dataWriter =
      ref new DataWriter(outputStreamAndDataWriter->outputStream);
...
void ListenerContext::OnMessage(
  ...
  RNS2_WindowsStore8* rns2 =
      RNS2_WindowsStore8::GetRNS2FromDatagramSocket(socket);
  if (rns2 == 0)
    return;
  ...
  RNS2RecvStruct* recvFromStruct =
      eventHandler->AllocRNS2RecvStruct(_FILE_AND_LINE_);
  ...
  Platform::Array<unsigned char> ^ managedBytes =
      ref new Platform::Array<unsigned char>(uselessBuffer->Length);
  ...
  recvFromStruct->systemAddress.address.addr4.sin_addr.s_addr =
      RNS2_WindowsStore8::WinRTInet_Addr(ip);
  char portStr[64];
...
RNS2BindResult RNS2_WindowsStore8::Bind(Platform::String ^ localServiceName) {
  ...
  listener->MessageReceived += ref new TypedEventHandler<
      DatagramSocket ^
...
RNS2SendResult RNS2_WindowsStore8::Send(
  ...
  OutputStreamAndDataWriter ^ outputStreamAndDataWriter =
      listenerContext->GetOutputStreamAndDataWriter(s_addr);
  // DataWriter ^dataWriter = ref new DataWriter(outputStream);

  auto platformBuffer = ref new Platform::Array<BYTE>(
      (unsigned char*)sendParameters->data, (UINT)sendParameters->length);
  ...
  task<unsigned int>(outputStreamAndDataWriter->dataWriter->StoreAsync())
      .then([&](task<unsigned int> writeTask) {
        try {
...
void RNS2_WindowsStore8::DomainNameToIP(const char* domainName, char ip[65]) {
  ...
  IAsyncOperation<
      ...
      op = DatagramSocket::GetEndpointPairsAsync(hostName, L"0");
  ...
  op->Completed = ref new AsyncOperationCompletedHandler<
      ...
      [&](IAsyncOperation<
        ...
        Windows::Foundation::Collections::IVectorView<
            Windows::Networking::EndpointPair ^> ^
            result2 = result->GetResults();
        if (result2->Size > 0) {
          Platform::String ^ name =
              result2->GetAt(0)->RemoteHostName->DisplayName;
          RakString rs2;
...
int RNS2_WindowsStore8::WinRTSetSockOpt(
    Windows::Networking::Sockets::DatagramSocket ^ s,
    int level,
    int optname,
    const char* optval,
...
int RNS2_WindowsStore8::WinRTIOCTLSocket(
    Windows::Networking::Sockets::DatagramSocket ^ s,
    long cmd,
    unsigned long* argp) {
...
int RNS2_WindowsStore8::WinRTGetSockName(
    Windows::Networking::Sockets::DatagramSocket ^ s,
