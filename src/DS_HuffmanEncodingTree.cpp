...
void HuffmanEncodingTree::GenerateFromFrequencyTable(
  ...
  int counter;
  HuffmanEncodingTreeNode* node;
  HuffmanEncodingTreeNode* leafList
      [256]; // Keep a copy of the pointers to all the leaves so we can generate the encryption table bottom-up, which is easier
  ...
  for (counter = 0; counter < 256; counter++) {
    ...
    InsertNodeIntoSortedList(
        node, &huffmanEncodingTreeNodeList); // Insert and maintain sort order.
  }
...
#pragma warning( \
  ...
  for (counter = 0; counter < 256; counter++) {
    ...
    encodingTable[counter].bitLength =
        (unsigned char)bitStream.CopyData(&encodingTable[counter].encoding);
...
void HuffmanEncodingTree::EncodeArray(
  ...
  if (output->GetNumberOfBitsUsed() % 8 != 0) {
    ...
    RakAssert(
        counter !=
        256); // Given 256 elements, we should always be able to find an input that would be >= 7 bits

#endif
  }
}

unsigned HuffmanEncodingTree::DecodeArray(
