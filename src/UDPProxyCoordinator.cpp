...
#include "NativeFeatureIncludes.h"
    ...
    _RAKNET_SUPPORT_UDPForwarder == 1
...
static const int DEFAULT_UNRESPONSIVE_PING_TIME_COORDINATOR =
    DEFAULT_CLIENT_UNRESPONSIVE_PING_TIME + 1000;
...
int UDPProxyCoordinator::ForwardingRequestComp(
    const SenderAndTargetAddress& key,
  ...
  while (idx < forwardingRequestList.Size()) {
    ...
    if (fw->timeRequestedPings != 0 &&
      ...
      idx++;
        ...
        fw->timeoutAfterSuccess != 0 && curTime > fw->timeoutAfterSuccess) {
...
void UDPProxyCoordinator::OnForwardingRequestFromClientToCoordinator(
  ...
  sata.senderClientGuid =
      rakPeerInterface->GetGuidFromSystemAddress(sourceAddress);
  ...
  if (rakPeerInterface->GetConnectionState(targetAddress) != IS_CONNECTED &&
      usesAddress == false) {
    outgoingBs.Write((MessageID)ID_UDP_PROXY_GENERAL);
    outgoingBs.Write(
        (MessageID)ID_UDP_PROXY_RECIPIENT_GUID_NOT_CONNECTED_TO_COORDINATOR);
    outgoingBs.Write(sata.senderClientAddress);
    outgoingBs.Write(targetAddress);
    outgoingBs.Write(targetGuid);
    rakPeerInterface->Send(
  ...
  if (serverList.Size() > 1) {
    outgoingBs.Write((MessageID)ID_UDP_PROXY_GENERAL);
    outgoingBs.Write(
        (MessageID)ID_UDP_PROXY_PING_SERVERS_FROM_COORDINATOR_TO_CLIENT);
...
void UDPProxyCoordinator::SendForwardingRequest(
  ...
  outgoingBs.Write((MessageID)ID_UDP_PROXY_GENERAL);
  outgoingBs.Write(
      (MessageID)ID_UDP_PROXY_FORWARDING_REQUEST_FROM_COORDINATOR_TO_SERVER);
  outgoingBs.Write(sourceAddress);
  outgoingBs.Write(targetAddress);
...
void UDPProxyCoordinator::OnPingServersReplyFromClientToCoordinator(
    Packet* packet) {
  RakNet::BitStream incomingBs(packet->data, packet->length, false);
  incomingBs.IgnoreBytes(2);
...
void UDPProxyCoordinator::TryNextServer(
  ...
  while (fw->remainingServersToTry.Size() > 0) {
    ...
    if (serverList.GetIndexOf(fw->currentlyAttemptedServerAddress) !=
        (unsigned int)-1) {
  ...
  if (pickedGoodServer == false) {
    SendAllBusy(
        sata.senderClientAddress,
        sata.targetClientAddress,
        sata.targetClientGuid,
        fw->requestingAddress);
...
void UDPProxyCoordinator::SendAllBusy(
...
void UDPProxyCoordinator::ForwardingRequest::OrderRemainingServersToTry(void) {
  ...
  DataStructures::OrderedList<
      unsigned short,
      UDPProxyCoordinator::ServerWithPing,
      ServerWithPingComp>
      swpList;
