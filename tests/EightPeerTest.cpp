...
int EightPeerTest::RunTest(
  ...
  int connectionAmount
      [peerNum]; //Counter for me to keep track of connection requests and accepts
  int recievedFromList
      [peerNum][peerNum]; //Counter for me to keep track of packets received
  int lastNumberReceivedFromList
      [peerNum]
      [peerNum]; //Counter for me to keep track of last recieved sequence number
  ...
  for (
      int k = 0; k < numPackets ||
      GetTimeMS() - finishTimer <
          5000;) //Quit after we send 100 messages while connected, if not all connected and not failure, otherwise fail after 20 seconds and exit
  {
    bool allConnected =
        true; //Start true, only one failed case makes it all fail
    for (int i = 0; i < peerNum;
         i++) //Make sure all peers are connected to eachother
    {
    ...
    if (k >= numPackets -
            3) //This is our last 3 packets, give it time to send packet and arrive on interface, 2 seconds is more than enough
    {
      RakSleep(300);
      if (k == numPackets) {
    ...
    for (int i = 0; i < peerNum; i++) //Receive for all peers
    {
      if (allConnected) //If all connected try to make the data more visually appealing by bunching it in one receive
      {
        ...
        do {
          ...
          if (waittime >
              1000) //Check for packet every millisec and if one second has passed move on, don't block execution
          {
            break;
          }
        } while (
            !packet); //For testing purposes wait for packet a little while, go if not recieved
      ...
      while (packet) {
        switch (packet->data[0]) {
          ...
          case ID_NO_FREE_INCOMING_CONNECTIONS: //Should not happend
            if (isVerbose) {
              printf(
                  "The server is full. This shouldn't happen in this test ever.\n");
    ...
    for (int j = 0; j < peerNum; j++) {
      if (i != j) {
        ...
        if (recievedFromList[i][j] != numPackets) {
          if (isVerbose) {
            printf(
                "Not all packets recieved. it was in reliable ordered mode so that means test failed or wait time needs increasing\n");
