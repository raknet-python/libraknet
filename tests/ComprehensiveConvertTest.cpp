...
int ComprehensiveConvertTest::RunTest(
  ...
  while (GetTimeMS() < endTime) {
    ...
    if (nextAction < .04f) {
      ...
      sprintf(
          data + 3,
          "dataLength=%i priority=%i reliability=%i orderingChannel=%i target=%i broadcast=%i\n",
          dataLength,
          priority,
          reliability,
          orderingChannel,
      ...
      // Offline Ping
      peerIndex = randomMT() % NUM_PEERS;
      peers[peerIndex]->Ping(
          "127.0.0.1", 60000 + (randomMT() % NUM_PEERS), (randomMT() % 2) > 0);
