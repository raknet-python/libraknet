...
void ManyClientsOneServerDeallocateBlockingTest::WaitAndPrintResults(
    RakPeerInterface** clientList,
    int clientNum,
    bool isVerbose,
...
*/
int ManyClientsOneServerDeallocateBlockingTest::RunTest(
  ...
  if (retTimeout != timeoutTime) {
    if (isVerbose) {
      DebugTools::ShowError(
          "GetTimeoutTime did not match the timeout that was set. \n",
          !noPauses && isVerbose,
          __LINE__,
          __FILE__);
    }
  ...
  for (int i = 0; i < clientNum; i++) {
    ...
    if (connNum != 1) //Did we connect all?
    {
      if (isVerbose) {
        printf(
            "Not all clients reconnected normally.\nFailed on client number %i\n",
            i);
...
RakString ManyClientsOneServerDeallocateBlockingTest::ErrorCodeToString(
    int errorCode) {
  switch (errorCode) {
    case 0:
      return "No error";
...
ManyClientsOneServerDeallocateBlockingTest::
    ManyClientsOneServerDeallocateBlockingTest(void) = default;
