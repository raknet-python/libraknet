...
*/
int PacketAndLowLevelTestsTest::RunTest(
  ...
  const int dataSize =
      3000000; //around 30 meg didn't want to calculate the exact
  ...
  while (CommonFunctions::WaitForMessageWithID(
      server, ID_DOWNLOAD_PROGRESS, 500)) //Clear out the rest before next test
  {
  }
...
void PacketAndLowLevelTestsTest::FloodWithHighPriority(
    RakPeerInterface* client) {
  for (int i = 0; i < 60000; i++) {
    TestHelpers::BroadCastTestPacket(
        client, UNRELIABLE, HIGH_PRIORITY, ID_USER_PACKET_ENUM + 2);
  }
}
...
PacketAndLowLevelTestsTest::PacketAndLowLevelTestsTest(void) {
  ...
  errorList.Push(
      "Plugin is still modifying packets after detach", _FILE_AND_LINE_);
  errorList.Push(
      "GetSplitMessageProgressInterval returned wrong value", _FILE_AND_LINE_);
  ...
  errorList.Push("Client failed to connect to server", _FILE_AND_LINE_);
  errorList.Push(
      "PushBackPacket messed up future communication", _FILE_AND_LINE_);
