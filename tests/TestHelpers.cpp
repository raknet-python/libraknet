...
void TestHelpers::StandardServerPrep(
...
void TestHelpers::StandardClientPrep(
...
bool TestHelpers::WaitAndConnectTwoPeersLocally(
    RakPeerInterface* connector,
    RakPeerInterface* connectee,
    int millisecondsToWait) {
  SystemAddress connecteeAdd = connectee->GetInternalID();
  return CommonFunctions::WaitAndConnect(
      connector, "127.0.0.1", connecteeAdd.GetPort(), millisecondsToWait);
}

//returns false if connect fails
bool TestHelpers::ConnectTwoPeersLocally(
    RakPeerInterface* connector,
    RakPeerInterface* connectee) {
  SystemAddress connecteeAdd = connectee->GetInternalID();
...
bool TestHelpers::BroadCastTestPacket(
    RakPeerInterface* sender,
    PacketReliability rel,
    PacketPriority pr,
    int typeNum) //returns send return value
{
...
bool TestHelpers::SendTestPacketDirected(
    RakPeerInterface* sender,
    char* ip,
    int port,
    PacketReliability rel,
    ...
    int typeNum) //returns send return value
{
  ...
  recAddress.SetPortHostOrder(port);

  char str2[] = "AAAAAAAAAA";
  str2[0] = typeNum;
  return sender->Send(
             str2, (int)strlen(str2) + 1, pr, rel, 0, recAddress, false) > 0;
}

bool TestHelpers::WaitForTestPacket(
