...
*/
int PeerConnectDisconnectWithCancelPendingTest::RunTest(
    DataStructures::List<RakString> params,
    bool isVerbose,
    bool noPauses) {
  const int peerNum = 8;
  const int maxConnections = peerNum *
      3; //Max allowed connections for test set to times 3 to eliminate problem variables
  ...
  while (GetTimeMS() - entryTime < 10000) //Run for 10 Secoonds
  {
    ...
    for (int i = 0; i < peerNum; i++) {
           ...
           j++) //Start at i+1 so don't connect two of the same together.
      {
        ...
        peerList[i]->CancelConnectionAttempt(
            currentSystem); //Make sure a connection is not pending before trying to connect.
      }
    }
           ...
           j++) //Start at i+1 so don't connect two of the same together.
      {
        if (peerList[i]->Connect("127.0.0.1", 60000 + j, nullptr, 0) !=
          ...
          if (CommonFunctions::ConnectionStateMatchesOptions(
                  ...
                  false,
                  true,
                  true)) //Did we drop the pending connection?
          {
            if (isVerbose) {
              DebugTools::ShowError(
                  "Did not cancel the pending request \n",
                  !noPauses && isVerbose,
                  __LINE__,
                  __FILE__);
            }
          ...
          if (!CommonFunctions::ConnectionStateMatchesOptions(
                  peerList[i], currentSystem, true, true, true, true)) {
            if (isVerbose) {
              DebugTools::ShowError(
                  "Problem while calling connect. \n",
                  !noPauses && isVerbose,
  ...
  while (GetTimeMS() - entryTime <
         2000) //Run for 2 Secoonds to process incoming disconnects
  {
    for (int i = 0; i < peerNum; i++) //Receive for all peers
    {
      printedYet = false;
  ...
  for (int i = 0; i < peerNum; i++) {
    ...
    if (connNum != peerNum - 1) //Did we connect to all?
    {
      if (isVerbose) {
        DebugTools::ShowError(
            "Not all peers reconnected normally.\n ",
            !noPauses && isVerbose,
            __LINE__,
            __FILE__);
      }
...
RakString PeerConnectDisconnectWithCancelPendingTest::ErrorCodeToString(
    int errorCode) {
  switch (errorCode) {
    case 0:
      return "No error";
...
PeerConnectDisconnectWithCancelPendingTest::
    PeerConnectDisconnectWithCancelPendingTest(void) = default;
