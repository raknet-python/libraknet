...
int PingTestsTest::RunTest(
  ...
  sender2->SetOccasionalPing(
      false); //Test the lowest ping and such without  occassionalping,occasional ping comes later
  RakTimer timer(1500);
  ...
  CommonFunctions::DisconnectAndWait(
      sender2, "127.0.0.1", 60000); //Eliminate variables.
...
int PingTestsTest::TestAverageValue(
    int averagePing,
    int line,
    bool noPauses,
    bool isVerbose) {
