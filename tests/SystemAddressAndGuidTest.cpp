...
*/
int SystemAddressAndGuidTest::RunTest(
  ...
  if (conNum != len) {
    if (conNum == 1 || len == 1) {
      if (conNum != 1) {
        printf(
            "system list size is %i and NumberOfConnections return is %i ",
            len,
            conNum);
  ...
  printf(
      "Test GetExternalID, automatic testing is not only required for this\nbecause of it's nature\nShould be supplemented by internet tests\n");
...
bool SystemAddressAndGuidTest::compareSystemAddresses(
    SystemAddress ad1,
    SystemAddress ad2) {
  return ad1 == ad2;
}
...
SystemAddressAndGuidTest::SystemAddressAndGuidTest() {
  ...
  errorList.Push(
      "Mismatch between guidList size and systemList size ", _FILE_AND_LINE_);
  ...
  errorList.Push(
      ...
      _FILE_AND_LINE_);
  errorList.Push(
      "GetGUIDFromIndex failed to return correct values", _FILE_AND_LINE_);
  errorList.Push(
