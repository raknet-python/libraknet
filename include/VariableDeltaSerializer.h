...
class RAK_DLL_EXPORT VariableDeltaSerializer {
 ...
 public:
  ...
  void BeginUnreliableAckedSerialize(
  ...
  void
  OnMessageReceipt(RakNetGUID guid, uint32_t receiptId, bool messageArrived);
  ...
  void SerializeVariable(
    ...
    if (context->newSystemSend) {
      if (context->variableHistory->variableListDeltaTracker
        ...
        context->variableHistory->variableListDeltaTracker.WriteVarToBitstream(
 ...
 protected:
  ...
  void StoreChangedVariablesList(
      RemoteSystemVariableHistory* variableHistory,
      ChangedVariablesList* changedVariables,
      uint32_t sendReceipt);
