...
void OrderedChannelHeap<
    ...
    Push(const channel_key_type& channelID, const heap_data_type& data) {
...
void OrderedChannelHeap<
  ...
  if (queueAndWeight->randResultQueue.Size() == 0) {
    ...
    maxRange =
        queueAndWeight
            ->randResultQueue[queueAndWeight->randResultQueue.Size() - 1] *
        .99999999;
    minRange = 0.0;
  } else {
    if (index == 0) {
...
void OrderedChannelHeap<
    ...
    heap_data_type,
    channel_key_comparison_func>::
    AddChannel(const channel_key_type& channelID, const double weight) {
