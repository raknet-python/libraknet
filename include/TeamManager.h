...
class RAK_DLL_EXPORT TM_TeamMember {
 public:
  ...
  void GetRequestedSpecificTeams(
 ...
 protected:
  ...
  bool JoinSpecificTeamCheck(TM_Team* specificTeamToJoin, bool ignoreRequested)
      const;
  bool SwitchSpecificTeamCheck(
      TM_Team* teamToJoin,
      TM_Team* teamToLeave,
...
class TM_World {
 ...
 protected:
  ...
  unsigned int GetAvailableTeamIndexWithFewestMembers(
      TeamMemberLimit secondaryLimit,
      JoinPermissions joinPermissions);

  void GetSortedJoinRequests(DataStructures::OrderedList<
                             JoinRequestHelper,
                             JoinRequestHelper,
                             JoinRequestHelperComp>& joinRequests);

  // Send a message to all participants
  void BroadcastToParticipants(
...
class RAK_DLL_EXPORT TeamManager : public PluginInterface2 {
 ...
 protected:
  ...
  void EncodeTeamFullOrLocked(
      RakNet::BitStream* bitStream,
      TM_TeamMember* teamMember,
      TM_Team* team);
  void DecomposeTeamFullOrLocked(
  ...
  void EncodeTeamAssigned(
      RakNet::BitStream* bitStream,
      TM_TeamMember* teamMember);
  void RemoveFromTeamsRequestedAndAddTeam(
      TM_TeamMember* teamMember,
      TM_Team* team,
      bool isTeamSwitch,
      TM_Team* teamToLeave);
