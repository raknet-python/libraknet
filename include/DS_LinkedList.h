...
template <class LinkedListType>
bool LinkedList<LinkedListType>::operator=(
    const LinkedList<LinkedListType>& original_copy) {
  ...
  if ((&original_copy) != this) {
    ...
    if (original_copy.list_size == 0) {
      ...
      do {
        ...
        this->position =
            RakNet::OP_NEW<typename LinkedList::node>(_FILE_AND_LINE_);
...
template <class CircularLinkedListType>
CircularLinkedList<CircularLinkedListType>::CircularLinkedList(
    const CircularLinkedList& original_copy) {
  ...
  if (original_copy.list_size == 0) {
    ...
    this->root =
        RakNet::OP_NEW<typename CircularLinkedList::node>(_FILE_AND_LINE_);
    // root->item = RakNet::OP_NEW<CircularLinkedListType>( _FILE_AND_LINE_ );
    this->root->next = this->root;
    this->root->previous = this->root;
    this->list_size = 1;
...
#endif
template <class CircularLinkedListType>
bool CircularLinkedList<CircularLinkedListType>::operator=(
    const CircularLinkedList& original_copy) {
  node* original_copy_pointer;
  node* last;
  node* save_position;
...
template <class CircularLinkedListType>
void CircularLinkedList<CircularLinkedListType>::Insert(
  ...
  if (list_size == 0) {
    ...
    this->root =
        this->position; // Since we're inserting into a 1 element list the old root is now the second item
    this->list_size = 2;
  }
...
template <class CircularLinkedListType>
CircularLinkedListType& CircularLinkedList<CircularLinkedListType>::Add(
    const CircularLinkedListType& input) {
  node* new_node;
...
template <class CircularLinkedListType>
inline void CircularLinkedList<CircularLinkedListType>::Replace(
    const CircularLinkedListType& input) {
  if (this->list_size > 0)
    // *(position->item)=input;
    this->position->item = input;
...
template <class CircularLinkedListType>
bool CircularLinkedList<CircularLinkedListType>::IsIn(
    const CircularLinkedListType& input) {
...
template <class CircularLinkedListType>
bool CircularLinkedList<CircularLinkedListType>::Find(
    const CircularLinkedListType& input) {
  node* return_value;
...
template <class CircularLinkedListType>
typename CircularLinkedList<CircularLinkedListType>::node*
CircularLinkedList<CircularLinkedListType>::FindPointer(
    const CircularLinkedListType& input) {
  node* current;
...
void CircularLinkedList<CircularLinkedListType>::Clear(void) {
  if (this->list_size == 0)
    return;
      ...
      this->list_size ==
      1) // {RakNet::OP_DELETE(root->item); RakNet::OP_DELETE(root, _FILE_AND_LINE_);}
  {
    RakNet::OP_DELETE(this->root, _FILE_AND_LINE_);
  }
...
template <class CircularLinkedListType>
CircularLinkedList<CircularLinkedListType>
CircularLinkedList<CircularLinkedListType>::Mergesort(
