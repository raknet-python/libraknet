...
class RAK_DLL_EXPORT BitStream {
 public:
  ...
  template <
      class
      templateType> // templateType for this function must be a float or double
  ...
  template <class templateType>
  void WriteDelta(
      const templateType& currentValue,
      const templateType& lastValue);
  ...
  template <
      ...
      templateType> // templateType for this function must be a float or double
  void
  WriteNormQuat(templateType w, templateType x, templateType y, templateType z);
  ...
  template <>
  void WriteDelta(
      const SystemAddress& currentValue,
...
bool BitStream::SerializeBitsFromIntegerRange(
  ...
  return SerializeBitsFromIntegerRange(
      writeToBitstream,
      value,
      minimum,
      maximum,
...
template <class templateType>
inline bool BitStream::SerializeNormQuat(
    bool writeToBitstream,
    templateType& w,
    templateType& x,
    templateType& y,
...
inline bool BitStream::SerializeBits(
    bool writeToBitstream,
    unsigned char* inOutByteArray,
    const BitSize_t numberOfBitsToSerialize,
...
inline void BitStream::Write(const SystemAddress& inTemplateVar) {
  ...
  if (inTemplateVar.GetIPVersion() == 4) {
    ...
    // Don't endian swap
    WriteBits(
        (const unsigned char*)&inTemplateVar.address.addr6,
        sizeof(inTemplateVar.address.addr6) * 8,
        true);
#endif
  }
}
...
#ifdef _MSC_VER
#pragma warning( \
    disable : 4244) // '=' : conversion from 'unsigned long' to 'unsigned short', possible loss of data
#endif
...
inline bool BitStream::Read(SystemAddress& outTemplateVar) {
  ...
  if (ipVersion == 4) {
    ...
    bool b = ReadBits(
        (unsigned char*)&outTemplateVar.address.addr4.sin_port,
        sizeof(outTemplateVar.address.addr4.sin_port) * 8,
        true);
...
#if RAKNET_SUPPORT_IPV6 == 1
    bool b = ReadBits(
        (unsigned char*)&outTemplateVar.address.addr6,
        sizeof(outTemplateVar.address.addr6) * 8,
        true);
