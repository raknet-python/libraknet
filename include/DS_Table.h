...
class RAK_DLL_EXPORT Table {
 public:
  ...
  struct RAK_DLL_EXPORT Cell {
    ...
    void
    SetByType(double numericValue, char* charValue, void* ptr, ColumnType type);
    void Clear();
  ...
  bool
  UpdateCell(unsigned rowId, unsigned columnIndex, int byteLength, char* data);
  ...
  void
  GetCellValueByIndex(unsigned rowIndex, unsigned columnIndex, int* output);
  void
  GetCellValueByIndex(unsigned rowIndex, unsigned columnIndex, char* output);
  ...
  void PrintColumnHeaders(char* out, int outLength, char columnDelineator)
      const;
  ...
  /// \brief Direct access to make things easier.
  const DataStructures::BPlusTree<unsigned, Row*, _TABLE_BPLUS_TREE_ORDER>&
  GetRows() const;
