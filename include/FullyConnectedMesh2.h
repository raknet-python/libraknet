...
class RAK_DLL_EXPORT FullyConnectedMesh2 : public PluginInterface2 {
 public:
  ...
  /// \param[out] additionalData \a additionalData parameter passed to RespondOnVerifiedJoinCapable().
  virtual void GetVerifiedJoinRejectedAdditionalData(
      Packet* packet,
      BitStream* additionalData);

  /// Override to write data when ID_FCM2_VERIFIED_JOIN_CAPABLE is sent
  virtual void WriteVJCUserData(RakNet::BitStream* bsOut) {
    (void)bsOut;
  }
 ...
 protected:
  ...
  virtual void OnVerifiedJoinFailed(
      RakNetGUID hostGuid,
  ...
  void UpdateVerifiedJoinInProgressMember(
