...
class RAK_DLL_EXPORT WeightedGraph {
 public:
  ...
  bool GetSpanningTree(
      DataStructures::Tree<node_type>& outTree,
...
WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::WeightedGraph(
  ...
  if (isValidPath) {
    ...
    memcpy(
        costMatrix,
        original_copy.costMatrix,
        costMatrixIndices.Size() * costMatrixIndices.Size() *
            sizeof(weight_type));
    memcpy(
        leastNodeArray,
        original_copy.leastNodeArray,
...
WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::operator=(
    const WeightedGraph& original_copy) {
  adjacencyLists = original_copy.adjacencyLists;
...
template <class node_type, class weight_type, bool allow_unlinkedNodes>
void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::RemoveNode(
    const node_type& node) {
  unsigned i;
...
#pragma warning( \
      ...
      false) // If we do not allow _unlinked nodes, then if there are no connections, remove the node
  {
    if (adjacencyLists.Get(node1)->Size() == 0)
      RemoveNode(
          node1); // Will also remove node1 from the adjacency list of node2
...
template <class node_type, class weight_type, bool allow_unlinkedNodes>
node_type
WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetNodeAtIndex(
...
template <class node_type, class weight_type, bool allow_unlinkedNodes>
unsigned
WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetConnectionCount(
    unsigned nodeIndex) const {
...
bool WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::
  ...
  for (i = 0; i < adjacencyList->Size(); i++) {
    nap2.node =
        RakNet::OP_NEW<DataStructures::Tree<node_type>>(_FILE_AND_LINE_);
...
void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::
  ...
  costMatrix = RakNet::OP_NEW_ARRAY<weight_type>(
      adjacencyLists.Size() * adjacencyLists.Size(), _FILE_AND_LINE_);
  leastNodeArray =
      RakNet::OP_NEW_ARRAY<node_type>(adjacencyLists.Size(), _FILE_AND_LINE_);
  ...
  while (row < adjacencyLists.Size() - 1) {
    ...
    for (openSetIndex = 0; openSetIndex < openSet.Size(); openSetIndex++)
      minHeap.Push(
          ...
          openSet.GetKeyAtIndex(openSetIndex),
          _FILE_AND_LINE_);
