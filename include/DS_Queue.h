...
class RAK_DLL_EXPORT Queue {
 public:
  ...
  void PushAtHead(
      const queue_type& input,
      unsigned index,
      const char* file,
      unsigned int line);
  queue_type& operator[](unsigned int position)
      const; // Not a normal thing you do with a queue but can be used for efficiency
  void RemoveAtIndex(
      unsigned int
...
template <class queue_type>
void Queue<queue_type>::PushAtHead(
    const queue_type& input,
    unsigned index,
    const char* file,
    unsigned int line) {
...
void Queue<queue_type>::Push(
  ...
  if (tail == head) {
    ...
    new_array =
        RakNet::OP_NEW_ARRAY<queue_type>((int)allocation_size * 2, file, line);
#ifdef _DEBUG
    RakAssert(new_array);
#endif
    if (new_array == 0)
...
Queue<queue_type>::Queue(Queue& original_copy) {
  ...
  if (original_copy.Size() == 0) {
    ...
    for (unsigned int counter = 0; counter < original_copy.Size(); ++counter)
      array[counter] = original_copy.array
                           [(original_copy.head + counter) %
...
void Queue<queue_type>::Compress(const char* file, unsigned int line) {
  ...
  while (newAllocationSize <= Size())
    newAllocationSize <<=
        1; // Must be a better way to do this but I'm too dumb to figure it out quickly :)
