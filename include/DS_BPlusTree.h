...
class RAK_DLL_EXPORT BPlusTree {
 ...
 protected:
  ...
  void RotateLeft(
      ...
      int childIndex,
      ReturnAction* returnAction);
  Page<KeyType, DataType, order>* InsertIntoNode(
      const KeyType key,
      ...
      Page<KeyType, DataType, order>* cur,
      ReturnAction* returnAction);
  Page<KeyType, DataType, order>* InsertBranchDown(
      const KeyType key,
      const DataType& data,
      Page<KeyType, DataType, order>* cur,
      ReturnAction* returnAction,
...
bool BPlusTree<KeyType, DataType, order>::Delete(
  ...
  if (root == leftmostLeaf) {
    ...
    return true;
      ...
      FindDeleteRebalance(
          key, root, &underflow, root->keys[0], &returnAction, out) == false)
    return false;
...
bool BPlusTree<KeyType, DataType, order>::FindDeleteRebalance(
  ...
  if (cur->children[branchIndex]->isLeaf == false) {
    ...
    if (returnAction->action == ReturnAction::SET_BRANCH_KEY &&
        branchIndex != childIndex) {
      returnAction->action = ReturnAction::NO_ACTION;
...
#pragma warning( \
    ...
    if (order <= 3 && branchIndex > 0 &&
        cur->children[branchIndex]
            ->isLeaf) // With order==2 it is possible to delete data[0], which is not possible with higher orders.
      cur->keys[branchIndex - 1] = cur->children[branchIndex]->keys[0];
...
BPlusTree<KeyType, DataType, order>::InsertIntoNode(
  ...
  if (cur->size < order) {
    ...
    Page<KeyType, DataType, order>* newPage =
        pagePool.Allocate(_FILE_AND_LINE_);
...
template <class KeyType, class DataType, int order>
bool BPlusTree<KeyType, DataType, order>::CanRotateLeft(
    Page<KeyType, DataType, order>* cur,
    int childIndex) {
...
template <class KeyType, class DataType, int order>
void BPlusTree<KeyType, DataType, order>::RotateLeft(
    Page<KeyType, DataType, order>* cur,
    int childIndex,
    ReturnAction* returnAction) {
...
template <class KeyType, class DataType, int order>
bool BPlusTree<KeyType, DataType, order>::CanRotateRight(
    Page<KeyType, DataType, order>* cur,
    int childIndex) {
...
template <class KeyType, class DataType, int order>
void BPlusTree<KeyType, DataType, order>::RotateRight(
    Page<KeyType, DataType, order>* cur,
    int childIndex,
    ReturnAction* returnAction) {
  Page<KeyType, DataType, order>* dest = cur->children[childIndex + 1];
...
BPlusTree<KeyType, DataType, order>::InsertBranchDown(
  ...
  Page<KeyType, DataType, order>* newPage;
  if (cur->isLeaf == false) {
    if (cur->children[branchIndex]->isLeaf == true &&
        cur->children[branchIndex]->size == order) {
      ...
      if (CanRotateLeft(cur, branchIndex)) {
        ...
        if (key < cur->children[branchIndex]
                      ->keys[cur->children[branchIndex]->size - 1]) {
...
bool BPlusTree<KeyType, DataType, order>::Insert(
  ...
  if (root == 0) {
    ...
    returnAction.action = ReturnAction::NO_ACTION;
    Page<KeyType, DataType, order>* newPage =
        InsertBranchDown(key, data, root, &returnAction, &success);
    if (success == false)
      return false;
    if (newPage) {
      KeyType newKey;
      ...
      // propagate the root
      Page<KeyType, DataType, order>* newRoot =
          pagePool.Allocate(_FILE_AND_LINE_);
...
template <class KeyType, class DataType, int order>
void BPlusTree<KeyType, DataType, order>::ValidateTreeRecursive(
    Page<KeyType, DataType, order>* cur) {
