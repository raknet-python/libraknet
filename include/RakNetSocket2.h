...
class RAK_DLL_EXPORT RNS2EventHandler {
 public:
  ...
  virtual void DeallocRNS2RecvStruct(
      RNS2RecvStruct* s,
      const char* file,
      unsigned int line) = 0;
...
class RNS2_WindowsStore8 : public RakNetSocket2 {
 public:
  ...
  static int WinRTIOCTLSocket(
      Windows::Networking::Sockets::DatagramSocket ^ s,
      long cmd,
      unsigned long* argp);

  static int WinRTGetSockName(
      Windows::Networking::Sockets::DatagramSocket ^ s,
      struct sockaddr* name,
      socklen_t* namelen);

  static RNS2_WindowsStore8* GetRNS2FromDatagramSocket(
      Windows::Networking::Sockets::DatagramSocket ^ s);
...
class RNS2_NativeClient : public RakNetSocket2 {
 ...
 protected:
  ...
  static RNS2_SendParameters_NativeClient* CloneSP(
...
class RNS2_Berkley : public IRNS2_Berkley {
 ...
 protected:
  ...
  RNS2BindResult BindShared(
      RNS2_BerkleyBindParameters* bindParameters,
      const char* file,
      unsigned int line);
  RNS2BindResult BindSharedIPV4(
      RNS2_BerkleyBindParameters* bindParameters,
      const char* file,
      unsigned int line);
  RNS2BindResult BindSharedIPV4And6(
      RNS2_BerkleyBindParameters* bindParameters,
      const char* file,
      unsigned int line);

  static void GetSystemAddressIPV4(
      RNS2Socket rns2Socket,
      SystemAddress* systemAddressOut);
  static void GetSystemAddressIPV4And6(
      RNS2Socket rns2Socket,
      SystemAddress* systemAddressOut);
...
class RNS2_Windows_Linux_360 {
 public:
 protected:
  static RNS2SendResult Send_Windows_Linux_360NoVDP(
      RNS2Socket rns2Socket,
      RNS2_SendParameters* sendParameters,
      const char* file,
      unsigned int line);
...
class RAK_DLL_EXPORT SocketLayerOverride {
 public:
  ...
  virtual int RakNetRecvFrom(
      char dataOut[MAXIMUM_MTU_SIZE],
      SystemAddress* senderOut,
