...
class RAK_DLL_EXPORT ReplicaManager3 : public PluginInterface2 {
 public:
  ...
  Connection_RM3* GetConnectionAtIndex(unsigned index, WorldId worldId = 0)
      const;
  ...
  Connection_RM3* GetConnectionBySystemAddress(
      const SystemAddress& sa,
      WorldId worldId = 0) const;

  /// \brief Returns a connection pointer previously added with PushConnection.()
  /// \param[in] guid The guid of the connection to return
  /// \param[in] worldId Used for multiple worlds. World 0 is created automatically by default. See AddWorld()
  /// \return A Connection_RM3 pointer, or 0 if not found
  Connection_RM3* GetConnectionByGUID(RakNetGUID guid, WorldId worldId = 0)
      const;
 ...
 protected:
  ...
  PluginReceiveResult OnConstruction(
      Packet* packet,
      ...
      WorldId worldId);
  PluginReceiveResult OnSerialize(
      Packet* packet,
      unsigned char* packetData,
      int packetDataLength,
      RakNetGUID senderGuid,
      ...
      unsigned char packetDataOffset,
      WorldId worldId);
  PluginReceiveResult OnDownloadStarted(
      Packet* packet,
      unsigned char* packetData,
      int packetDataLength,
      RakNetGUID senderGuid,
      unsigned char packetDataOffset,
      WorldId worldId);
  PluginReceiveResult OnDownloadComplete(
      Packet* packet,
      unsigned char* packetData,
      int packetDataLength,
      RakNetGUID senderGuid,
...
class RAK_DLL_EXPORT Connection_RM3 {
 public:
  ...
  virtual ConstructionMode QueryConstructionMode() const {
  ...
  virtual bool QuerySerializationList(
  ...
  /// \param[in] worldId Which world, see ReplicaManager3::AddWorld()
  /// \param[in] curTime The current time
  virtual SendSerializeIfChangedResult SendSerializeIfChanged(
 ...
 protected:
  ...
  // Working lists
  DataStructures::List<Replica3*> constructedReplicasCulled,
...
class RAK_DLL_EXPORT Replica3 : public NetworkIDObject {
 public:
  ...
  virtual RM3ConstructionState QueryConstruction_ClientConstruction(
      RakNet::Connection_RM3* destinationConnection,
  ...
  /// \param[in] isThisTheServer True if this system is the server, false if not.
  virtual bool QueryRemoteConstruction_ClientConstruction(
      RakNet::Connection_RM3* sourceConnection,
      bool isThisTheServer);
  ...
  /// \param[in] destinationConnection destinationConnection parameter passed to QueryConstruction()
  /// \param[in] isThisTheServer True if this system is the server, false if not.
  virtual RM3ConstructionState QueryConstruction_ServerConstruction(
      RakNet::Connection_RM3* destinationConnection,
      bool isThisTheServer);
  ...
  /// \param[in] sourceConnection destinationConnection parameter passed to QueryConstruction()
  /// \param[in] isThisTheServer True if this system is the server, false if not.
  virtual bool QueryRemoteConstruction_ServerConstruction(
      RakNet::Connection_RM3* sourceConnection,
      bool isThisTheServer);
  ...
  virtual RM3ActionOnPopConnection QueryActionOnPopConnection_Client(
  ...
  virtual RM3ActionOnPopConnection QueryActionOnPopConnection_Server(
      RakNet::Connection_RM3* droppedConnection) const;
...
class RAK_DLL_EXPORT Replica3Composite : public Replica3 {
 ...
 public:
  ...
  void OnSerializeTransmission(
      ...
      RakNet::Connection_RM3* destinationConnection,
      RakNet::BitSize_t
          bitsPerChannel[RakNet::RM3_NUM_OUTPUT_BITSTREAM_CHANNELS],
      RakNet::Time curTime) override {
