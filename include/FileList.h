...
class RAK_DLL_EXPORT FileListProgress {
 public:
  ...
  virtual void
  OnFile(FileList* fileList, char* dir, char* fileName, unsigned int fileSize) {
    (void)fileList;
    (void)dir;
    (void)fileName;
    (void)fileSize;
...
class RAK_DLL_EXPORT FileList {
 public:
  ...
  void ListMissingOrChangedFiles(
      const char* applicationDirectory,
