...
class RAK_DLL_EXPORT RangeList {
 public:
  ...
  DataStructures::
      OrderedList<range_type, RangeNode<range_type>, RangeNodeComp<range_type>>
          ranges;
};
...
RakNet::BitSize_t RangeList<range_type>::Serialize(
  ...
  for (i = 0; i < ranges.Size(); i++) {
    if ((int)sizeof(unsigned short) * 8 + bitsWritten +
            (int)sizeof(range_type) * 8 * 2 + 1 >
        maxBits)
      break;
    ...
    if (ranges[i].minIndex == ranges[i].maxIndex)
      ...
      minEqualsMax = 0;
    tempBS.Write(
        minEqualsMax); // Use one byte, intead of one bit, for speed, as this is done a lot
...
void RangeList<range_type>::Insert(range_type index) {
  ...
  if (index < ranges[insertionIndex].minIndex - (range_type)1) {
    ...
    if (insertionIndex > 0 &&
        ranges[insertionIndex - 1].maxIndex + (range_type)1 ==
            ranges[insertionIndex].minIndex) {
    ...
    return;
      ...
      index >= ranges[insertionIndex].minIndex &&
    ...
    if (insertionIndex < ranges.Size() - 1 &&
        ranges[insertionIndex + (range_type)1].minIndex ==
            ranges[insertionIndex].maxIndex + (range_type)1) {
