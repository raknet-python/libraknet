...
struct RAK_DLL_EXPORT SystemAddress {
  ...
  bool
  FromString(const char* str, char portDelineator = '|', int ipVersion = 0);
  ...
  /// Old version, for crap platforms that don't support newer socket functions
  void ToString_Old(bool writePort, char* dest, char portDelineator = ':')
      const;
...
struct RAK_DLL_EXPORT RakNetGUID {
  ...
  RakNetGUID& operator=(const RakNetGUID& input) = default;
