...
struct RAK_DLL_EXPORT ThreadPool {
 ...
 protected:
  ...
  RakNet::SimpleMutex inputQueueMutex, outputQueueMutex,
      workingThreadCountMutex, runThreadsMutex;
...
#pragma warning( \
    disable : 4701) // potentially uninitialized local variable 'inputData' used
#endif
...
*/
{
  ...
  if (threadPool->perThreadDataFactory)
    ...
    perThreadData = threadPool->threadDataInterface->PerThreadFactory(
        threadPool->tdiContext);
  else
    perThreadData = 0;
  ...
  if (threadPool->perThreadDataDestructor)
    ...
    threadPool->threadDataInterface->PerThreadDestructor(
        perThreadData, threadPool->tdiContext);
...
bool ThreadPool<InputType, OutputType>::StartThreads(
  ...
  for (i = 0; i < numThreads; i++) {
    ...
    errorCode =
        RakNet::RakThread::Create(WorkerThread<InputType, OutputType>, this);
...
template <class InputType, class OutputType>
void ThreadPool<InputType, OutputType>::SetThreadDataInterface(
